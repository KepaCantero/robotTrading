...
def save_multi_strategy_results(
    ...
    for strategy_name, result in consolidated_results.get("per_strategy", {}).items():
        metrics = {
            "name": strategy_name,
            "allocation_pct": result.get("initial_capital", 0) / consolidated_results["combined"]["total_initial_capital"] * 100,
    ...
    output_data = {
        ...
        "combined": {
            "total_pnl": combined.get("total_final_capital", 0) - combined.get("total_initial_capital", 0),
    ...
    # Save to JSON
    output_file = output_dir / f"multi_strategy_{backtest_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def generate_multi_strategy_summary_text(
    ...
    strategy_list = [
        (name, result) for name, result in per_strategy.items()
    ]
    if strategy_list:
    ...
    for strategy_name, result in per_strategy.items():
        if strategy_name == "pairs_trading":
            ...
            summary += f"- **Allocation**: {allocation.get(strategy_name, {}).get('weight', 0):.1%}\n"
