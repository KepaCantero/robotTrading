...
class ComprehensiveBacktestLoader:
    ...
    def load_all_results(self) -> List[Dict[str, Any]]:
        ...
        if not json_files:
            logger.debug(f"🔍 No se encontraron archivos JSON en {self.results_dir}. Buscando en estructura completa...")
            # Buscar también en el directorio padre (reports/)
            parent_dir = self.results_dir.parent if self.results_dir.name == "comprehensive_backtest" else self.results_dir.parent
        ...
        if json_files:
            ...
            logger.debug(f"ℹ️ No hay archivos JSON aún en {self.results_dir} (normal si no se han ejecutado tests)")
        ...
        for json_file in json_files:
            try:
                with open(json_file, 'r') as f:
                    ...
                    for result in file_results:
                        ...
                        if 'before_training_metrics' in result:
                            ...
                            if isinstance(before_metrics, str) and before_metrics.strip():
                                try:
                                    import ast
                                    result['before_training_metrics'] = ast.literal_eval(before_metrics) if before_metrics.startswith('{') else {}
                                except:
                                    try:
                                        result['before_training_metrics'] = json.loads(before_metrics)
                                    except:
                                        logger.debug(f"No se pudo parsear before_training_metrics: {before_metrics[:50]}")
                        ...
                        if 'after_training_metrics' in result:
                            ...
                            if isinstance(after_metrics, str) and after_metrics.strip():
                                try:
                                    import ast
                                    result['after_training_metrics'] = ast.literal_eval(after_metrics) if after_metrics.startswith('{') else {}
                                except:
                                    try:
                                        result['after_training_metrics'] = json.loads(after_metrics)
                                    except:
                                        logger.debug(f"No se pudo parsear after_training_metrics: {after_metrics[:50]}")
                        ...
                        if 'improvement_pct' in result:
                            ...
                            if isinstance(improvement, str) and improvement.strip():
                                try:
                                    import ast
                                    result['improvement_pct'] = ast.literal_eval(improvement) if improvement.startswith('{') else {}
                                except:
                                    try:
                                        result['improvement_pct'] = json.loads(improvement)
                                    except:
                                        logger.debug(f"No se pudo parsear improvement_pct: {improvement[:50]}")
                                        result['improvement_pct'] = {}
                        
                        # Contar learning engines con datos de entrenamiento
                        if result.get('test_type') == 'learning_engine' and (
                            result.get('before_training_metrics') or 
                            result.get('after_training_metrics') or 
                    ...
                    logger.debug(f"Cargado: {json_file.name} ({len(file_results)} resultados, {len(results)} total)")
    ...
    def get_summary_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            'worst_drawdown': float(df['max_drawdown'].min()) if 'max_drawdown' in df.columns else 0.0,
        }
    ...
    def load_meta_analysis_results(self) -> Optional[Dict[str, Any]]:
        ...
        meta_files = sorted(
            meta_dir.glob("meta_analysis_*.json"),
    ...
    def get_meta_analysis_summary(self) -> Optional[Dict[str, Any]]:
        ...
        summary = {
            ...
            'clusters_found': len(analysis_data.get('clusters', {})) if isinstance(analysis_data.get('clusters'), dict) else 0,
