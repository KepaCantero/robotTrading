...
from app.services.cost_analysis_service import CostAnalysisService, CostAnalysisResult
...
async def update_cost_parameters(
    ...
    try:
        if "commission_rates" in parameters:
            for asset_class, rate in parameters["commission_rates"].items():
                if rate > 0.1:  # Max 10% commission
                    raise ValueError(f"Commission rate for {asset_class} ({rate}) exceeds maximum allowed (0.1)")
        ...
        if "slippage_rates" in parameters:
            for asset_class, rate in parameters["slippage_rates"].items():
                if rate > 0.05:  # Max 5% slippage
                    raise ValueError(f"Slippage rate for {asset_class} ({rate}) exceeds maximum allowed (0.05)")
        ...
        if "infrastructure_cost_per_trade" in parameters:
            service.infrastructure_cost_per_trade = Decimal(str(parameters["infrastructure_cost_per_trade"]))
        ...
        if "profitability_thresholds" in parameters:
            ...
            if "min_profitability_threshold" in thresholds:
                service.min_profitability_threshold = Decimal(str(thresholds["min_profitability_threshold"]))
