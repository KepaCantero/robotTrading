...
from app.services.portfolio_analytics_service import get_portfolio_analytics_service, PortfolioAnalyticsService
from app.models.portfolio_analytics import (
    PerformanceMetrics, RiskMetrics, PortfolioAnalytics, PortfolioAllocation,
    PortfolioRebalance, PortfolioComparison, PerformancePeriod, ExtendedPortfolio
)
...
class PerformanceMetricsRequest(BaseModel):
    ...
    period: PerformancePeriod = Field(default=PerformancePeriod.MONTHLY, description="Performance period")
...
class RebalanceRequest(BaseModel):
    ...
    portfolio_id: UUID = Field(..., description="Portfolio ID")
    target_equity_allocation: Optional[Decimal] = Field(None, description="Target equity allocation percentage")
    target_cash_allocation: Optional[Decimal] = Field(None, description="Target cash allocation percentage")
    rebalance_threshold: Optional[Decimal] = Field(None, description="Rebalancing threshold percentage")
...
async def get_risk_metrics(
    ...
    try:
        ...
        return RiskMetricsResponse(
            success=False,
            error=f"Failed to get risk metrics: {str(e)}"
        )
...
async def get_rebalance_recommendation(
    ...
    try:
        ...
        if request.target_equity_allocation is not None or request.target_cash_allocation is not None:
        ...
        return RebalanceResponse(
            success=False,
            error=f"Failed to get rebalance recommendation: {str(e)}"
        )
...
async def compare_portfolios(
    ...
    try:
        ...
        return PortfolioComparisonResponse(
            success=False,
            error=f"Failed to compare portfolios: {str(e)}"
        )
