...
async def get_session(
    session_id: UUID, service: PaperTradingService = Depends(get_paper_trading_service)
) -> SessionResponse:
    """Get session by ID."""
...
async def list_sessions(
    portfolio_id: Optional[UUID] = Query(None, description="Filter by portfolio ID"),
...
async def get_trade(
    trade_id: UUID, service: PaperTradingService = Depends(get_paper_trading_service)
) -> TradeResponse:
    """Get trade by ID."""
...
async def get_config(
    config_id: UUID, service: PaperTradingService = Depends(get_paper_trading_service)
) -> PaperTradingConfig:
