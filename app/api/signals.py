    ...
    try:
        ...
        if signal:
            ...
            return SignalResponse(
                success=False,
                signal=None,
                message="No actionable signals available"
            )
...
async def update_thresholds(
    ...
    try:
        ...
        if not (0 <= liquidity_threshold <= 100):
            raise HTTPException(status_code=400, detail="Liquidity threshold must be between 0 and 100")
...
async def update_position_size_limit(
    ...
    try:
        if not (0 < max_percent <= 100):
            raise HTTPException(status_code=400, detail="Max position size must be between 0 and 100%")
