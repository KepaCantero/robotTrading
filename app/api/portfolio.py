...
from app.models.portfolio import Portfolio, Position, AssetUniverse, MarketRegimeData
...
async def get_positions(
    ...
    try:
        ...
        if portfolio is None:
            raise HTTPException(status_code=503, detail="Portfolio unavailable due to circuit breaker")
...
async def get_market_regime(
    ...
    try:
        ...
        if regime_data is None:
            raise HTTPException(status_code=404, detail=f"Market regime data for {symbol} not available")
        return regime_data
    except HTTPException:
        raise
    except Exception as e:
...
async def portfolio_health_check(
    ...
    try:
        ...
        return {
            "status": "unhealthy",
            "message": f"Portfolio service error: {str(e)}"
        }
