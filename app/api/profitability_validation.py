...
async def validate_strategy_profitability(
    ...
    try:
        ...
        logger.info(f"Validation completed for {request.strategy_name}: {result.validation.status.value}")
...
async def validate_multiple_strategies(
    ...
    try:
        ...
        if not requests:
            raise HTTPException(
                status_code=400,
                detail="At least one validation request is required"
            )
...
async def generate_validation_report(
    validations: List[ProfitabilityValidation],
    include_comparison: bool = Query(default=True, description="Incluir comparación de estrategias"),
