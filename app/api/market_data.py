...
from app.models.market_data import (
    ...
    DataFrequency, MarketDataStatus, MarketDataSubscription
)
...
class CreateFeedConfigRequest(BaseModel):
    ...
    supported_frequencies: List[DataFrequency] = Field(default_factory=list, description="Supported data frequencies")
    max_history_days: int = Field(default=365, ge=1, le=3650, description="Maximum historical data days")
...
class SubscribeRequest(BaseModel):
    ...
    frequency: DataFrequency = Field(default=DataFrequency.REAL_TIME, description="Data update frequency")
...
async def get_top_liquid_quotes(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error getting top liquid quotes: {str(e)}"
        )
...
async def create_feed_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error creating feed configuration: {str(e)}"
        )
...
async def list_feed_configs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error listing feed configurations: {str(e)}"
        )
...
async def get_feed_config(
    ...
    try:
        ...
        if config:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Feed configuration not found: {config_id}"
            )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error getting feed configuration: {str(e)}"
        )
...
async def connect_feed(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to connect to feed {config_id}"
            )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error connecting to feed: {str(e)}"
        )
...
async def disconnect_feed(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to disconnect from feed {config_id}"
            )
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error disconnecting from feed: {str(e)}"
        )
...
async def subscribe_to_symbols(
    ...
    try:
        success = await service.subscribe_to_symbols(
            request.symbols, request.feed_id
        )
