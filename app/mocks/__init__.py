...
class MockIBKRClient:
    ...
    def __init__(self, account_id: str = "DU123456"):
        ...
        self.original_orders: Dict[str, Order] = {}  # Store references to original orders
        self.market_data: Dict[str, MarketData] = {}
        self.balances: Dict[str, Decimal] = {"USD": Decimal("100000"), "AAPL": Decimal("1000")}  # Default balance
            ...
            if updated_order.order_type == OrderType.LIMIT:
                ...
                if market_data:
                    ...
                    if updated_order.side == OrderSide.BUY:
                        if updated_order.price < current_price * Decimal("0.9"):  # More than 10% below market
                        ...
                        if updated_order.price > current_price * Decimal("1.1"):  # More than 10% above market
                            updated_order.status = OrderStatus.REJECTED
                            updated_order.rejected_reason = "Insufficient funds"
                            order.status = OrderStatus.REJECTED
                            order.rejected_reason = "Insufficient funds"
    ...
    async def cancel_order(self, order_id: str) -> bool:
        ...
        if order_id in self.orders:
            ...
            if order.status in [OrderStatus.PENDING, OrderStatus.SUBMITTED, OrderStatus.REJECTED]:
    ...
    async def _execute_order(self, order: Order):
        ...
        order.filled_price = order.price if order.order_type == OrderType.LIMIT else self._get_market_price(order.symbol)
        ...
        if order.symbol in self.positions:
            ...
            if order.side == OrderSide.BUY:
                ...
                position.avg_price = (position.avg_price * (position.quantity - order.filled_quantity) + 
...
class MockBinanceClient:
    ...
    async def get_klines(self, symbol: str, interval: str = "1d", limit: int = 100) -> List[Dict[str, Any]]:
        ...
        for i in range(limit):
            ...
            klines.append({
                "open_time": int((datetime.utcnow() - timedelta(days=limit-i)).timestamp() * 1000),
                "open": str(price),
                "high": str(price * Decimal("1.02")),
                "low": str(price * Decimal("0.98")),
                "close": str(price),
                "volume": "100.00000000",
                "close_time": int((datetime.utcnow() - timedelta(days=limit-i-1)).timestamp() * 1000),
    ...
    def _should_execute_order(self, order: Order) -> bool:
        ...
        if order.side == OrderSide.BUY:
            ...
            base_asset = order.symbol.split("USDT")[0] if "USDT" in order.symbol else order.symbol.split("USD")[0]
    ...
    async def _execute_order(self, order: Order):
        ...
        if order.side == OrderSide.BUY:
            ...
            self.balances[quote_asset] = self.balances.get(quote_asset, Decimal("0")) - (order.filled_quantity * order.filled_price)
            self.balances[base_asset] = self.balances.get(base_asset, Decimal("0")) + order.filled_quantity
        else:
...
def create_mock_binance_client(api_key: str = "mock_api_key", api_secret: str = "mock_api_secret") -> MockBinanceClient:
