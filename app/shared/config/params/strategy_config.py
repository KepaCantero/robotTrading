...
class StockAllocationSettings(BaseSettings):
    ...
    def from_yaml(cls, tier: Optional[str] = None) -> StockAllocationSettings:
        ...
        # Type alias for YAML-loaded config sections (nested dicts with primitive values)
        _section = dict[str, Union[int, float, bool, str]]
        _weights_section = dict[str, float]

        raw_config = load_strategy_stock_allocator_config(tier)
        ...
        if "data_validation" in config:
            dv = cast("_section", config["data_validation"])
            kwargs["LOOKBACK_MAX_DAYS"] = int(dv.get("lookback_max_days", 126))
            kwargs["MIN_LIQUIDITY_USD"] = float(dv.get("min_liquidity_usd", 500_000))

        # Statistical tests
        if "statistical_tests" in config:
            st = cast("_section", config["statistical_tests"])
            if "adf" in st:
                adf_section = cast("_section", st["adf"])
                kwargs["ADF_P_VALUE_THRESHOLD"] = float(adf_section.get("p_value_threshold", 0.01))
                kwargs["ADF_P_VALUE_THRESHOLD_MEAN_REVERSION"] = float(
                    adf_section.get("p_value_mean_reversion", 0.05)
                )
            if "kpss" in st:
                kpss_section = cast("_section", st["kpss"])
                kwargs["KPSS_P_VALUE_THRESHOLD"] = float(
                    kpss_section.get("p_value_threshold", 0.05)
                )
            if "hurst" in st:
                hurst_section = cast("_section", st["hurst"])
                kwargs["HURST_MOMENTUM_THRESHOLD"] = float(
                    hurst_section.get("momentum_threshold", 0.52)
                )
                kwargs["HURST_MEAN_REVERSION_THRESHOLD"] = float(
                    hurst_section.get("mean_reversion_threshold", 0.45)
                )

        # Mean reversion
        if "mean_reversion" in config:
            mr = cast("_section", config["mean_reversion"])
            kwargs["MAX_HALF_LIFE_DAYS"] = int(mr.get("max_half_life_days", 120))
            kwargs["MIN_HALF_LIFE_DAYS"] = int(mr.get("min_half_life_days", 1))

        # Exposure
        if "exposure" in config:
            exp = cast("_section", config["exposure"])
            kwargs["MAX_STRATEGY_EXPOSURE"] = float(exp.get("max_strategy_exposure", 0.50))
            kwargs["MAX_PAIR_EXPOSURE"] = float(exp.get("max_pair_exposure", 0.15))
            kwargs["MAX_ASSETS_PER_PAIR"] = int(exp.get("max_assets_per_pair", 2))

        # Scoring weights
        if "scoring_weights" in config:
            sw = cast("_section", config["scoring_weights"])
            if "momentum" in sw:
                # Convert "tau_inverse" to "1/tau" for compatibility
                momentum = cast("_weights_section", sw["momentum"]).copy()
                if "tau_inverse" in momentum:
                    momentum["1/tau"] = momentum.pop("tau_inverse")
                kwargs["MOMENTUM_WEIGHTS"] = momentum
            if "mean_reversion" in sw:
                mr_sw = cast("_weights_section", sw["mean_reversion"]).copy()
                if "tau_inverse" in mr_sw:
                    mr_sw["1/tau"] = mr_sw.pop("tau_inverse")
                kwargs["MEAN_REVERSION_WEIGHTS"] = mr_sw
            if "pairs_trading" in sw:
                pt = cast("_weights_section", sw["pairs_trading"]).copy()
        ...
        if "risk_metrics" in config:
            risk_metrics = cast("_section", config["risk_metrics"])
            kwargs["MIN_SORTINO_RATIO"] = float(risk_metrics.get("min_sortino_ratio", 0.5))

        # Optimization
        if "optimization" in config:
            opt = cast("_section", config["optimization"])
            kwargs["ERC_OPTIMIZATION_TOLERANCE"] = float(opt.get("tolerance", 1e-6))
            kwargs["ERC_MAX_ITERATIONS"] = int(opt.get("max_iterations", 1000))

        # GARCH
        if "garch" in config:
            garch = cast("_section", config["garch"])
            kwargs["GARCH_FORECAST_HORIZON"] = int(garch.get("forecast_horizon", 1))
            kwargs["DYNAMIC_WINDOW_ENABLED"] = bool(garch.get("dynamic_window_enabled", True))
            kwargs["SLOPE_WINDOW_MIN"] = int(garch.get("slope_window_min", 30))
            kwargs["SLOPE_WINDOW_MAX"] = int(garch.get("slope_window_max", 90))

        # Additional
        if "additional" in config:
            additional = cast("_section", config["additional"])
            kwargs["MIN_COINTEGRATION_LOOKBACK_DAYS"] = int(
                additional.get("min_cointegration_lookback_days", 250)
            )

        # Logging
        if "logging" in config:
            log = cast("_section", config["logging"])
            kwargs["LOG_ALL_DECISIONS"] = bool(log.get("log_all_decisions", True))
            kwargs["LOG_FILTER_REJECTIONS"] = bool(log.get("log_filter_rejections", True))
