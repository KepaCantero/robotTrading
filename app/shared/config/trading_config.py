...
Usage:
    from app.shared.config.trading_config import get_config, CentralizedConfig
...
from app.shared.config.signal_risk import (
...
from app.shared.config.technical_indicators import (
...
class TradingThresholds(BaseModel):
    ...
    capital_loss_threshold: float = Field(
        default=0.40, description="Capital loss threshold for deployment decisions (0-1)"
    )
    max_cost_impact_ratio: float = Field(
        default=0.30, description="Maximum cost impact ratio (Chan's 30% threshold) (0-1)"
    )
    ...
    momentum_fixed_max_position: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Max position size for fixed position sizing (50%)"
    )
    ...
    circuit_breaker_daily_loss: float = Field(
        default=0.05, description="Circuit breaker daily loss threshold"
    )
    ...
    validator_min_position_pct: float = Field(
        default=0.01, description="Minimum position as % of capital"
    )
    validator_max_position_pct: float = Field(
        default=0.50, description="Maximum position as % of capital"
    )
    validator_stop_loss_warning_pct: float = Field(
        default=0.10, description="Warning threshold for stop-loss distance"
    )
    ...
    percentage_multiplier: int = Field(
        default=100, description="Multiplier for percentage conversion"
    )
    milliseconds_multiplier: int = Field(
        default=1000, description="Multiplier for milliseconds conversion"
    )
    ...
    spread_threshold_half_divisor: float = Field(
        default=2.0, description="Divisor for half spread threshold"
    )
    cointegration_relaxed_multiplier: float = Field(
        default=0.8, description="Multiplier for relaxed cointegration threshold"
    )
    ...
    medium_confidence_threshold: float = Field(
        default=0.7, description="Medium confidence threshold"
    )
    ...
    fallback_feasibility_ratio: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Feasibility ratio for fallback backtest strategy"
    )
    ...
    breadth_high_threshold: Decimal = Field(
        default=Decimal("1000"), description="High breadth threshold for Fundamental Law analysis"
    )
    breadth_medium_threshold: Decimal = Field(
        default=Decimal("100"), description="Medium breadth threshold for Fundamental Law analysis"
    )

    # Reconciliation tolerance thresholds (R16)
    reconciliation_quantity_tolerance: float = Field(
        default=1.0, ge=0.0, le=100.0, description="Quantity tolerance for reconciliation (shares)"
    )
    reconciliation_price_tolerance_pct: float = Field(
        default=0.001, ge=0.0, le=0.1, description="Price tolerance for reconciliation (0.1%)"
    )
    reconciliation_value_tolerance_pct: float = Field(
        default=0.005, ge=0.0, le=0.5, description="Value tolerance for reconciliation (0.5%)"
    )
    ...
    pairs_trading_history_length: int = Field(
        default=250, ge=10, le=1000, description="Price history length for pairs trading (days)"
    )
    ...
    chan_fixed_stop_pct: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Fixed percentage stop loss for Chan methods (5%)"
    )
    ...
    # LOW risk tolerance
    risk_low_max_drawdown: float = Field(
        default=0.15, ge=0.0, le=0.5, description="Max drawdown for LOW risk tolerance (15%)"
    )
    risk_low_max_volatility: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Max volatility for LOW risk tolerance (20%)"
    )
    risk_low_max_position: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Max position size for LOW risk tolerance (5%)"
    )
    risk_low_atr_multiplier: float = Field(
        default=2.0, ge=1.0, le=5.0, description="ATR multiplier for LOW risk tolerance stop loss"
    )
    risk_low_take_profit_multiplier: float = Field(
        default=3.0, ge=1.0, le=10.0, description="Take profit multiplier for LOW risk tolerance"
    )
    risk_low_var_confidence: float = Field(
        default=0.95, ge=0.5, le=0.99, description="VaR confidence for LOW risk tolerance"
    )
    risk_low_es_confidence: float = Field(
        default=0.95,
        ge=0.5,
        le=0.99,
        description="Expected shortfall confidence for LOW risk tolerance",
    )

    # MEDIUM risk tolerance
    risk_medium_max_drawdown: float = Field(
        default=0.25, ge=0.0, le=0.5, description="Max drawdown for MEDIUM risk tolerance (25%)"
    )
    risk_medium_max_volatility: float = Field(
        default=0.30, ge=0.0, le=1.0, description="Max volatility for MEDIUM risk tolerance (30%)"
    )
    risk_medium_max_position: float = Field(
        default=0.10,
        ge=0.0,
        le=0.5,
        description="Max position size for MEDIUM risk tolerance (10%)",
    )
    risk_medium_max_leverage: float = Field(
        default=1.5, ge=1.0, le=3.0, description="Max leverage for MEDIUM risk tolerance (1.5x)"
    )
    risk_medium_atr_multiplier: float = Field(
        default=2.5,
        ge=1.0,
        le=5.0,
        description="ATR multiplier for MEDIUM risk tolerance stop loss",
    )
    risk_medium_take_profit_multiplier: float = Field(
        default=4.0, ge=1.0, le=10.0, description="Take profit multiplier for MEDIUM risk tolerance"
    )
    risk_medium_var_confidence: float = Field(
        default=0.95, ge=0.5, le=0.99, description="VaR confidence for MEDIUM risk tolerance"
    )
    risk_medium_es_confidence: float = Field(
        default=0.95,
        ge=0.5,
        le=0.99,
        description="Expected shortfall confidence for MEDIUM risk tolerance",
    )

    # HIGH risk tolerance
    risk_high_max_drawdown: float = Field(
        default=0.40, ge=0.0, le=0.8, description="Max drawdown for HIGH risk tolerance (40%)"
    )
    risk_high_max_volatility: float = Field(
        default=0.50, ge=0.0, le=1.0, description="Max volatility for HIGH risk tolerance (50%)"
    )
    risk_high_max_position: float = Field(
        default=0.20, ge=0.0, le=0.5, description="Max position size for HIGH risk tolerance (20%)"
    )
    risk_high_max_leverage: float = Field(
        default=2.0, ge=1.0, le=3.0, description="Max leverage for HIGH risk tolerance (2x)"
    )
    risk_high_atr_multiplier: float = Field(
        default=3.0, ge=1.0, le=5.0, description="ATR multiplier for HIGH risk tolerance stop loss"
    )
    risk_high_take_profit_multiplier: float = Field(
        default=6.0, ge=1.0, le=10.0, description="Take profit multiplier for HIGH risk tolerance"
    )
    risk_high_var_confidence: float = Field(
        default=0.99, ge=0.5, le=0.999, description="VaR confidence for HIGH risk tolerance"
    )
    risk_high_es_confidence: float = Field(
        default=0.975,
        ge=0.5,
        le=0.999,
        description="Expected shortfall confidence for HIGH risk tolerance",
    )

    # Optimization configuration (for InputProfileRouter)
    opt_lookback_short: int = Field(
        default=63, ge=10, le=500, description="Short lookback period (3 months)"
    )
    opt_lookback_medium: int = Field(
        default=126, ge=10, le=500, description="Medium lookback period (6 months)"
    )
    opt_lookback_long: int = Field(
        default=252, ge=10, le=500, description="Long lookback period (1 year)"
    )
    opt_min_weight_low: float = Field(
        default=0.01, ge=0.0, le=0.1, description="Min weight for LOW risk tolerance optimization"
    )
    opt_max_weight_low: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Max weight for LOW risk tolerance optimization"
    )
    opt_min_weight_medium: float = Field(
        default=0.02,
        ge=0.0,
        le=0.1,
        description="Min weight for MEDIUM risk tolerance optimization",
    )
    opt_max_weight_medium: float = Field(
        default=0.10,
        ge=0.0,
        le=0.5,
        description="Max weight for MEDIUM risk tolerance optimization",
    )
    opt_min_weight_high: float = Field(
        default=0.02, ge=0.0, le=0.1, description="Min weight for HIGH risk tolerance optimization"
    )
    opt_max_weight_high: float = Field(
        default=0.20, ge=0.0, le=0.5, description="Max weight for HIGH risk tolerance optimization"
    )
    opt_max_turnover: float = Field(
        default=0.50, ge=0.0, le=1.0, description="Max portfolio turnover (50%)"
    )

    # Constraint configuration (for InputProfileRouter)
    constraint_capital_small: float = Field(
        default=10000.0, ge=0.0, description="Small portfolio capital threshold"
    )
    constraint_capital_medium: float = Field(
        default=100000.0, ge=0.0, description="Medium portfolio capital threshold"
    )
    constraint_max_pos_small: int = Field(
        default=10, ge=1, le=50, description="Max positions for small portfolios"
    )
    constraint_max_pos_medium: int = Field(
        default=25, ge=1, le=100, description="Max positions for medium portfolios"
    )
    constraint_max_pos_large: int = Field(
        default=50, ge=1, le=200, description="Max positions for large portfolios"
    )
    constraint_min_liquidity_low: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Min liquidity score for small portfolios"
    )
    constraint_min_liquidity_high: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Min liquidity score for large portfolios"
    )
    constraint_sector_exposure_low: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Max sector exposure for LOW risk (25%)"
    )
    constraint_sector_exposure_high: float = Field(
        default=0.40, ge=0.0, le=1.0, description="Max sector exposure for MEDIUM/HIGH risk (40%)"
    )
    constraint_capital_liquidity: float = Field(
        default=50000.0, ge=0.0, description="Capital threshold for high liquidity requirement"
    )

    # Strategy-specific configuration (for InputProfileRouter)
    strat_mom_lookback: int = Field(
        default=252, ge=10, le=500, description="Momentum strategy lookback period (days)"
    )
    strat_mom_top_pct: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Momentum strategy top percentile (30%)"
    )
    strat_mom_bottom_pct: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Momentum strategy bottom percentile (30%)"
    )
    strat_div_min_yield: float = Field(
        default=0.02, ge=0.0, le=0.5, description="Dividend strategy minimum yield (2%)"
    )
    strat_div_max_yield: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Dividend strategy maximum yield (10%)"
    )
    strat_div_min_growth: float = Field(
        default=0.0, ge=-1.0, le=1.0, description="Dividend strategy minimum dividend growth rate"
    )
    strat_div_max_payout: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Dividend strategy maximum payout ratio (80%)"
    )
    strat_div_min_years: int = Field(
        default=5, ge=1, le=20, description="Dividend strategy minimum years of dividends"
    )
    strat_lv_max_beta: float = Field(
        default=0.8, ge=0.0, le=2.0, description="Low volatility strategy maximum beta"
    )
    strat_lv_max_vol: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Low volatility strategy maximum volatility (25%)"
    )
    strat_mf_weight_value: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Multi-factor value weight"
    )
    strat_mf_weight_size: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Multi-factor size weight"
    )
    strat_mf_weight_momentum: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Multi-factor momentum weight"
    )
    strat_mf_weight_quality: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Multi-factor quality weight"
    )
    ...
    covered_call_min_position_size: float = Field(
        default=0.01, ge=0.0, le=0.10, description="Minimum position size for validation (1%)"
    )
    ...
    market_regime_default_confidence: float = Field(
        default=0.75, ge=0.0, le=1.0, description="Default confidence for market regime detection"
    )
    market_regime_default_atr_ratio: float = Field(
        default=0.02, ge=0.0, le=1.0, description="Default ATR ratio for market regime detection"
    )
    ...
    portfolio_rebalance_threshold: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Portfolio rebalance threshold (5%)"
    )
    portfolio_tolerance_pct: float = Field(
        default=0.01, ge=0.0, le=0.1, description="Portfolio tolerance for reconciliation (1%)"
    )
    portfolio_max_deviation_warning: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Max deviation before warning (5%)"
    )
    portfolio_max_deviation_moderate: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Max deviation before moderate risk alert (10%)"
    )
    portfolio_max_deviation_high: float = Field(
        default=0.15, ge=0.0, le=1.0, description="Max deviation before high risk alert (15%)"
    )
    ...
    portfolio_risk_free_rate: float = Field(
        default=0.02, ge=0.0, le=1.0, description="Risk-free rate for portfolio calculations (2%)"
    )
    ...
    analytics_benchmark_return: float = Field(
        default=0.08, ge=0.0, le=1.0, description="Benchmark return for portfolio analytics (8%)"
    )
    analytics_var_95_confidence: float = Field(
        default=0.95, ge=0.5, le=0.99, description="VaR 95% confidence level"
    )
    analytics_var_99_confidence: float = Field(
        default=0.99, ge=0.5, le=0.99, description="VaR 99% confidence level"
    )
    analytics_rebalance_threshold: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Rebalancing threshold for portfolio (5%)"
    )
    ...
    analytics_sharpe_ratio_threshold: float = Field(
        default=0.5, ge=0.0, le=5.0, description="Sharpe ratio threshold for recommendations (0.5)"
    )
    ...
    analytics_volatility_high_threshold: float = Field(
        default=0.20, ge=0.0, le=1.0, description="High volatility threshold (20%)"
    )
    analytics_volatility_extreme_threshold: float = Field(
        default=0.30, ge=0.0, le=1.0, description="Extreme volatility threshold (30%)"
    )
    analytics_largest_position_high_threshold: float = Field(
        default=0.20, ge=0.0, le=1.0, description="High largest position threshold (20%)"
    )
    analytics_largest_position_extreme_threshold: float = Field(
        default=0.30, ge=0.0, le=1.0, description="Extreme largest position threshold (30%)"
    )
    analytics_max_drawdown_high_threshold: float = Field(
        default=0.20, ge=0.0, le=1.0, description="High max drawdown threshold (20%)"
    )
    analytics_var_95_high_threshold: float = Field(
        default=0.10, ge=0.0, le=1.0, description="High VaR 95% threshold (10%)"
    )
    analytics_cash_ratio_high_threshold: float = Field(
        default=0.5, ge=0.0, le=1.0, description="High cash ratio threshold (50%)"
    )
    analytics_risk_score_low_threshold: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Low risk score threshold (25%)"
    )
    analytics_risk_score_moderate_threshold: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Moderate risk score threshold (50%)"
    )
    analytics_risk_score_high_threshold: float = Field(
        default=0.75, ge=0.0, le=1.0, description="High risk score threshold (75%)"
    )
    analytics_default_average_correlation: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Default average correlation for mock data (30%)"
    )
    ...
    analytics_volatility_max_score: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Max volatility for risk score calculation (20%)"
    )
    ...
    analytics_max_performance_return: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Max annualized return for health score (20%)"
    )
    ...
    analytics_min_position_count_diversified: int = Field(
        default=5, ge=1, le=50, description="Minimum position count for good diversification (5)"
    )
    analytics_tail_ratio_size: float = Field(
        default=0.1, ge=0.01, le=0.5, description="Tail size for tail ratio calculation (10%)"
    )

    # Transaction costs by asset class (per trade)
    tx_cost_equity: float = Field(
        default=0.0005, ge=0.0, le=0.1, description="Transaction cost for equity (0.05%)"
    )
    tx_cost_crypto: float = Field(
        default=0.001, ge=0.0, le=0.1, description="Transaction cost for crypto (0.1%)"
    )
    tx_cost_forex: float = Field(
        default=0.0001, ge=0.0, le=0.1, description="Transaction cost for forex (0.01%)"
    )
    tx_cost_fixed_income: float = Field(
        default=0.001, ge=0.0, le=0.1, description="Transaction cost for fixed income (0.1%)"
    )
    tx_cost_commodity: float = Field(
        default=0.0005, ge=0.0, le=0.1, description="Transaction cost for commodities (0.05%)"
    )
    tx_cost_real_estate: float = Field(
        default=0.001, ge=0.0, le=0.1, description="Transaction cost for real estate (0.1%)"
    )

    # Fundamental Law of Active Management thresholds
    fundamental_law_ic_excellent: float = Field(
        default=0.05, ge=0.0, le=1.0, description="IC threshold for excellent skill level (5%)"
    )
    fundamental_law_ic_good: float = Field(
        default=0.03, ge=0.0, le=1.0, description="IC threshold for good skill level (3%)"
    )
    fundamental_law_ic_fair: float = Field(
        default=0.01, ge=0.0, le=1.0, description="IC threshold for fair skill level (1%)"
    )
    fundamental_law_ir_excellent: float = Field(
        default=1.0, ge=0.0, le=10.0, description="IR threshold for excellent (1.0)"
    )
    fundamental_law_ir_good: float = Field(
        default=0.5, ge=0.0, le=10.0, description="IR threshold for good (0.5)"
    )
    fundamental_law_ir_fair: float = Field(
        default=0.25, ge=0.0, le=10.0, description="IR threshold for fair (0.25)"
    )
    fundamental_law_tc_excellent: float = Field(
        default=0.8, ge=0.0, le=1.0, description="TC threshold for excellent (80%)"
    )
    fundamental_law_tc_good: float = Field(
        default=0.6, ge=0.0, le=1.0, description="TC threshold for good (60%)"
    )
    fundamental_law_tc_fair: float = Field(
        default=0.4, ge=0.0, le=1.0, description="TC threshold for fair (40%)"
    )
    fundamental_law_trading_days_per_year: int = Field(
        default=252, ge=1, le=365, description="Number of trading days per year for IR calculation"
    )
    ...
    signal_spread_poor: float = Field(
        default=0.05, ge=0.0, le=1.0, description="Spread threshold for poor signal quality (5%)"
    )
    ...
    signal_ema_trend_strong: float = Field(
        default=0.02, ge=0.0, le=1.0, description="Strong EMA trend threshold (2%)"
    )
    ...
    signal_spread_acceptable: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Acceptable spread threshold for liquidity (10%)"
    )
    ...
    signal_volatility_default: float = Field(
        default=0.02, ge=0.0, le=1.0, description="Default volatility for signal scoring (2%)"
    )
    ...
    signal_volatility_acceptable_min: float = Field(
        default=0.005, ge=0.0, le=1.0, description="Acceptable minimum volatility (0.5%)"
    )
    signal_volatility_acceptable_max: float = Field(
        default=0.05, ge=0.0, le=1.0, description="Acceptable maximum volatility (5%)"
    )
    signal_volatility_high_threshold: float = Field(
        default=0.05, ge=0.0, le=1.0, description="High volatility threshold (5%)"
    )
    signal_volatility_low_threshold: float = Field(
        default=0.01, ge=0.0, le=1.0, description="Low volatility threshold (1%)"
    )
    ...
    signal_volatility_atr_high: float = Field(
        default=0.03, ge=0.0, le=1.0, description="High ATR ratio threshold (3%)"
    )
    signal_volatility_atr_low: float = Field(
        default=0.01, ge=0.0, le=1.0, description="Low ATR ratio threshold (1%)"
    )
    ...
    signal_spread_low: float = Field(
        default=0.2, ge=0.0, le=1.0, description="Low spread threshold for good liquidity (0.2%)"
    )
    signal_spread_moderate: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Moderate spread threshold (0.5%)"
    )
    signal_spread_high: float = Field(
        default=1.0, ge=0.0, le=1.0, description="High spread threshold (1%)"
    )
    signal_spread_stable_low: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Low spread for price stability (0.1%)"
    )
    signal_spread_stable_moderate: float = Field(
        default=0.3, ge=0.0, le=1.0, description="Moderate spread for price stability (0.3%)"
    )
    signal_spread_stable_high: float = Field(
        default=0.5, ge=0.0, le=1.0, description="High spread for price stability (0.5%)"
    )

    # Signal volume trend thresholds
    signal_volume_trend_high: float = Field(
        default=0.1, ge=0.0, le=1.0, description="High volume trend threshold (10%)"
    )
    signal_volume_trend_low: float = Field(
        default=-0.1, le=0.0, ge=-1.0, description="Low volume trend threshold (-10%)"
    )
    ...
    currency_default_correlation: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Default correlation for currency pairs"
    )
    currency_hedge_rolling_base: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Base hedge ratio for rolling hedge strategy"
    )
    ...
    currency_correlation_adjustment: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Correlation adjustment factor for hedge ratio"
    )
    currency_execution_cost_factor: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Execution cost factor as fraction of spread"
    )
    currency_slippage_base_bps: float = Field(
        default=0.5, ge=0.0, le=10.0, description="Base slippage in basis points"
    )
    currency_size_large_threshold: float = Field(
        default=1000000.0, ge=0.0, description="Large contract size threshold for better rates"
    )
    currency_size_large_factor: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Size reduction factor for large contracts"
    )
    ...
    slippage_vol_adjustment_extreme: float = Field(
        default=2.0, ge=0.0, le=10.0, description="Volatility adjustment factor for extreme events"
    )
    slippage_vol_adjustment_high: float = Field(
        default=1.0, ge=0.0, le=5.0, description="Volatility adjustment factor for high volatility"
    )
    ...
    var_confidence_level: float = Field(
        default=0.95, ge=0.5, le=0.99, description="Confidence level for VaR calculation (95%)"
    )
    var_lookback_days: int = Field(
        default=60, ge=10, le=250, description="Lookback period in days for VaR calculation"
    )
    ...
    var_enforcer_halt_limit: float = Field(
        default=0.05, ge=0.0, le=1.0, description="VaR halt threshold for risk limits enforcer (5%)"
    )
    risk_enforcer_max_position_pct: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Max position size for risk limits enforcer (20%)"
    )
    risk_enforcer_max_concentration_pct: float = Field(
        default=0.40, ge=0.0, le=1.0, description="Max concentration for risk limits enforcer (40%)"
    )
    risk_enforcer_max_leverage: float = Field(
        default=2.0, ge=1.0, le=5.0, description="Max leverage for risk limits enforcer (2x)"
    )
    risk_heatmap_high_threshold: float = Field(
        default=0.15, ge=0.0, le=1.0, description="High risk threshold for risk heatmap (15%)"
    )
    risk_heatmap_medium_threshold: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Medium risk threshold for risk heatmap (10%)"
    )
    risk_concentration_high_threshold: float = Field(
        default=60.0, ge=0.0, le=100.0, description="High concentration threshold (% of top 3 risk)"
    )
    ...
    slo_latency_threshold_ms: float = Field(
        default=100.0, ge=0.0, description="SLO latency threshold in milliseconds"
    )
    ...
    limit_micro_stop_loss_pct: float = Field(
        default=0.02, ge=0.0, le=0.5, description="Stop loss percentage for micro tier (2%)"
    )
    limit_micro_daily_loss_limit_pct: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Daily loss limit percentage for micro tier (5%)"
    )
    limit_micro_max_position_pct: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Max position percentage for micro tier (10%)"
    )
    limit_micro_leverage: float = Field(
        default=1.0, ge=1.0, le=3.0, description="Leverage for micro tier (1x)"
    )
    limit_micro_margin_requirement: float = Field(
        default=0.50, ge=0.0, le=1.0, description="Margin requirement for micro tier (50%)"
    )
    limit_micro_max_drawdown_pct: float = Field(
        default=0.10, ge=0.0, le=0.5, description="Max drawdown percentage for micro tier (10%)"
    )

    limit_small_stop_loss_pct: float = Field(
        default=0.025, ge=0.0, le=0.5, description="Stop loss percentage for small tier (2.5%)"
    )
    limit_small_daily_loss_limit_pct: float = Field(
        default=0.08, ge=0.0, le=0.5, description="Daily loss limit percentage for small tier (8%)"
    )
    limit_small_max_position_pct: float = Field(
        default=0.15, ge=0.0, le=1.0, description="Max position percentage for small tier (15%)"
    )
    limit_small_leverage: float = Field(
        default=1.0, ge=1.0, le=3.0, description="Leverage for small tier (1x)"
    )
    limit_small_margin_requirement: float = Field(
        default=0.33, ge=0.0, le=1.0, description="Margin requirement for small tier (33%)"
    )
    limit_small_max_drawdown_pct: float = Field(
        default=0.15, ge=0.0, le=0.5, description="Max drawdown percentage for small tier (15%)"
    )

    limit_medium_stop_loss_pct: float = Field(
        default=0.03, ge=0.0, le=0.5, description="Stop loss percentage for medium tier (3%)"
    )
    ...
    limit_medium_max_position_pct: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Max position percentage for medium tier (20%)"
    )
    limit_medium_leverage: float = Field(
        default=1.5, ge=1.0, le=3.0, description="Leverage for medium tier (1.5x)"
    )
    limit_medium_margin_requirement: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Margin requirement for medium tier (25%)"
    )
    limit_medium_max_drawdown_pct: float = Field(
        default=0.20, ge=0.0, le=0.5, description="Max drawdown percentage for medium tier (20%)"
    )

    limit_large_stop_loss_pct: float = Field(
        default=0.035, ge=0.0, le=0.5, description="Stop loss percentage for large tier (3.5%)"
    )
    limit_large_daily_loss_limit_pct: float = Field(
        default=0.15, ge=0.0, le=0.5, description="Daily loss limit percentage for large tier (15%)"
    )
    limit_large_max_position_pct: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Max position percentage for large tier (25%)"
    )
    limit_large_leverage: float = Field(
        default=2.0, ge=1.0, le=3.0, description="Leverage for large tier (2x)"
    )
    limit_large_margin_requirement: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Margin requirement for large tier (20%)"
    )
    limit_large_max_drawdown_pct: float = Field(
        default=0.25, ge=0.0, le=0.5, description="Max drawdown percentage for large tier (25%)"
    )
    ...
    limit_vol_multiplier_low: float = Field(
        default=1.1, ge=0.5, le=2.0, description="Volatility multiplier for low volatility"
    )
    ...
    limit_vol_ratio_very_low_threshold: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Volatility ratio threshold for very low state"
    )
    limit_vol_ratio_low_threshold: float = Field(
        default=0.9, ge=0.0, le=1.0, description="Volatility ratio threshold for low state"
    )
    limit_vol_ratio_normal_upper: float = Field(
        default=1.1, ge=1.0, le=2.0, description="Upper threshold for normal volatility ratio"
    )
    limit_vol_ratio_high_upper: float = Field(
        default=1.5, ge=1.0, le=3.0, description="Upper threshold for high volatility ratio"
    )

    # Drawdown thresholds for limit adjustment
    limit_dd_healthy_threshold: float = Field(
        default=0.05, ge=0.0, le=0.5, description="Drawdown threshold for healthy state (5%)"
    )
    limit_dd_caution_threshold: float = Field(
        default=0.10, ge=0.0, le=0.5, description="Drawdown threshold for caution state (10%)"
    )
    limit_dd_warning_threshold: float = Field(
        default=0.15, ge=0.0, le=0.5, description="Drawdown threshold for warning state (15%)"
    )
    limit_dd_critical_threshold: float = Field(
        default=0.20, ge=0.0, le=0.5, description="Drawdown threshold for critical state (20%)"
    )
    ...
    asset_class_equity_vol_min: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Equity minimum typical volatility (10%)"
    )
    asset_class_equity_vol_max: float = Field(
        default=0.30, ge=0.0, le=2.0, description="Equity maximum typical volatility (30%)"
    )
    asset_class_crypto_vol_min: float = Field(
        default=0.40, ge=0.0, le=2.0, description="Crypto minimum typical volatility (40%)"
    )
    asset_class_crypto_vol_max: float = Field(
        default=1.20, ge=0.0, le=3.0, description="Crypto maximum typical volatility (120%)"
    )
    asset_class_forex_vol_min: float = Field(
        default=0.05, ge=0.0, le=1.0, description="Forex minimum typical volatility (5%)"
    )
    asset_class_forex_vol_max: float = Field(
        default=0.15, ge=0.0, le=1.0, description="Forex maximum typical volatility (15%)"
    )
    asset_class_fixed_income_vol_min: float = Field(
        default=0.02, ge=0.0, le=0.5, description="Fixed income minimum typical volatility (2%)"
    )
    asset_class_fixed_income_vol_max: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Fixed income maximum typical volatility (10%)"
    )
    asset_class_commodity_vol_min: float = Field(
        default=0.15, ge=0.0, le=1.0, description="Commodity minimum typical volatility (15%)"
    )
    asset_class_commodity_vol_max: float = Field(
        default=0.40, ge=0.0, le=2.0, description="Commodity maximum typical volatility (40%)"
    )
    asset_class_real_estate_vol_min: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Real estate minimum typical volatility (10%)"
    )
    asset_class_real_estate_vol_max: float = Field(
        default=0.25, ge=0.0, le=1.0, description="Real estate maximum typical volatility (25%)"
    )
    asset_class_cash_vol_min: float = Field(
        default=0.00, ge=0.0, le=0.1, description="Cash minimum typical volatility (0%)"
    )
    asset_class_cash_vol_max: float = Field(
        default=0.02, ge=0.0, le=0.5, description="Cash maximum typical volatility (2%)"
    )
    asset_class_default_vol_min: float = Field(
        default=0.05, ge=0.0, le=1.0, description="Default minimum typical volatility (5%)"
    )
    asset_class_default_vol_max: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Default maximum typical volatility (20%)"
    )

    # Asset class typical expected return ranges (min, max as decimals)
    asset_class_equity_return_min: float = Field(
        default=0.05, ge=-0.5, le=1.0, description="Equity minimum typical expected return (5%)"
    )
    asset_class_equity_return_max: float = Field(
        default=0.12, ge=0.0, le=2.0, description="Equity maximum typical expected return (12%)"
    )
    asset_class_crypto_return_min: float = Field(
        default=-0.20, ge=-1.0, le=1.0, description="Crypto minimum typical expected return (-20%)"
    )
    asset_class_crypto_return_max: float = Field(
        default=0.50, ge=0.0, le=2.0, description="Crypto maximum typical expected return (50%)"
    )
    asset_class_forex_return_min: float = Field(
        default=-0.05, ge=-0.5, le=1.0, description="Forex minimum typical expected return (-5%)"
    )
    asset_class_forex_return_max: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Forex maximum typical expected return (10%)"
    )
    ...
    asset_class_commodity_return_max: float = Field(
        default=0.20, ge=0.0, le=1.0, description="Commodity maximum typical expected return (20%)"
    )
    asset_class_real_estate_return_min: float = Field(
        default=0.03, ge=0.0, le=1.0, description="Real estate minimum typical expected return (3%)"
    )
    ...
    asset_class_cash_return_min: float = Field(
        default=0.00, ge=0.0, le=0.5, description="Cash minimum typical expected return (0%)"
    )
    asset_class_cash_return_max: float = Field(
        default=0.03, ge=0.0, le=0.5, description="Cash maximum typical expected return (3%)"
    )
    asset_class_default_return_min: float = Field(
        default=0.00, ge=-0.5, le=1.0, description="Default minimum typical expected return (0%)"
    )
    asset_class_default_return_max: float = Field(
        default=0.10, ge=0.0, le=1.0, description="Default maximum typical expected return (10%)"
    )
    ...
    cost_max_commission_rate: float = Field(
        default=0.1, ge=0.0, le=1.0, description="Maximum allowed commission rate (10%)"
    )
    ...
    max_single_position_limit: float = Field(
        default=0.50, ge=0.01, le=1.0, description="Maximum allowed single position size (50%)"
    )
    ...
    fx_corr_very_strong: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Very strong correlation threshold (80%)"
    )
    fx_corr_strong: float = Field(
        default=0.6, ge=0.0, le=1.0, description="Strong correlation threshold (60%)"
    )
    fx_corr_moderate: float = Field(
        default=0.4, ge=0.0, le=1.0, description="Moderate correlation threshold (40%)"
    )
    fx_signal_strong_threshold: float = Field(
        default=80.0, ge=0.0, le=100.0, description="Strong signal strength threshold (0-100)"
    )
    fx_signal_min_confidence_strong: float = Field(
        default=70.0, ge=0.0, le=100.0, description="Minimum confidence for strong signals (0-100)"
    )
    fx_signal_max_expected_move: float = Field(
        default=0.10, ge=0.01, le=1.0, description="Maximum expected daily move (10%)"
    )
    ...
    portfolio_value_tolerance: float = Field(
        default=0.01, ge=0.001, le=0.1, description="Portfolio value validation tolerance (1%)"
    )
...
class CentralizedConfig(SettingsBase):
    ...
    trading_costs: TradingCostThresholds = Field(
        default_factory=TradingCostThresholds,
        description="Trading cost thresholds for profitability validation",
    )
    market_microstructure: MarketMicrostructureThresholds = Field(
        default_factory=MarketMicrostructureThresholds,
        description="Market microstructure and liquidity analysis thresholds",
    )
