...
class WindowSizes(ConfigBase):
    ...
    momentum_short_window: int = Field(
        default=10, ge=3, le=50, description="Short window for momentum"
    )
    momentum_long_window: int = Field(
        default=30, ge=10, le=200, description="Long window for momentum"
    )
    ...
    volatility_window: int = Field(
        default=20, ge=5, le=100, description="Volatility calculation window"
    )
    ...
    min_history_length: int = Field(
        default=60, ge=30, le=200, description="Minimum history length for signals"
    )
    ...
    default_sequence_length: int = Field(
        default=60, ge=20, le=200, description="Default sequence length"
    )
    transformer_sequence_length: int = Field(
        default=30, ge=10, le=100, description="Transformer sequence length"
    )
...
class ConversionMultipliers(ConfigBase):
    ...
    milliseconds_multiplier: int = Field(
        default=1000, ge=1, le=10000, description="Multiplier for ms conversion"
    )
...
class PerformanceMetrics(ConfigBase):
    ...
    min_sharpe_ratio: float = Field(
        default=1.0, ge=0.0, le=10.0, description="Minimum Sharpe ratio"
    )
...
class FundamentalAnalysisThresholds(ConfigBase):
    ...
    pe_ideal_min: float = Field(
        default=10.0, ge=0.0, le=50.0, description="Ideal minimum P/E ratio"
    )
    pe_ideal_max: float = Field(
        default=20.0, ge=0.0, le=100.0, description="Ideal maximum P/E ratio"
    )
    pe_very_cheap_max: float = Field(
        default=5.0, ge=0.0, le=20.0, description="Very cheap P/E threshold"
    )
    pe_acceptable_max: float = Field(
        default=25.0, ge=10.0, le=100.0, description="Acceptable P/E threshold"
    )
    pe_too_expensive_min: float = Field(
        default=25.0, ge=10.0, le=100.0, description="Too expensive P/E threshold"
    )
    pe_problematic_max: float = Field(
        default=5.0, ge=0.0, le=20.0, description="Problematic P/E threshold"
    )
    ...
    pb_very_cheap_max: float = Field(
        default=1.0, ge=0.0, le=5.0, description="Very cheap P/B threshold"
    )
    pb_too_expensive: float = Field(
        default=5.0, ge=1.0, le=50.0, description="Too expensive P/B threshold"
    )

    # ROE thresholds
    roe_excellent: float = Field(
        default=15.0, ge=0.0, le=100.0, description="Excellent ROE threshold (%)"
    )
    roe_good: float = Field(default=10.0, ge=0.0, le=50.0, description="Good ROE threshold (%)")

    # Dividend coverage
    dividend_coverage_excellent: float = Field(
        default=2.0, ge=1.0, le=10.0, description="Excellent dividend coverage"
    )
    dividend_coverage_good: float = Field(
        default=1.5, ge=1.0, le=5.0, description="Good dividend coverage"
    )

    # Score thresholds
    dividend_buy_score: float = Field(
        default=70.0, ge=0.0, le=100.0, description="Dividend buy score threshold"
    )
    dividend_hold_score: float = Field(
        default=50.0, ge=0.0, le=100.0, description="Dividend hold score threshold"
    )
...
class DividendThresholds(ConfigBase):
    ...
    min_dividend_yield: float = Field(
        default=0.02, ge=0.0, le=0.2, description="Minimum dividend yield (2%)"
    )
    max_dividend_yield: float = Field(
        default=0.10, ge=0.0, le=0.5, description="Maximum dividend yield (10%)"
    )

    # Dividend growth
    min_dividend_growth_rate: float = Field(
        default=0.05, ge=0.0, le=1.0, description="Minimum dividend growth rate"
    )

    # Payout ratio
    max_payout_ratio: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Maximum payout ratio (70%)"
    )

    # Dividend history
    min_dividend_history_years: int = Field(
        default=5, ge=1, le=30, description="Minimum dividend history years"
    )
...
class FXCarryTradeThresholds(ConfigBase):
    ...
    min_interest_differential: float = Field(
        default=0.02, ge=0.0, le=0.2, description="Minimum interest differential"
    )
