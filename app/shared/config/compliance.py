...
class ComplianceConfig(ConfigBase):
    ...
    RETURN_STABILITY_WINDOW: int = Field(
        default=15, ge=5, le=100, description="Window size for return stability calculation"
    )
    VOLATILITY_WINDOW: int = Field(
        default=20, ge=5, le=100, description="Window size for volatility calculation"
    )
    SPREAD_WINDOW: int = Field(
        default=20, ge=5, le=100, description="Window size for spread calculation"
    )
    FLOW_VOLATILITY_WINDOW: int = Field(
        default=10, ge=5, le=100, description="Window size for flow volatility calculation"
    )
...
class SpainTaxConfig(ConfigBase):
    ...
    IRPF_RATE_19: float = Field(
        default=0.19, ge=0.0, le=1.0, description="IRPF rate for income up to 12,450 EUR (19%)"
    )
    IRPF_RATE_21: float = Field(
        default=0.21, ge=0.0, le=1.0, description="IRPF rate for income 12,450-20,200 EUR (21%)"
    )
    IRPF_RATE_23: float = Field(
        default=0.23, ge=0.0, le=1.0, description="IRPF rate for income 20,200-35,200 EUR (23%)"
    )
    IRPF_RATE_27: float = Field(
        default=0.27, ge=0.0, le=1.0, description="IRPF rate for income 35,200-60,000 EUR (27%)"
    )
    IRPF_RATE_28: float = Field(
        default=0.28, ge=0.0, le=1.0, description="IRPF rate for income 60,000-300,000 EUR (28%)"
    )
    IRPF_RATE_30: float = Field(
        default=0.30, ge=0.0, le=1.0, description="IRPF rate for income over 300,000 EUR (30%)"
    )
    ...
    MODELO_720_THRESHOLD_ACCOUNTS_EUR: float = Field(
        default=50000.0, ge=0.0, description="Threshold for accounts - 50,000 EUR"
    )
    MODELO_720_THRESHOLD_SECURITIES_EUR: float = Field(
        default=50000.0, ge=0.0, description="Threshold for securities - 50,000 EUR"
    )
    MODELO_720_THRESHOLD_REAL_ESTATE_EUR: float = Field(
        default=50000.0, ge=0.0, description="Threshold for real estate rights - 50,000 EUR"
    )
    MODELO_720_THRESHOLD_INSURANCE_EUR: float = Field(
        default=50000.0, ge=0.0, description="Threshold for insurance products - 50,000 EUR"
    )
    ...
    TRADING_TAX_METHOD: str = Field(
        default="FIFO", description="Tax method for trading: FIFO, LIFO, HIFO, or AVERAGE"
    )
    ...
    TAX_LOSS_HARVESTING_ENABLED: bool = Field(
        default=True, description="Whether tax loss harvesting is enabled for Spain"
    )
