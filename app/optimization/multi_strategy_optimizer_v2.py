...
class MultiStrategyOptimizerV2:
    ...
    def _create_search_space(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        return {
            ...
            # Momentum
            "momentum.rsi_threshold": trial.suggest_int(
                "momentum.rsi_threshold", 35, 60
            ),
            "momentum.momentum_threshold": trial.suggest_float(
                "momentum.momentum_threshold", 0.01, 0.03
            ),
            "momentum.volume_threshold": trial.suggest_float(
                "momentum.volume_threshold", 1.0, 2.0
            ),
            "momentum.ema_period": trial.suggest_int(
                "momentum.ema_period", 9, 50
            ),
            ...
            # Global
            "global.stop_loss_pct": trial.suggest_float(
                "global.stop_loss_pct", 0.01, 0.03
            ),
            "global.position_size_pct": trial.suggest_float(
                "global.position_size_pct", 0.02, 0.08
            ),
        }
    ...
    def _save_results(self, study: optuna.Study) -> None:
        ...
        with open(opt_results_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                "trial", "objective", "sharpe", "return", "max_dd", "total_trades",
                "momentum_trades", "mean_rev_trades", "pairs_trades",
                "params_json"
            ])
