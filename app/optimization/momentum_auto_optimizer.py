...
class MomentumAutoOptimizer:
    ...
    def optimize_parameters(
        ...
        if current_params and self.history:
            ...
            min_improvement = self.auto_opt_config.get("performance_thresholds", {}).get("min_sharpe_improvement", 0.1)
            
            if best_score < last_sharpe + min_improvement:
                logger.info(f"Optimization did not improve enough ({best_score:.3f} vs {last_sharpe:.3f}), keeping current params")
        ...
        opt_record = {
            ...
            "improvement": best_score - (last_performance.get("sharpe_ratio", 0.0) if current_params else 0.0),
        }
