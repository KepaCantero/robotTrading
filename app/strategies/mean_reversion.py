...
class MeanReversionStrategy(BaseStrategy):
    ...
    def _calculate_z_score(self, market_data: Quote) -> Decimal:
        ...
        std_dev = market_data.last * Decimal("0.05")    # Desviación estándar simulada
    ...
    def _is_buy_signal(self, z_score: Decimal, volatility: Decimal, 
                      market_data: Quote) -> bool:
        """
        ...
        """
        return (
            z_score < -self.z_score_threshold and  # Precio significativamente bajo
            volatility < self.volatility_threshold and  # Volatilidad controlada
            abs(z_score) > self.min_z_score  # Desviación suficiente
        )
    
    def _is_sell_signal(self, z_score: Decimal, volatility: Decimal,
                       market_data: Quote) -> bool:
        """
        ...
        """
        return (
            z_score > self.z_score_threshold and  # Precio significativamente alto
            volatility < self.volatility_threshold and  # Volatilidad controlada
            abs(z_score) > self.min_z_score  # Desviación suficiente
        )
