...
class DeepLearningEngine(BaseLearningEngine):
    ...
    def __init__(self, config: Dict, defer_pytorch_init: bool = False):
        ...
        if defer_pytorch_init:
            ...
            logger.debug("DeepLearningEngine creado con defer_pytorch_init=True (PyTorch se inicializará en subprocess)")
            ...
            self.feature_columns = config.get("feature_columns", ['price', 'volume', 'rsi', 'ema_fast', 'ema_slow', 'momentum', 'atr'])
            self.scaler = None
            self.model = None
            return
        ...
        try:
            ...
            if 'mutex' in error_msg or 'lock' in error_msg:
                logger.error(
                    ...
                    f"Esto indica un problema con la instalación de PyTorch/MKL. "
    ...
    def _train_in_subprocess(
        ...
        def _train_worker(config_dict, training_data_bytes, validation_data_bytes, result_queue):
            ...
            try:
                ...
                validation_data = pickle.loads(validation_data_bytes) if validation_data_bytes else None
