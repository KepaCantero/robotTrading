...
class TransformerEngine(BaseLearningEngine):
    ...
    def __init__(self, config: Dict, defer_pytorch_init: bool = False):
        ...
        if defer_pytorch_init:
            ...
            logger.debug("TransformerEngine creado con defer_pytorch_init=True (PyTorch se inicializará en subprocess)")
        ...
        for i in range(len(data) - sequence_length):
            ...
            for item in window_data:
                ...
                numeric_features = [
                    ...
                    if isinstance(v, (int, float)) and k != target_key
                ]
                if numeric_features:
        ...
        try:
            ...
            if isinstance(training_data, dict):
                if 'sequences' in training_data and 'labels' in training_data:
                    ...
                    sequences, labels = self._prepare_sequences(
                        training_data.get('features', []),
                        sequence_length=30
                    )
                else:
                    # Intentar como lista de dicts
                    sequences, labels = self._prepare_sequences(
                        training_data.get('data', []),
                        sequence_length=30
                    )
            else:
                sequences, labels = self._prepare_sequences(
            ...
            if validation_data is not None:
                val_sequences, val_labels = self._prepare_sequences(
                    validation_data.get('data', validation_data),
                    sequence_length=30
                )
            ...
            logger.info(f"TransformerEngine entrenado: Loss={final_loss:.6f}" + 
    ...
    def predict(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'sequence' in features:
                ...
                feature_list = [v for v in features['features'].values() if isinstance(v, (int, float))]
        ...
        try:
            ...
            class TransformerDataset(Dataset):
                ...
                def __init__(self, sequences, labels):
                    ...
                    with torch.no_grad():
                        self.sequences = torch.from_numpy(sequences_np).clone()
                        self.labels = torch.from_numpy(labels_np).clone() if labels is not None else None
                def __len__(self):
                    return len(self.sequences)
                def __getitem__(self, idx):
                    if self.labels is not None:
