...
class AutomatedBacktestRunner:
    ...
    def _prepare_training_data(self, quotes: List[Quote], engine_type: str) -> Dict:
        ...
        temp_config = BacktestConfig(initial_capital=self.initial_capital, commission_per_trade=Decimal("1.0"), slippage_percentage=Decimal("0.001"))
        ...
        if engine_type == "supervised":
            return preparator.prepare_supervised_training_data(quotes=quotes, trades=historical_trades, min_sequence_length=60)
        elif engine_type == "deep":
            return preparator.prepare_deep_learning_training_data(quotes=quotes, trades=historical_trades, sequence_length=60, min_sequence_length=120)
        elif engine_type == "reinforcement":
            return preparator.prepare_reinforcement_learning_data(quotes=quotes, initial_capital=self.initial_capital)
