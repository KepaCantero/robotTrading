...
class HyperparameterOptimizer:
        ...
        for i, config in enumerate(configs):
            try:
                ...
                if result:
                    ...
                    logger.info(f"   Score: {score:.4f} | PnL: ${result.get('total_pnl', 0):.2f} | "
    ...
    def _generate_grid_search_configs(self, max_iterations: int) -> List[Dict[str, Any]]:
        ...
        for main_combo in main_combinations[:max_iterations]:
            ...
            config['atr_percentile_threshold'] = random.choice(self.parameter_space['atr_percentile_threshold'])
            ...
            if config['enable_learning'] and config['learning_engine_type']:
                algorithms = self.parameter_space['learning_algorithm'].get(config['learning_engine_type'], [])
                if algorithms:
                    config['learning_algorithm'] = random.choice(algorithms)
                    config['min_success_probability'] = random.choice(self.parameter_space['min_success_probability'])
                    config['rebalance_frequency_days'] = random.choice(self.parameter_space['rebalance_frequency_days'])
    ...
    def _generate_random_search_configs(self, max_iterations: int) -> List[Dict[str, Any]]:
        ...
        for _ in range(max_iterations):
            config = {
                ...
                'atr_percentile_threshold': random.choice(self.parameter_space['atr_percentile_threshold']),
            ...
            if config['enable_learning']:
                config['learning_engine_type'] = random.choice(self.parameter_space['learning_engine_type'])
                algorithms = self.parameter_space['learning_algorithm'].get(config['learning_engine_type'], [])
                if algorithms:
                    config['learning_algorithm'] = random.choice(algorithms)
                    config['min_success_probability'] = random.choice(self.parameter_space['min_success_probability'])
    ...
    def _run_backtest(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'annualized_return': float(result.annualized_return) if hasattr(result, 'annualized_return') else 0.0
            }
    ...
    def _build_strategy_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config_path = Path(__file__).parent.parent.parent.parent / "config" / "strategies" / "momentum_modular.yaml"
        base_config = {}
        ...
        if 'momentum_filter' in strategy_config['modules']:
            mom_params = strategy_config['modules']['momentum_filter'].get('max_positive_momentum', {})
            for preset in ['conservative', 'balanced', 'aggressive']:
                if preset in mom_params:
                    mom_params[preset]['min_positive_momentum'] = config.get('momentum_threshold', 0.015)
        ...
        if config.get('enable_learning', False):
            ...
            strategy_config['adaptive_learning']['engine_type'] = config.get('learning_engine_type', 'supervised')
            strategy_config['adaptive_learning']['algorithm'] = config.get('learning_algorithm', 'random_forest')
            strategy_config['adaptive_learning']['min_success_probability'] = config.get('min_success_probability', 0.6)
            strategy_config['adaptive_learning']['rebalance_frequency_days'] = config.get('rebalance_frequency_days', 7)
