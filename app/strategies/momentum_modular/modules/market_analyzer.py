...
class MarketAnalyzer:
    ...
    def __init__(self, config: Dict):
        ...
        self.trend_detector = TrendDetector(config) if config.get("trend_detection", {}).get("enabled", True) else None
        self.volatility_detector = VolatilityDetector(config) if config.get("volatility_detection", {}).get("enabled", True) else None
        self.range_detector = RangeDetector(config) if config.get("range_detection", {}).get("enabled", True) else None
    ...
    def analyze(self, market_data, price_history: List[float], atr_history: List[float]) -> Dict:
        ...
        if self.volatility_detector:
            vol_info = self.volatility_detector.detect(price_history, atr_history=atr_history if atr_history else [])
        else:
