    ...
    def get_recommendations(self) -> Dict:
        ...
        for context, metrics in self.context_performance.items():
            if metrics['trades'] >= 10:
                ctx_win_rate = metrics['successful'] / metrics['trades'] if metrics['trades'] > 0 else 0
                ctx_avg_pnl = float(metrics['pnl'] / Decimal(str(metrics['trades']))) if metrics['trades'] > 0 else 0
