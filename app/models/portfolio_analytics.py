...
from app.models.portfolio import Portfolio as BasePortfolio, Position
...
class ExtendedPortfolio(BasePortfolio):
    ...
    def validate_portfolio_consistency(self) -> 'ExtendedPortfolio':
        ...
        if abs(self.total_value - calculated_total) > Decimal('0.01'):
            raise ValueError(f"Total value mismatch. Expected: {calculated_total}, Got: {self.total_value}")
...
class PerformanceMetrics(BaseModel):
    ...
    @field_validator('total_return', 'annualized_return', 'volatility', 'sharpe_ratio', 'sortino_ratio', 
                     'max_drawdown', 'var_95', 'var_99', 'calmar_ratio', 'information_ratio', 
                     'treynor_ratio', 'jensen_alpha')
    @classmethod
...
class RiskMetrics(BaseModel):
    ...
    @field_validator('daily_volatility', 'annualized_volatility', 'realized_volatility', 
                     'downside_deviation', 'semi_variance', 'lower_partial_moment')
    @classmethod
...
class PortfolioAllocation(BaseModel):
    ...
    alternative_allocation: Decimal = Field(..., description="Alternative investments allocation percentage")
    
    # Geographic Allocation
    domestic_allocation: Decimal = Field(..., description="Domestic allocation percentage")
    international_allocation: Decimal = Field(..., description="International allocation percentage")
    
    # Sector Allocation
    sector_allocations: Dict[str, Decimal] = Field(default_factory=dict, description="Sector allocations")
    ...
    allocation_deviation: Optional[Dict[str, Decimal]] = Field(None, description="Allocation deviation from target")
    
    @field_validator('equity_allocation', 'fixed_income_allocation', 'cash_allocation', 
                     'alternative_allocation', 'domestic_allocation', 'international_allocation')
    @classmethod
...
class PortfolioRebalance(BaseModel):
    ...
    # Rebalancing Actions
    rebalance_actions: List[Dict[str, Any]] = Field(default_factory=list, description="Rebalancing actions")
...
class PortfolioComparison(BaseModel):
    ...
    performance_comparison: Dict[str, PerformanceMetrics] = Field(..., description="Performance comparison")
