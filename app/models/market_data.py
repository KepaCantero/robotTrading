...
class Quote(BaseModel):
    ...
    @field_validator('bid', 'ask', 'last', 'open', 'high', 'low', 'close')
    @classmethod
    def validate_price_fields(cls, v) -> Decimal:
        """Validate price fields are positive and within reasonable limits."""
    ...
    def validate_quote_consistency(self) -> 'Quote':
        ...
        if not (self.low <= self.open <= self.high):
            raise ValueError(f"Open price ({self.open}) must be between low ({self.low}) and high ({self.high}).")
