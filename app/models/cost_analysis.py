...
class CostBreakdownModel(BaseModel):
    ...
    infrastructure_cost: Decimal = Field(..., ge=0, description="Infrastructure cost", env="INFRASTRUCTURE_COST")
    ...
    cost_percentage: Decimal = Field(..., ge=0, le=100, description="Cost as percentage of trade value", env="COST_PERCENTAGE")
    cost_impact_ratio: Decimal = Field(..., ge=0, description="Cost Impact Ratio (CIR)", env="COST_IMPACT_RATIO")
    ...
    def validate_total_cost(cls, v, info=None):
        ...
        if 'commission' in values and 'slippage' in values and 'market_impact' in values:
            expected_total = (
                info.data['commission'] if info and hasattr(info, 'data') and 'commission' in info.data else None + 
                info.data['slippage'] if info and hasattr(info, 'data') and 'slippage' in info.data else None + 
                info.data['market_impact'] if info and hasattr(info, 'data') and 'market_impact' in info.data else None + 
                info.data.get('infrastructure_cost', Decimal('0') if info and hasattr(info, "data") else None) + 
                info.data.get('borrowing_cost', Decimal('0') if info and hasattr(info, "data") else None)
            )
    ...
    def validate_cost_percentage(cls, v, info=None):
        ...
        if 'total_cost' in values and 'execution_price' in values and 'quantity' in values:
            trade_value = info.data['execution_price'] if info and hasattr(info, 'data') and 'execution_price' in info.data else None * info.data['quantity'] if info and hasattr(info, 'data') and 'quantity' in info.data else None
            if trade_value > 0:
                expected_percentage = (info.data['total_cost'] if info and hasattr(info, 'data') and 'total_cost' in info.data else None / trade_value) * 100
...
class CostAnalysisResultModel(BaseModel):
    ...
    # Cost metrics
    total_commission: Decimal = Field(..., ge=0, description="Total commission costs", env="TOTAL_COMMISSION")
    total_slippage: Decimal = Field(..., ge=0, description="Total slippage costs", env="TOTAL_SLIPPAGE")
    total_market_impact: Decimal = Field(..., ge=0, description="Total market impact costs", env="TOTAL_MARKET_IMPACT")
    total_infrastructure: Decimal = Field(..., ge=0, description="Total infrastructure costs", env="TOTAL_INFRASTRUCTURE")
    total_borrowing: Decimal = Field(..., ge=0, description="Total borrowing costs", env="TOTAL_BORROWING")
    ...
    cost_impact_ratio: Decimal = Field(..., ge=0, description="Cost Impact Ratio (CIR)", env="COST_IMPACT_RATIO")
    profitability_threshold: Decimal = Field(..., ge=0, description="Minimum profitability threshold", env="PROFITABILITY_THRESHOLD")
    
    # Cost breakdown by trade
    cost_breakdowns: List[CostBreakdownModel] = Field(..., description="Cost breakdowns by trade")
    
    # Validation results
    is_profitable: bool = Field(..., description="Whether strategy is profitable", env="IS_PROFITABLE")
    exceeds_cost_threshold: bool = Field(..., description="Whether costs exceed threshold", env="EXCEEDS_COST_THRESHOLD")
    ...
    def validate_total_costs(cls, v, info=None):
        ...
        if all(key in values for key in ['total_commission', 'total_slippage', 'total_market_impact', 'total_infrastructure', 'total_borrowing']):
            expected_total = (
                info.data['total_commission'] if info and hasattr(info, 'data') and 'total_commission' in info.data else None + 
                info.data['total_slippage'] if info and hasattr(info, 'data') and 'total_slippage' in info.data else None + 
                info.data['total_market_impact'] if info and hasattr(info, 'data') and 'total_market_impact' in info.data else None + 
                info.data['total_infrastructure'] if info and hasattr(info, 'data') and 'total_infrastructure' in info.data else None + 
                info.data['total_borrowing'] if info and hasattr(info, 'data') and 'total_borrowing' in info.data else None
            )
            if abs(v - expected_total) > Decimal('0.01'):
    ...
    def validate_net_profit(cls, v, info=None):
        ...
        if 'gross_profit' in values and 'total_costs' in values:
            expected_net = info.data['gross_profit'] if info and hasattr(info, 'data') and 'gross_profit' in info.data else None - info.data['total_costs'] if info and hasattr(info, 'data') and 'total_costs' in info.data else None
    ...
    def validate_cost_impact_ratio(cls, v, info=None):
        ...
        if 'total_costs' in values and 'gross_profit' in values:
            if info.data['gross_profit'] if info and hasattr(info, 'data') and 'gross_profit' in info.data else None > 0:
                expected_cir = (info.data['total_costs'] if info and hasattr(info, 'data') and 'total_costs' in info.data else None / info.data['gross_profit'] if info and hasattr(info, 'data') and 'gross_profit' in info.data else None) * 100
...
class TradeCostAnalysisRequest(BaseModel):
    ...
    status: TradeStatus = Field(default=TradeStatus.CLOSED, description="Trade status", env="STATUS")
...
class ProfitabilityValidationRequest(BaseModel):
    ...
    cost_impact_ratio: Decimal = Field(..., ge=0, description="Cost Impact Ratio", env="COST_IMPACT_RATIO")
    is_profitable: bool = Field(..., description="Whether strategy is profitable", env="IS_PROFITABLE")
    exceeds_cost_threshold: bool = Field(..., description="Whether costs exceed threshold", env="EXCEEDS_COST_THRESHOLD")
...
class CostParametersModel(BaseModel):
    ...
    infrastructure_cost_per_trade: Decimal = Field(..., ge=0, description="Infrastructure cost per trade", env="INFRASTRUCTURE_COST_PER_TRADE")
    borrowing_cost_rate: Decimal = Field(..., ge=0, le=1, description="Annual borrowing cost rate", env="BORROWING_COST_RATE")
    min_profitability_threshold: Decimal = Field(..., ge=0, description="Minimum profitability threshold", env="MIN_PROFITABILITY_THRESHOLD")
    max_cost_impact_ratio: Decimal = Field(..., ge=0, le=100, description="Maximum allowed Cost Impact Ratio", env="MAX_COST_IMPACT_RATIO")
...
class CostAnalysisResponse(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Response timestamp", env="TIMESTAMP")
...
class ProfitabilityValidationResponse(BaseModel):
    ...
    cost_impact_ratio: Decimal = Field(..., ge=0, description="Cost Impact Ratio", env="COST_IMPACT_RATIO")
    max_allowed_cir: Decimal = Field(..., ge=0, description="Maximum allowed CIR", env="MAX_ALLOWED_CIR")
    recommendations: List[str] = Field(..., description="Recommendations for improvement")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Response timestamp", env="TIMESTAMP")
