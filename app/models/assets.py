...
class Asset(BaseModel):
    ...
    # Liquidity metrics
    liquidity_score: float = Field(default=0.0, ge=0, le=100, description="Overall liquidity score (0-100)")
    ...
    max_trade_size: Decimal = Field(default=Decimal("1000000"), ge=0, description="Maximum trade size")
    ...
    def volume_score(self) -> float:
        ...
        return min(100.0, max(0.0, (log_volume - 2) * 20))  # Scale 100-10000 to 0-100
...
class AssetUniverse(BaseModel):
    ...
    avg_liquidity_score: float = Field(default=0.0, ge=0, le=100, description="Average liquidity score")
    total_market_cap: Optional[Decimal] = Field(None, ge=0, description="Total market cap of universe")
    ...
    def _update_statistics(self):
        ...
        if self.assets:
            self.avg_liquidity_score = sum(asset.liquidity_score for asset in self.assets) / len(self.assets)
    ...
    def update_asset_liquidity(self, symbol: str, liquidity_score: float, 
                              avg_volume: Decimal, avg_spread: Decimal) -> bool:
...
class LiquidityMetrics(BaseModel):
    ...
    volume_volatility: float = Field(default=0.0, ge=0, le=100, description="Volume volatility (0-100)")
    ...
    spread_volatility: float = Field(default=0.0, ge=0, le=100, description="Spread volatility (0-100)")
    
    # Price metrics
    current_price: Decimal = Field(ge=0, description="Current market price")
    price_volatility: float = Field(default=0.0, ge=0, le=100, description="Price volatility (0-100)")
...
class AssetRanking(BaseModel):
    ...
    def add_ranking(self, symbol: str, liquidity_score: float, rank: int, 
                   volume_score: float, spread_score: float) -> None:
        """Add asset ranking."""
        ranking_entry = {
            "symbol": symbol,
...
class AssetFilter(BaseModel):
    ...
    min_liquidity_score: float = Field(default=50.0, ge=0, le=100, description="Minimum liquidity score")
