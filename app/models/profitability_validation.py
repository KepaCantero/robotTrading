...
class CostBreakdown(BaseModel):
    ...
    @field_validator('commissions', 'slippage', 'market_impact', 'infrastructure', 
                     'data_fees', 'financing', 'other')
    @classmethod
...
class ValidationCriteria(BaseModel):
    ...
    min_profit_factor: Decimal = Field(default=Decimal('1.5'), description="Factor de ganancia mínimo")
    max_cost_impact_ratio: Decimal = Field(default=Decimal('0.3'), description="Ratio máximo de impacto de costos")
...
class ProfitabilityValidation(BaseModel):
    ...
    validation_date: datetime = Field(default_factory=datetime.now, description="Fecha de validación")
...
class StrategyComparison(BaseModel):
    ...
    comparison_date: datetime = Field(default_factory=datetime.now, description="Fecha de comparación")
    strategies: List[ProfitabilityValidation] = Field(..., description="Validaciones de estrategias")
...
class ValidationReport(BaseModel):
    ...
    strategy_validations: List[ProfitabilityValidation] = Field(..., description="Validaciones de estrategias")
    strategy_comparison: Optional[StrategyComparison] = Field(None, description="Comparación de estrategias")
    historical_analysis: Optional[HistoricalValidation] = Field(None, description="Análisis histórico")
