...
class PaperTrade(BaseModel):
    ...
    def validate_trade_consistency(self) -> 'PaperTrade':
        ...
        if self.filled_quantity > self.quantity:
            raise ValueError(f"Filled quantity ({self.filled_quantity}) cannot exceed order quantity ({self.quantity})")
        
        if self.status == TradeStatus.FILLED and self.filled_quantity != self.quantity:
            raise ValueError(f"Filled trade must have filled_quantity equal to quantity")
        
        if self.status == TradeStatus.PARTIALLY_FILLED and self.filled_quantity >= self.quantity:
            raise ValueError(f"Partially filled trade must have filled_quantity less than quantity")
        
        if self.filled_at and self.filled_at < self.created_at:
            raise ValueError(f"Fill time ({self.filled_at}) cannot be before creation time ({self.created_at})")
...
class PaperPosition(BaseModel):
    ...
    # Position details
    quantity: Decimal = Field(..., description="Current position quantity (can be negative for short)")
    ...
    # Timing
    opened_at: datetime = Field(default_factory=datetime.utcnow, description="Position opening time")
...
class PaperPortfolio(BaseModel):
    ...
    # Settings
    simulation_mode: PaperTradingMode = Field(default=PaperTradingMode.REALISTIC, description="Simulation mode")
    config_id: Optional[UUID] = Field(None, description="Configuration ID")
    commission_rate: Decimal = Field(default=Decimal("0.001"), ge=0, le=Decimal("0.01"), description="Commission rate")
    slippage_rate: Decimal = Field(default=Decimal("0.0005"), ge=0, le=Decimal("0.01"), description="Slippage rate")
    
    # Timestamps
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Portfolio creation time")
    last_updated: datetime = Field(default_factory=datetime.utcnow, description="Last update time")
...
class PaperTradingConfig(BaseModel):
    ...
    # Simulation settings
    simulation_mode: PaperTradingMode = Field(default=PaperTradingMode.REALISTIC, description="Simulation mode")
    initial_cash: Decimal = Field(default=Decimal("100000"), gt=0, description="Initial cash amount")
    
    # Trading costs
    commission_rate: Decimal = Field(default=Decimal("0.001"), ge=0, le=Decimal("0.01"), description="Commission rate")
    slippage_rate: Decimal = Field(default=Decimal("0.0005"), ge=0, le=Decimal("0.01"), description="Slippage rate")
    market_impact_rate: Decimal = Field(default=Decimal("0.0001"), ge=0, le=Decimal("0.01"), description="Market impact rate")
    
    # Risk limits
    max_position_size: Decimal = Field(default=Decimal("0.1"), gt=0, le=Decimal("1"), description="Max position size as % of portfolio")
    max_daily_loss: Decimal = Field(default=Decimal("0.05"), gt=0, le=Decimal("0.5"), description="Max daily loss as % of portfolio")
    max_drawdown: Decimal = Field(default=Decimal("0.15"), gt=0, le=Decimal("0.5"), description="Max drawdown as % of portfolio")
    
    # Execution settings
    execution_delay_ms: int = Field(default=100, ge=0, le=5000, description="Execution delay in milliseconds")
    partial_fill_probability: Decimal = Field(default=Decimal("0.1"), ge=0, le=Decimal("1"), description="Probability of partial fills")
...
class PaperTradingSession(BaseModel):
    ...
    def validate_session_timing(self) -> 'PaperTradingSession':
        ...
        if self.ended_at and self.ended_at < self.started_at:
            raise ValueError(f"End time ({self.ended_at}) cannot be before start time ({self.started_at})")
        
        if self.last_activity < self.started_at:
            raise ValueError(f"Last activity ({self.last_activity}) cannot be before start time ({self.started_at})")
