...
class BacktestMetaAnalyzer:
    ...
    def __init__(
        ...
        logger.info(f"BacktestMetaAnalyzer inicializado: data_dir={data_dir}, output_dir={self.output_dir}")
    ...
    async def load_results(self, path: Optional[str] = None) -> int:
        ...
        logger.info(f"Encontrados {total_files} archivos ({len(json_files)} JSON, {len(csv_files)} CSV)")
    ...
    def analyze_performance(self) -> Dict[str, Any]:
        ...
        for cat_col in category_columns:
            if cat_col in self.df_results.columns:
                analysis['performance_by_category'][cat_col] = self._analyze_by_category(cat_col, available_metrics)
    ...
    def detect_clusters(self, n_clusters: int = 3, features: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        if features is None:
            features = [
                'sharpe_ratio', 'total_pnl', 'max_drawdown', 'win_rate',
                'return_pct', 'total_trades'
            ]
    ...
    def _generate_visualizations(self) -> None:
        ...
        try:
            ...
            if 'sharpe_ratio' in self.df_results.columns:
                ...
                plt.savefig(self.output_dir / 'sharpe_distribution.png', dpi=150, bbox_inches='tight')
                plt.close()
            ...
            if len(self.df_results.select_dtypes(include=[np.number]).columns) > 1:
                numeric_cols = self.df_results.select_dtypes(include=[np.number]).columns[:10]  # Top 10
