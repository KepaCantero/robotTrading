...
class InsightGenerator:
    ...
    def generate_statistical_insights(self, metrics: Dict[str, float]) -> List[str]:
        ...
        try:
            ...
            if sharpe > 2.0:
                self.insights.append(f"✅ Exceptional risk-adjusted returns (Sharpe: {sharpe:.2f} > 2.0)")
            elif sharpe > 1.0:
    ...
    def generate_recommendations(
        ...
        try:
            ...
            if regimes and len(regimes) > 0:
                ...
                sharpe_variance = np.var([r.get("sharpe_ratio", 0) for r in regime_performance if isinstance(r, dict)])
            ...
            if regimes:
                ...
                for regime_name, regime_data in regimes.items():
                    if isinstance(regime_data, dict):
                        ...
                        report_lines.append(f"- Time in regime: {regime_data.get('pct_time', 0):.1f}%")
                        report_lines.append(f"- Return: {regime_data.get('total_return', 0)*100:.2f}%")
                        report_lines.append(f"- Sharpe: {regime_data.get('sharpe_ratio', 0):.2f}")
                        report_lines.append(f"- Max DD: {regime_data.get('max_drawdown', 0)*100:.1f}%")
                        report_lines.append("")
