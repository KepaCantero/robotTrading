...
class SuccessfulConfigManager:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        Args:
            storage_dir: Directorio donde guardar configuraciones. Por defecto: config/successful_configs/
        """
        if storage_dir is None:
            storage_dir = Path("config") / "successful_configs"
        else:
    ...
    def load_config(self, config_id: Optional[str] = None, name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def list_configs(
        ...
        if tags:
            filtered = [
                c for c in filtered
                if all(tag in c.get('tags', []) for tag in tags)
            ]
        ...
        filtered.sort(
            key=lambda x: x.get(sort_by) or (0 if sort_by != 'timestamp' else ''),
            reverse=reverse
        )
    ...
    def delete_config(self, config_id: Optional[str] = None, name: Optional[str] = None) -> bool:
        ...
        for i, config in enumerate(self._configs):
            if (config_id and config.get('id') == config_id) or (name and config.get('name') == name):
                self._configs.pop(i)
                removed = True
                break
        ...
        if removed:
            ...
            logger.warning(f"Configuración no encontrada para eliminar: id={config_id}, name={name}")
        
        return removed
    
    def get_config_for_runner(self, config_id: Optional[str] = None, name: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def compare_configs(self, config_id1: str, config_id2: str) -> Dict[str, Any]:
        ...
        comparison = {
            'config1': {
                'name': config1.get('name'),
                'id': config1.get('id'),
                'metrics': metrics1
            },
            'config2': {
                'name': config2.get('name'),
                'id': config2.get('id'),
                'metrics': metrics2
            },
            'differences': {}
        ...
        numeric_metrics = ['sharpe_ratio', 'return_pct', 'win_rate', 'max_drawdown', 'total_pnl', 'sortino_ratio']
