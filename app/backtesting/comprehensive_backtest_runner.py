...
class ComprehensiveBacktestRunner:
        ...
        if self.meta_enabled:
            try:
                ...
                meta = integrate_meta_analyzer_with_runner(
                    ...
                    enable_analysis=self.config.get('meta_analysis', {}).get(
                ...
                hash_display = self.audit_hash[:16] if self.audit_hash else 'N/A'
                ...
                if self.audit_trail:
                    ...
                    if git_info.get('commit_hash'):
                        logger.info(
                            f"📝 Git commit: {git_info['commit_hash'][:8]} (branch: {git_info.get('branch', 'unknown')})"
                        )
            except Exception as e:
    ...
    def _load_portfolio_market_data(self, start_date: datetime, end_date: datetime) -> List:
        ...
        quotes = portfolio_builder.build_portfolio_quotes(start_date=start_date, end_date=end_date)
    ...
    def _create_strategy_config(
        ...
        if strategy_yaml_path.exists():
            ...
            base_config = {'name': 'momentum_modular', 'enabled': True, 'preset': 'balanced'}
        ...
        if learning_engine_override:
            ...
            learning_engine_config = self.config.get('learning_engines', {}).get(
                learning_engine_override, {}
            )
    ...
    def _calculate_consistent_metrics(
        self, result: Any, initial_capital: Decimal
    ) -> Dict[str, float]:
        """
        ...
        return_pct = (
            float((final_capital - initial_capital) / initial_capital * 100)
    ...
    def _train_learning_engine_if_needed(
        ...
        if self.learning_storage and self.config.get('meta_analysis', {}).get(
            'enable_incremental_learning', True
        ):
            try:
                logger.info(
                    f"    🔄 Intentando cargar pesos guardados para {learning_engine_name}..."
                )
                saved_data = self.learning_storage.load_weights(
                    engine_name=learning_engine_name, latest=True
                )
                ...
                if saved_data and 'weights' in saved_data:
                    ...
                    if hasattr(model, 'load_state_dict') and isinstance(weights, dict):
                        ...
                        weights_loaded = True
                        logger.info(
                            f"    ✅ Pesos cargados desde test anterior ({saved_data.get('test_id', 'unknown')})"
                        )
                        ...
                        weights_loaded = True
                        logger.info(
                            f"    ✅ Modelo cargado desde test anterior ({saved_data.get('test_id', 'unknown')})"
                        )
                ...
                logger.debug(
                    f"    ℹ️ No se encontraron pesos guardados para {learning_engine_name}, entrenando desde cero"
                )
            except Exception as e:
                logger.warning(
                    f"    ⚠️ Error cargando pesos guardados: {e}. Continuando con entrenamiento desde cero."
                )

        # Entrenar (desde cero o fine-tuning si se cargaron pesos)
        logger.info(
            f"    🎓 Entrenando {learning_engine_name} learning engine{' (fine-tuning)' if weights_loaded else ' (desde cero)'}..."
        )
        ...
        logger.info(
            f"       - Strategy tiene learning_engine: {strategy.learning_engine is not None}"
        )
        ...
        try:
            training_data = self._prepare_learning_training_data(
                self.quotes, learning_engine_name, strategy
            )
            ...
            if training_data:
                if isinstance(training_data, dict):
                    ...
                    if learning_engine_name == 'supervised':
                        has_valid_data = (
                            'X' in training_data
                            and 'y' in training_data
                            and len(training_data.get('X', [])) > 0
                        )
                    # Para deep: debe tener 'sequences' y 'labels'
                    elif learning_engine_name == 'deep':
                        has_valid_data = (
                            'sequences' in training_data
                            and 'labels' in training_data
                            and len(training_data.get('sequences', [])) > 0
                        )
                    # Para reinforcement: debe tener 'quotes' o 'observations'
                    elif learning_engine_name == 'reinforcement':
                        has_valid_data = (
                            'quotes' in training_data or 'observations' in training_data
                        )
            ...
            if has_valid_data:
                ...
                if learning_engine_name == 'supervised' and isinstance(training_data, dict):
                    if 'labels' in training_data:
                        ...
                        if hasattr(labels, 'unique'):
                            ...
                            unique_labels = len(
                                set(labels) if hasattr(labels, '__iter__') else [labels]
                            )
                ...
                if learning_engine_name in ['deep', 'transformer']:
                    ...
                    logger.info(
                        f"    🔄 Usando subprocess para {learning_engine_name} (previene mutex.cc blocking)"
                    )
                    ...
                    if use_subprocess and hasattr(strategy.learning_engine.train, '__code__'):
                        ...
                        if 'use_subprocess' in sig.parameters:
                            metrics = strategy.learning_engine.train(
                                training_data, use_subprocess=use_subprocess
                            )
                        else:
                ...
                logger.warning(
                    f"    ⚠️ No se pudieron preparar datos de entrenamiento para {learning_engine_name}"
                )
                logger.debug(
                    f"    📋 training_data type: {type(training_data)}, content: {training_data}"
                )
                if training_data:
                    logger.debug(
                        f"    📋 training_data keys: {training_data.keys() if isinstance(training_data, dict) else 'N/A'}"
                    )
                return False
        except Exception as e:
    ...
    def run_baseline_backtest(self) -> Dict[str, Any]:
        ...
        strategy_name = (
            getattr(strategy, 'name', 'momentum_modular') if strategy else 'momentum_modular'
        )
        ...
        backtester = SimpleBacktester(
            config=backtest_config, strategy=strategy, strategy_name=strategy_name
        )
        ...
        logger.info(
            f"✅ Baseline Backtest completado: PnL=${result_dict['total_pnl']:.2f}, Sharpe={result_dict['sharpe_ratio']:.2f}"
        )
    ...
    def run_learning_engines_backtest(self) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"📊 Learning engines a probar: {learning_engines_to_test if learning_engines_to_test else 'ninguno'}"
        )

        if not learning_engines_to_test and not use_multi_strategy:
            logger.warning(
                "⚠️ No hay learning engines habilitados para probar y multi-strategy está deshabilitado"
            )
        ...
        if use_multi_strategy:
            try:
                ...
                self.results.append(result_dict)
                logger.info(
                    f"  ✅ Multi-Strategy Baseline: PnL=${result_dict['total_pnl']:.2f}, Sharpe={result_dict['sharpe_ratio']:.2f}, Trades={result_dict['total_trades']}"
                )
            except Exception as e:
        ...
        for learning_engine in learning_engines_to_test:
            ...
            baseline_result = baseline_backtester.run_backtest(
                self.quotes, signals=baseline_signals
            )
            ...
            logger.info(
                f"    📉 Baseline (antes): PnL=${before_training_metrics['total_pnl']:.2f}, Sharpe={before_training_metrics['sharpe_ratio']:.2f}, Return={before_training_metrics['return_pct']:.2f}%"
            )
            ...
            use_subprocess = learning_engine in [
                'deep',
                'transformer',
            ]  # Siempre usar subprocess para estos
            ...
            try:
                ...
                if 'mutex' in error_msg or 'lock' in error_msg or 'blocking' in error_msg:
                    ...
                    if learning_engine in ['deep', 'transformer'] and not use_subprocess:
                        logger.warning(
                            f"⚠️ Fallo en entrenamiento de {learning_engine}. Reintentando con subprocess..."
                        )
                        try:
                            training_successful = self._train_learning_engine_if_needed(
                                strategy, learning_engine, use_subprocess=True
            ...
            if not training_successful:
                ...
                if learning_engine in ['deep', 'transformer']:
                    if (
                        hasattr(strategy.learning_engine, 'model')
                        and strategy.learning_engine.model is None
                    ):
                        logger.warning(
                ...
                for quote in self.quotes:
                    try:
                        ...
                        if (
                            'mutex' in str(e).lower()
                            or 'lock' in str(e).lower()
                            or 'blocking' in str(e).lower()
                        ):
                            logger.warning(
                                f"    ⚠️ Bloqueo detectado generando señales para {quote.symbol if hasattr(quote, 'symbol') else 'unknown'}. Saltando..."
                            )
                            continue
                        raise
            ...
            logger.info(
                f"    📈 Después: PnL=${after_training_metrics['total_pnl']:.2f}, Sharpe={after_training_metrics['sharpe_ratio']:.2f}, Return={after_training_metrics['return_pct']:.2f}%"
            )
            logger.info(
                f"    📊 Mejora: Sharpe {improvement_pct.get('sharpe_ratio', 0):.2f}%, Return {improvement_pct.get('return_pct', 0):.2f}%"
            )
            ...
            self.results.append(result_dict)
            logger.info(
                f"  ✅ {learning_engine}: PnL=${result_dict['total_pnl']:.2f}, Sharpe={result_dict['sharpe_ratio']:.2f}, Mejora Sharpe={improvement_pct.get('sharpe_ratio', 0):.2f}%"
            )
        ...
        if learning_engines_to_test:
            ...
            def run_engine_test(learning_engine: str) -> Optional[Dict[str, Any]]:
                ...
                try:
                    ...
                    baseline_consistent_metrics = self._calculate_consistent_metrics(
                        baseline_result, initial_capital
                    )
                    ...
                    training_successful = self._train_learning_engine_if_needed(
                        strategy, learning_engine, use_subprocess=use_subprocess_engine
                    )
                        ...
                        for metric in before_training_metrics.keys():
                            ...
                            if before_val != 0:
                                improvement_pct[metric] = (
                                    (after_val - before_val) / abs(before_val)
                                ) * 100
                            else:
                                improvement_pct[metric] = 100.0 if after_val > 0 else 0.0
    ...
    def run_ablation_study(self) -> List[Dict[str, Any]]:
        ...
        for module_name in modules_to_test:
            ...
            learning_engine_type = (
                self.config.get('learning_engines', {}).get('supervised', {}).get('enabled')
                and 'supervised'
                or None
            )
            ...
            result_dict = {
                ...
                'modules_active': [
                    m for m in self.config['modules']['filters'].keys() if m != module_name
                ],
            ...
            logger.info(
                f"  ✅ {module_name} disabled: PnL=${result_dict['total_pnl']:.2f}, Sharpe={result_dict['sharpe_ratio']:.2f}"
            )
    ...
    def run_grid_search(self) -> List[Dict[str, Any]]:
        ...
        param_combinations = self._generate_parameter_combinations(
            parameters_to_optimize, num_combinations, search_method
        )
        ...
        for i, param_combo in enumerate(param_combinations):
            ...
            if (i + 1) % 10 == 0:
                logger.info(
                    f"  📊 Progress: {i+1}/{num_combinations}, Best {optimize_metric}: {best_metric_value:.2f}"
                )

        if best_result:
            logger.info(
                f"✅ Grid Search completado. Mejor combinación: {optimize_metric}={best_metric_value:.2f}"
            )
    ...
    def run_grid_search_parallel(self) -> List[Dict[str, Any]]:
        ...
        def run_combination_test(
            param_combo: Dict[str, Any], combo_num: int
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if completed % 10 == 0:
                        logger.info(
                            f"  📊 Progress: {completed}/{num_combinations}, Best {optimize_metric}: {best_metric_value:.2f}"
                        )
        ...
        if best_result:
            logger.info(
                f"✅ Grid Search (PARALELO) completado. Mejor combinación: {optimize_metric}={best_metric_value:.2f}"
            )
            logger.info(f"   Thresholds: {best_result['thresholds_used']}")
            logger.info(
                f"   PnL: ${best_result['total_pnl']:.2f}, Sharpe: {best_result['sharpe_ratio']:.2f}"
            )
    ...
    def _generate_parameter_combinations(
        self, parameters: List[str], num_combinations: int, method: str = 'random'
    ) -> List[Dict[str, Any]]:
    ...
    def _prepare_learning_training_data(
        self, quotes: List, engine_type: str, strategy: ModularMomentumStrategy
    ) -> Dict[str, Any]:
        """
        ...
        """
        from app.strategies.momentum_modular.learning.training_data_preparator import (
        ...
        if engine_type in ['deep', 'transformer']:
            logger.info(
                f"    🔄 Preparando datos para {engine_type} usando estrategia SIN learning engine (previene mutex.cc)"
            )
        ...
        if engine_type == "supervised":
            ...
            lookahead_days = supervised_config.get('lookahead_days') or supervised_config.get(
                'parameters', {}
            ).get('lookahead_days', 15)
            ...
            return preparator.prepare_reinforcement_learning_data(
                quotes=quotes, initial_capital=initial_capital
            )
    ...
    def run_walk_forward_backtest(self) -> List[Dict[str, Any]]:
        ...
        train_test_split = wf_config.get(
            'train_test_split', 0.7
        )  # 70% train, 30% test dentro de ventana
        ...
        while current_start_idx < len(self.quotes):
            ...
            window_end_date = self.quotes[current_start_idx].timestamp + timedelta(
                days=window_size_days
            )
            ...
            logger.info(
                f"  🔍 Window {window_num}: {window_quotes[0].timestamp.date()} to {window_quotes[-1].timestamp.date()} ({len(window_quotes)} quotes)"
            )
            ...
            if learning_engine and len(window_quotes) >= 60:
                ...
                logger.info(
                    f"    📊 Train: {len(train_window_quotes)} quotes, Test: {len(test_window_quotes)} quotes"
                )
                ...
                if strategy.learning_engine:
                    ...
                    try:
                        training_data = self._prepare_learning_training_data(
                            train_window_quotes, learning_engine, strategy
                        )
                        ...
                        if training_data and len(training_data) > 0:
                            ...
                            if len(val_quotes) >= 20:
                                validation_data = self._prepare_learning_training_data(
                                    val_quotes, learning_engine, strategy
                                )

                            # Entrenar
                            metrics = strategy.learning_engine.train(
                                training_data, validation_data=validation_data
                            )
            ...
            result_dict = {
                ...
                'modules_active': list(self.config['modules']['filters'].keys()),
                'learning_engine': learning_engine,
                'learning_engine_trained': (
                    learning_engine is not None and strategy.learning_engine is not None
                ),
            ...
            logger.info(
                f"    ✅ Window {window_num}: PnL=${result_dict['total_pnl']:.2f}, "
        ...
        if len(results) > 1:
            ...
            logger.info(
                f"    Consistencia: {sum(1 for r in results if r['sharpe_ratio'] > 0) / len(results) * 100:.1f}% ventanas positivas"
            )

        logger.info(f"✅ Walk-Forward completado: {len(results)} ventanas")
        return results

    def _optimize_thresholds_in_window(
        self, quotes: List, num_combinations: int = 20
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        grid_config = self.config['backtests'].get('grid_search', {})
        parameters_to_optimize = grid_config.get(
            'parameters_to_optimize',
            ["rsi_filter.buy_threshold", "momentum_filter.threshold", "volume_filter.threshold"],
        )
        ...
        param_combinations = self._generate_parameter_combinations(
            parameters_to_optimize, num_combinations, 'random'
        )
        ...
        if use_multi_strategy:
            try:
                ...
                multi_backtester, _ = self._create_multi_strategy_setup(
                    enable_dynamic_reallocation=multi_config.get(
                ...
                logger.warning(
                    f"  ⚠️ No se pudo crear multi-strategy setup: {e}. Usando estrategia modular."
                )
        ...
        for sim_num in range(num_simulations):
            ...
            if use_multi_strategy and multi_backtester:
                ...
                try:
                    ...
                    logger.warning(
                        f"  ⚠️ Error en multi-strategy simulation {sim_num+1}: {e}. Reintentando con estrategia modular."
                    )
            ...
            if not learning_engine_type:
                learning_engine_type = (
                    self.config.get('learning_engines', {}).get('deep', {}).get('enabled')
                    and 'deep'
                    or None
                )
            if not learning_engine_type:
                learning_engine_type = (
                    self.config.get('learning_engines', {}).get('reinforcement', {}).get('enabled')
                    and 'reinforcement'
                    or None
                )
    ...
    def run_monte_carlo_backtest_parallel(self) -> List[Dict[str, Any]]:
        ...
        results = []
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(run_single_simulation, i): i for i in range(num_simulations)}
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if completed % 10 == 0:
                        logger.info(
                            f"  📊 Progress: {completed}/{num_simulations} simulaciones completadas"
                        )
    ...
    def _analyze_regime_distribution(self, quotes: List) -> Dict[str, Any]:
        ...
        for quote in quotes:
            ...
            if len(price_history) >= 60:
                market_context = self._detect_regime_for_quote(
                    quote, price_history.copy(), atr_history.copy() if atr_history else []
                )
        ...
        regime_percentages = {
            k: (v / total * 100) if total > 0 else 0.0 for k, v in regime_counts.items()
        }
    ...
    def run_out_of_sample_backtest(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"  📊 Train: {len(train_quotes)} quotes ({train_quotes[0].timestamp.date()} to {train_quotes[-1].timestamp.date()})"
        )
        logger.info(
            f"  📊 Test: {len(test_quotes)} quotes ({test_quotes[0].timestamp.date()} to {test_quotes[-1].timestamp.date()})"
        )
        ...
        if validate_regimes:
            ...
            train_primary = (
                max(train_regimes['percentages'].items(), key=lambda x: x[1])[0]
                if train_regimes['percentages']
                else 'unknown'
            )
            test_primary = (
                max(test_regimes['percentages'].items(), key=lambda x: x[1])[0]
                if test_regimes['percentages']
                else 'unknown'
            )
            ...
            common_regimes = set(train_regimes['percentages'].keys()) & set(
                test_regimes['percentages'].keys()
            )
            regime_similarity = (
                sum(
                    abs(
                        train_regimes['percentages'].get(r, 0)
                        - test_regimes['percentages'].get(r, 0)
                    )
            ...
            if regime_similarity > 30.0:  # Más del 30% de diferencia
                logger.warning(
                    f"    ⚠️ ALTA DISIMILITUD de regímenes: {regime_similarity:.1f}% diferencia promedio"
                )
                regime_warning = True
        ...
        if use_optimized_params and self.results:
            ...
            if grid_results:
                ...
                logger.info(
                    f"  ✅ Usando parámetros optimizados del grid_search (Sharpe={best_grid.get('sharpe_ratio', 0):.2f})"
                )
                strategy_config = self._create_strategy_config(
                    thresholds_override=thresholds_override,
                    learning_engine_override=learning_engine,
                ...
                logger.warning(
                    "  ⚠️ No se encontraron resultados de grid_search, usando parámetros por defecto"
                )
                strategy_config = self._create_strategy_config(
                    learning_engine_override=learning_engine
                )
        ...
        if learning_engine:
            ...
            if strategy_train.learning_engine:
                try:
                    training_data = self._prepare_learning_training_data(
                        train_quotes, learning_engine, strategy_train
                    )
                    ...
                    if training_data and len(training_data) > 0:
                        ...
                        if len(val_quotes) >= 20:
                            validation_data = self._prepare_learning_training_data(
                                val_quotes, learning_engine, strategy_train
                            )
        ...
        if (
            learning_engine
            and strategy_train.learning_engine
            and strategy_train.learning_engine.is_ready()
        ):
            ...
            strategy_test = ModularMomentumStrategy(
                strategy_config
            )  # Misma configuración, sin learning engine
        ...
        train_sharpe = (
            float(result_train.performance.sharpe_ratio)
            if result_train.performance.sharpe_ratio
            else 0.0
        )
        test_sharpe = (
        ...
        sharpe_degradation = (
            ((train_sharpe - test_sharpe) / abs(train_sharpe)) * 100 if train_sharpe != 0 else 0.0
        )

        return_degradation = (
            (
                (train_metrics['return_pct'] - test_metrics['return_pct'])
                / abs(train_metrics['return_pct'])
            )
            * 100
        ...
        logger.info(
            f"   📊 TRAIN: PnL=${result_dict['train_total_pnl']:.2f}, Sharpe={result_dict['train_sharpe_ratio']:.2f}, Return={result_dict['train_return_pct']:.2f}%"
        )
        logger.info(
            f"   📉 Degradación: Sharpe={sharpe_degradation:.1f}%, Return={return_degradation:.1f}%"
        )
    ...
    def _detect_regime_for_quote(
        self, quote, price_history: List[float], atr_history: List[float]
    ) -> Dict[str, Any]:
        """
        ...
        analyzer = MarketAnalyzer(
            {
                ...
                'volatility_detection': market_config.get(
    ...
    def _filter_quotes_by_regime(
        self, regime_conditions: Dict[str, Any], regime_detections: List[Tuple[int, Dict[str, Any]]]
    ) -> List[int]:
        """
    ...
    def run_regime_test(self) -> List[Dict[str, Any]]:
        ...
        for regime in regimes:
            ...
            logger.info(
                f"  ✅ Encontrados {len(filtered_quotes)} quotes para régimen {regime_name} "
                f"({len(filtered_quotes)/len(self.quotes)*100:.1f}% del total)"
            )

            if len(filtered_quotes) < 10:
                logger.warning(
                    f"  ⚠️ Muy pocos quotes ({len(filtered_quotes)}) para backtest confiable"
                )
            ...
            logger.info(
                f"  ✅ {regime_name}: PnL=${result_dict['total_pnl']:.2f}, "
        ...
        if len(results) > 1:
            ...
            logger.info(
                f"    🏆 Mejor Sharpe: {best_sharpe['regime_name']} ({best_sharpe['sharpe_ratio']:.2f})"
            )
            logger.info(
                f"    💰 Mejor PnL: {best_pnl['regime_name']} (${best_pnl['total_pnl']:.2f})"
            )
            if best_win_rate:
                logger.info(
                    f"    📈 Mejor Win Rate: {best_win_rate['regime_name']} ({best_win_rate['win_rate']:.2f}%)"
                )
    ...
    def run_transformer_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(
                "⚠️ Transformer Engine no disponible. Usando optimización con grid search mejorado."
            )
            ...
            for param in parameters_to_optimize:
                ...
                if len(module_parts) == 2:
                    ...
                    if filter_name in self.config['modules']['filters']:
                        filter_config = self.config['modules']['filters'][filter_name]
                        if (
                            'parameters' in filter_config
                            and param_name in filter_config['parameters']
                        ):
                            param_config = filter_config['parameters'][param_name]
        ...
        logger.info(
            f"  🎯 Optimizando hasta {max_iterations} iteraciones (convergencia: {convergence_threshold})"
        )
        ...
        while iteration < max_iterations:
            ...
            if transformer_available and iteration > 1 and best_result:
                try:
                    ...
                    threshold_adjustments = self._transformer_suggest_adjustments(
                        current_thresholds, best_result, iteration
                    )
                    if threshold_adjustments:
                        for param, adjustment in threshold_adjustments.items():
                            current_thresholds[param] = max(
                                0.001, min(100.0, current_thresholds.get(param, 50) + adjustment)
                            )
                        logger.info(
                            f"    🔧 Ajustes sugeridos por Transformer: {threshold_adjustments}"
                        )
                except Exception as e:
                ...
                if iteration > 1:
                    ...
                    for param in current_thresholds.keys():
                        ...
                        current_thresholds[param] = max(
                            0.001, min(100.0, current_thresholds[param] + adjustment)
                        )
            ...
            try:
                ...
                if current_sharpe > best_sharpe:
                    ...
                    logger.info(
                        f"    ✅ Mejor Sharpe encontrado: {best_sharpe:.4f} (mejora: +{improvement:.4f})"
                    )
                ...
                if previous_sharpe is not None:
                    ...
                    if sharpe_change < convergence_threshold and iteration >= 5:
                        logger.info(
                            f"    🎯 Convergencia alcanzada: cambio {sharpe_change:.4f} < {convergence_threshold}"
                        )
                        break
        ...
        result_dict = {
            ...
            'final_capital': float(self.config['input']['initial_capital'])
            + (best_result['total_pnl'] if best_result else 0.0),
        }
    ...
    def _transformer_suggest_adjustments(
        self, current_thresholds: Dict[str, float], previous_result: Dict[str, Any], iteration: int
    ...
    def _create_multi_strategy_setup(
        self, enable_dynamic_reallocation: bool = True, reallocation_frequency_days: int = 30
        ...
        if arbitrage_config_dict:
            ...
            if "parameters" in arbitrage_config and isinstance(
                arbitrage_config["parameters"], dict
            ):
    ...
    def _run_multi_strategy_backtest_on_quotes(
        ...
        start_date = (
            quotes[0].timestamp
            if quotes
            else datetime.strptime(self.config['input']['start_date'], "%Y-%m-%d")
        )
        end_date = (
            quotes[-1].timestamp
            if quotes
            else datetime.strptime(self.config['input']['end_date'], "%Y-%m-%d")
        )

        # Ejecutar backtest multi-estrategia
        result = multi_backtester.run_multi_strategy_backtest(
            quotes=quotes, start_date=start_date, end_date=end_date
        )
        ...
        final_capital = Decimal(
            str(combined_metrics.get('total_final_capital', initial_capital_decimal))
        )
        consistent_metrics = self._calculate_consistent_metrics(
            type(
                'obj',
    ...
    def run_multi_strategy_backtest(self) -> Dict[str, Any]:
        ...
        try:
            result_dict = self._run_multi_strategy_backtest_on_quotes(
                quotes=self.quotes, test_name_prefix="Multi-Strategy Backtest"
            )
    ...
    def run_hyperparameter_optimization(self) -> List[Dict[str, Any]]:
        ...
        try:
            from app.strategies.momentum_modular.optimization.hyperparameter_optimizer import (
    ...
    def _create_strategy_config_from_hyperopt_config(
        self, hyperopt_config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _extract_thresholds_from_hyperopt_config(
        self, hyperopt_config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _compare_optimization_methods(self) -> None:
        ...
        grid_results = [r for r in self.results if r.get('test_type') == 'grid_search']
        hyperopt_results = [
            r for r in self.results if r.get('test_type') == 'hyperparameter_optimization'
        ]
        ...
        if grid_best and hyperopt_best:
            ...
            logger.info(
                f"   Preset: {hyperopt_best.get('hyperopt_config', {}).get('preset', 'N/A')}"
            )
            ...
            if hyperopt_sharpe > grid_sharpe:
                improvement = (
                    ((hyperopt_sharpe - grid_sharpe) / abs(grid_sharpe)) * 100
                    if grid_sharpe != 0
                    else 100
                )
                logger.info(
                ...
                improvement = (
                    ((grid_sharpe - hyperopt_sharpe) / abs(hyperopt_sharpe)) * 100
                    if hyperopt_sharpe != 0
                    else 100
                )
    ...
    def _save_test_audit_and_weights(
        self, test_result: Dict[str, Any], test_type: str, strategy: Optional[Any] = None
    ) -> None:
        """
        ...
        try:
            ...
            if self.audit_trail and self.audit_hash:
                result_path = str(
                    self.output_dir / f"{test_type}_{timestamp.replace(':', '-')}.json"
                )
                ...
                # pylint: disable=unsubscriptable-object
                hash_display = self.audit_hash[:16] if self.audit_hash else ''
            ...
            if self.learning_storage and strategy:
                if hasattr(strategy, 'learning_engine') and strategy.learning_engine:
                    if (
                        hasattr(strategy.learning_engine, 'model')
                        and strategy.learning_engine.model
                    ):
                        ...
                        try:
                            ...
                            logger.info(
                                f"✅ Pesos guardados para {test_type} ({engine_name}): {saved_path}"
                            )

                        except Exception as e:
                            logger.debug(
                                f"No se pudieron guardar pesos para {test_type} ({engine_name}): {e}"
                            )
    ...
    def run_specific_backtests(self, backtest_names: List[str]) -> pd.DataFrame:
        ...
        for name in backtest_names:
            if name in valid_names:
                ...
                logger.warning(
                    f"⚠️ Backtest desconocido: {name}. Opciones válidas: {list(valid_names.keys())}"
                )
        ...
        logger.info(
            f"✅ Backtests específicos completados. {len(self.results)} backtests ejecutados."
        )
    ...
    def _save_reports(self, df: pd.DataFrame):
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write(
                f"Period: {self.config['input']['start_date']} to {self.config['input']['end_date']}\n"
            )
            ...
            if not df.empty:
                ...
                for idx, row in top_10.iterrows():
                    ...
                    f.write(
                        f"  Win Rate: {row.get('win_rate', 0):.2f}%\n"
    ...
    def _generate_professional_reports(self, timestamp: str) -> None:
        ...
        if not self.backtest_results_objects:
            logger.debug(
                "No hay BacktestResult objects almacenados. Saltando reportes profesionales."
            )
            return
        ...
        if PYFOLIO_AVAILABLE:
            ...
            logger.debug(
                "pyfolio-reloaded no disponible. Instala con: pip install pyfolio-reloaded"
            )
        ...
        for test_name, backtest_result in self.backtest_results_objects:
            try:
                ...
                if not backtest_result.equity_curve:
                    logger.debug(
                        f"⚠️ No hay equity curve para {test_name}, saltando reporte quantstats"
                    )
                    continue

                # Crear DataFrame con equity curve
                equity_df = pd.DataFrame(
                    backtest_result.equity_curve, columns=['timestamp', 'equity']
                )
                ...
                if len(returns) < 10:
                    logger.debug(
                        f"⚠️ Pocos datos para {test_name} ({len(returns)} días), saltando reporte quantstats"
                    )
                    continue
                ...
                safe_test_name = "".join(
                    c for c in safe_test_name if c.isalnum() or c in ('_', '-')
                )[:50]
    ...
    def _generate_pyfolio_reports(self, timestamp: str) -> None:
        ...
        for test_name, backtest_result in self.backtest_results_objects:
            try:
                ...
                if not backtest_result.equity_curve:
                    logger.debug(
                        f"⚠️ No hay equity curve para {test_name}, saltando análisis pyfolio"
                    )
                    continue
                ...
                if len(returns) < 10:
                    logger.debug(
                        f"⚠️ Pocos datos para {test_name} ({len(returns)} días), saltando análisis pyfolio"
                    )
                    continue

                # Crear positions DataFrame desde trades
                positions = self._create_positions_df(
                    backtest_result.trades, backtest_result.start_date, backtest_result.end_date
                )
                ...
                try:
                    ...
                    perf_stats.to_json(
                        reports_dir / f"pyfolio_perf_{safe_test_name}_{timestamp}.json"
                    )
    ...
    def _create_positions_df(
        self, trades, start_date: datetime, end_date: datetime
    ) -> pd.DataFrame:
        """
        ...
        for trade in trades:
            ...
            quantity = (
                float(trade.quantity) if trade.side.value == 'BUY' else -float(trade.quantity)
            )
