...
class ProfileBatchBacktester:
    ...
    def _create_profile_config(self, profile: InputProfile) -> ConfigDict:
        """
        Create profile-specific configuration dict.

        Merges base configuration with profile-specific parameters
        for risk tolerance, objective, and capital tier.

        Args:
            profile: InputProfile to create config for

        Returns:
            Configuration dict containing:
            - input: Start/end dates
            - symbols: Trading symbols
            - strategy: Strategy parameters (merged risk + objective)
            - profile: Profile metadata (id, objective, risk, tier, horizon)

        Raises:
            No (uses defaults if config keys missing)

        Side Effects:
            None (creates new config dict)
        """
        """
        Create profile-specific configuration.
    ...
    def _run_baseline(
        ...
        Args:
            profile: InputProfile to test
            config: Configuration dict containing backtest parameters
            multi_strategy: If True, use multi-strategy execution mode

        Returns:
            Baseline metrics dict containing:
        ...
        try:
            ...
            logger.error(
                f"Baseline backtest failed for profile {profile.input_id}: {e}",
                extra={
                    "profile_id": profile.input_id,
                    "objective": profile.objetivo_inversion.value,
                    "risk_tolerance": profile.risk_tolerance.value,
    ...
    def _run_optimization_pipeline(
        ...
        """
        Run complete optimization pipeline with all validation phases.

        Pipeline phases:
        1. Baseline execution (or use provided metrics)
        ...
        Args:
            profile: InputProfile to optimize
            config: Configuration dict
            baseline_metrics: Pre-computed baseline metrics (optional)
            multi_strategy: If True, use multi-strategy optimization mode

        Returns:
            OptimizedStrategy containing:
            - profile_id: Profile identifier
            - baseline_metrics: Baseline performance
            - optimized_metrics: Optimized performance
            - best_parameters: Best parameter values found
            - optimization_history: Optuna trial history
            - walk_forward_results: Walk-forward validation results
            - monte_carlo_results: Monte Carlo simulation results
            - out_of_sample_results: Out-of-sample test results
            - comparison: BaselineOptimizationComparison object
            - ready_for_paper_trading: Deployment readiness flag
            - recommendation: Text recommendation

        Raises:
            RuntimeError: If optimization pipeline fails
            DatabaseError: If storage fails

        Side Effects:
            Runs Optuna study with multiple trials
            Executes multiple validation backtests
            Stores intermediate results via services
        """
        """
    ...
    def _run_bayesian_optimization(
        ...
        Searches parameter space for optimal strategy parameters using
        ...
        Returns:
            Optimization results dict containing:
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            try:
                ...
                logger.warning(
                    f"Optimization trial failed for profile {profile.input_id}: {e}",
                    extra={
                        "profile_id": profile.input_id,
                        "trial_params": params,
    ...
    def _run_backtest_with_params(
        ...
        Executes a single backtest with the given strategy parameters.
        Used by optimization and validation phases.
        ...
        Returns:
            Backtest metrics dict (single or multi-strategy) containing:
            - total_pnl: Total profit/loss
            - return_pct: Return percentage
            - sharpe_ratio: Sharpe ratio
            - max_drawdown: Maximum drawdown
            ...
            - total_trades: Total number of trades
            - winning_trades: Number of winning trades
            - per_strategy_results: Per-strategy metrics (if multi-strategy)
        ...
        try:
            ...
            logger.error(
                f"Backtest with params failed for profile {profile.input_id}: {e}",
                extra={
                    "profile_id": profile.input_id,
                    "params": params,
                    "multi_strategy": multi_strategy,
    ...
    def _run_walk_forward(
        ...
        """
        Run walk-forward validation using Tomasini methodology.

        Validates strategy robustness by testing on rolling windows
        with periodic re-optimization.
        ...
        Returns:
            Validation results dict containing:
            - passed: Whether validation passed
            - avg_sharpe: Average Sharpe ratio across test windows
            - std_sharpe: Standard deviation of Sharpe ratios
            - n_windows: Number of windows tested
            - success_rate: Percentage of windows with positive Sharpe
            - avg_decay: Average Sharpe decay from train to test
        ...
        Side Effects:
            Executes multiple backtest windows (train/test splits)
    ...
    def _run_monte_carlo(
        ...
        Assesses strategy robustness by simulating thousands of equity
        curve paths from historical returns.
        ...
        Returns:
            Validation results dict containing:
            - passed: Whether simulation passed confidence threshold
            - n_simulations: Number of simulations run
            - profitable_pct: Percentage of profitable simulations
            - avg_return: Average return across simulations
        ...
        Side Effects:
            Runs backtest to get returns
            Generates simulated return paths
        ...
        try:
            ...
            logger.error(
                f"Monte Carlo simulation failed for profile {profile.input_id}: {e}",
                extra={
                    "profile_id": profile.input_id,
                    "n_simulations": n_simulations,
                    "params": params,
    ...
    def _run_out_of_sample(
        ...
        """
        Run out-of-sample validation using holdout data.

        Tests strategy on unseen data to validate generalization
        ...
        Returns:
            Validation results dict containing:
            - passed: Whether OOS validation passed
            - sharpe_ratio: Sharpe ratio on OOS data
            - return_pct: Return percentage on OOS data
            - max_drawdown: Maximum drawdown on OOS data

        Raises:
            RuntimeError: If validation fails

        Side Effects:
            Executes backtest on OOS period
            Uses 20% holdout from validation config
        """
        """
        ...
        try:
            ...
            logger.error(
                f"Out-of-sample validation failed for profile {profile.input_id}: {e}",
                extra={
                    "profile_id": profile.input_id,
    ...
    def _aggregate_multi_strategy_results(
        ...
        """
        Aggregate multi-strategy results into combined metrics.

        Combines results from multiple strategies into a single
        aggregated result using weighted averages.

        Args:
            results: List of strategy results dicts
            profile: InputProfile for context

        Returns:
            Aggregated metrics dict containing:
            - strategy_name: "combined"
            - total_pnl: Sum of all PnL
            - return_pct: Weighted average return
            - sharpe_ratio: Average Sharpe ratio
            - max_drawdown: Minimum (worst) drawdown
            - win_rate: Overall win rate
            - total_trades: Total trades across all strategies
            - winning_trades: Total winning trades

        Raises:
            No (returns empty metrics if results is empty)

        Side Effects:
            None (pure aggregation function)
        """
        """
    ...
    def _apply_ensemble_voting(
        ...
        Combines signals from multiple strategies using configured
        ensemble method (voting, weighted, or regime selector).

        Args:
            profile: InputProfile for context
            strategy_mapping: Strategy mapping configuration
            per_strategy_signals: Signals dict from individual strategies

        Returns:
            Ensemble decision dict containing:
            - action: "buy", "sell", or "hold"
            - confidence: Confidence score (0-1)
            - meets_threshold: Whether confidence meets threshold
            - voting_breakdown: Vote counts/weights
            - ensemble_mode: Ensemble method used

        Raises:
            No (returns hold decision if no signals)
    ...
    def _safe_extract_first_result(
        ...
        Provides safe extraction with empty metrics fallback
        to prevent exceptions on empty results.
        ...
        Returns:
            First result dict, or empty metrics dict if results is empty

        Raises:
            No (returns empty metrics on error)
    ...
    def _get_empty_metrics(self) -> MetricsDict:
        """
        Get empty metrics dict with default values.

        Returns default metrics dict for error scenarios
        to prevent None access issues.

        Returns:
            Empty metrics dict containing zeros for all fields
