...
from app.backtesting.models import (
    BacktestConfig, BacktestResult, Trade, TradeStatus, PerformanceMetrics
)
...
class SimpleBacktester:
    ...
    def run_backtest(
        ...
        annualized_return = ((self.capital / self.config.initial_capital) ** (Decimal("1") / years) - Decimal("1")) * Decimal("100") if years > 0 else Decimal("0")
    ...
    def _execute_buy_signal(self, signal: Signal, market_data: MarketData):
        ...
        self.positions[signal.symbol] = self.positions.get(signal.symbol, Decimal("0")) + position_size
    ...
    def _execute_sell_signal(self, signal: Signal, market_data: MarketData):
        ...
        sell_quantity = min(current_position, self._calculate_position_size(signal, market_data.close_price))
    ...
    def _check_exit_conditions(self, market_data: MarketData):
        ...
        if self.config.stop_loss_percentage:
            stop_loss_price = entry_price * (Decimal("1") - self.config.stop_loss_percentage / Decimal("100"))
        ...
        if self.config.take_profit_percentage:
            take_profit_price = entry_price * (Decimal("1") + self.config.take_profit_percentage / Decimal("100"))
    ...
    def _calculate_performance_metrics(self) -> PerformanceMetrics:
        ...
        gross_profit = sum(trade.pnl for trade in winning_trades) if winning_trades else Decimal("0")
