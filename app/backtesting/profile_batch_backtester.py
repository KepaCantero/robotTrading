...
class ProfileBatchBacktester:
    ...
    def __init__(self, config_path: str):
        ...
        # Initialize database
        db_url = self.config.get("database", {}).get(
    ...
    def run_single_profile(
        ...
        optimized_strategy = self._run_optimization_pipeline(
            profile, profile_config, baseline_metrics=baseline_results, multi_strategy=multi_strategy
        )
    ...
    def _create_profile_config(self, profile: InputProfile) -> Dict[str, Any]:
        ...
        if not objective_params:
            # Track config key mismatch
            self._increment_fallback_counter("config_key_mismatch")
            logger.debug(f"Objective key '{objective_key}' not found in config, using empty defaults")
    ...
    def _run_walk_forward(
        ...
        if self.profile_config_loader is not None:
            try:
                ...
                logger.info(f"Falling back to workflow config: n_windows={n_windows}, train_pct={train_pct}")
        else:
            # Use workflow config
            wf_config = self.validation_config.get("walk_forward", {})
            n_windows = wf_config.get("n_windows", 5)
...
def create_profile_batch_backtester(config_path: str = "config/profile_batch_backtest.yaml") -> ProfileBatchBacktester:
    """
