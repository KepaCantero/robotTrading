...
class ParallelExecutor:
    ...
    def __init__(self, max_workers: Optional[int] = None, use_processes: bool = True):
        ...
        logger.info(f"ParallelExecutor inicializado: max_workers={self.max_workers}, use_processes={use_processes}")
    ...
    def run_parallel(
        ...
        logger.info(f"🚀 Ejecutando {len(tasks)} {task_name}(s) en paralelo (workers={self.max_workers})...")
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    if result:
                        ...
                        failed_count += 1
                        logger.warning(f"⚠️ {task_name} falló (retornó None): {task.get('name', 'unknown')}")
    ...
    def run_monte_carlo_parallel(
        ...
        **simulation_kwargs
        ...
        return self.run_parallel(
            tasks=tasks,
            task_function=simulation_function,
            task_name="Monte Carlo Simulation"
        )
    ...
    def run_grid_search_parallel(
        ...
        return self.run_parallel(
            tasks=tasks,
            task_function=combination_function,
            task_name="Grid Search Combination"
        )
    
    def run_learning_engines_parallel(
        self,
        engines: List[str],
        engine_function: Callable,
        **engine_kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        return self.run_parallel(
            tasks=tasks,
            task_function=engine_function,
            task_name="Learning Engine Test"
        )
