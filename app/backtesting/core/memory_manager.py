...
class AggressiveMemoryManager:
    ...
    def __init__(
        ...
        auto_monitor: bool = True,  # New: Enable automatic memory monitoring
    ):
        """
    ...
    def add_result(self, result: Dict[str, Any]) -> None:
        ...
        with self._lock:
            ...
            # Automatic memory monitoring (FIX: now actually runs)
    ...
    def add_backtest_object(self, key: str, obj: BacktestResult) -> None:
        ...
        with self._lock:
            ...
            # Create weak reference to prevent circular references (FIX)
    ...
    def _deep_cleanup_object(self, obj: Any) -> None:
        """
        Perform deep cleanup of an object to remove circular references (FIX).
    ...
    def _emergency_cleanup(self) -> None:
        ...
        """
        logger.critical("🚨 Emergency cleanup triggered - releasing memory")
        ...
        with self._lock:
            ...
            # Single GC pass (FIX: was 3 passes, which is excessive)
            gc.collect()
            ...
            logger.critical(
                ...
                f"BacktestObjects: {len(self._backtest_objects)}"
            )
    ...
    def _auto_check_memory(self) -> bool:
        """
        Internal automatic memory check (FIX: now actually called).
        ...
        try:
            ...
            # Log memory usage at regular intervals for monitoring (FIX: more frequent)
            if len(self._results) % 100 == 0 or len(self._backtest_objects) % 20 == 0:
                logger.debug(
                    f"Memory usage: {memory_mb:.0f}MB | "
                    f"Results: {len(self._results)} | "
                    f"BacktestObjects: {len(self._backtest_objects)} | "
                    f"WeakRefs: {len(self._backtest_refs)}"
                )
