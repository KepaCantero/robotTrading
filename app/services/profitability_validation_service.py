...
class ProfitabilityCalculator:
    ...
    def calculate_metrics(
        ...
        profit_margin = (net_profit / initial_capital) * 100 if initial_capital > 0 else Decimal('0')
        roi = profit_margin  # ROI es igual al profit margin en este contexto
        
        # Calcular métricas de trading
        win_rate, profit_factor, max_drawdown = self._calculate_trading_metrics(trades_data, initial_capital)
        
        # Calcular Sharpe ratio (simplificado)
        sharpe_ratio = self._calculate_sharpe_ratio(trades_data)
        
        # Calcular Cost Impact Ratio
        cost_impact_ratio = (cost_breakdown.total_costs / gross_profit) if gross_profit > 0 else Decimal('0')
    ...
    def _estimate_costs_from_trades(self, trades_data: List[Dict[str, Any]]) -> CostBreakdown:
        ...
        for trade in trades_data:
            ...
            estimated_trade_value = abs(pnl) * Decimal('10')  # Estimación conservadora
    ...
    def _calculate_sharpe_ratio(self, trades_data: List[Dict[str, Any]]) -> Optional[Decimal]:
        ...
        for trade in trades_data:
            ...
            return_pct = (pnl / Decimal('1000')) * 100  # Asumiendo capital base de 1000
...
class ProfitabilityValidator:
    ...
    def validate_profitability(
        ...
        if metrics.profit_margin >= criteria.min_profit_margin:
            ...
            failed_tests.append(f"min_profit_margin: {metrics.profit_margin}% < {criteria.min_profit_margin}%")
        ...
        if metrics.sharpe_ratio and metrics.sharpe_ratio >= criteria.min_sharpe_ratio:
            ...
            failed_tests.append(f"min_sharpe_ratio: {metrics.sharpe_ratio} < {criteria.min_sharpe_ratio}")
        
        # Test 5: Drawdown máximo
        if metrics.max_drawdown <= criteria.max_drawdown_limit:
            passed_tests.append("max_drawdown_limit")
        else:
            failed_tests.append(f"max_drawdown_limit: {metrics.max_drawdown}% > {criteria.max_drawdown_limit}%")
        ...
        if metrics.profit_factor >= criteria.min_profit_factor:
            ...
            failed_tests.append(f"min_profit_factor: {metrics.profit_factor} < {criteria.min_profit_factor}")
        ...
        if metrics.cost_impact_ratio <= criteria.max_cost_impact_ratio:
            ...
            failed_tests.append(f"max_cost_impact_ratio: {metrics.cost_impact_ratio} > {criteria.max_cost_impact_ratio}")
...
class ProfitabilityValidationService:
    ...
    def _calculate_average_metrics(
        ...
        sharpe_ratios = [v.metrics.sharpe_ratio for v in validations if v.metrics.sharpe_ratio is not None]
        avg_sharpe_ratio = Decimal(str(statistics.mean([float(sr) for sr in sharpe_ratios]))) if sharpe_ratios else None
    ...
    def _calculate_stability_score(
        ...
        net_profit_cv = statistics.stdev(net_profits) / abs(statistics.mean(net_profits)) if statistics.mean(net_profits) != 0 else 1
        margin_cv = statistics.stdev(profit_margins) / abs(statistics.mean(profit_margins)) if statistics.mean(profit_margins) != 0 else 1
    ...
    def _generate_recommendations(self, validation: ProfitabilityValidation) -> List[str]:
        ...
        if validation.metrics.cost_impact_ratio > Decimal('0.4'):
            recommendations.append("Optimizar costos operativos - considerar ejecución más eficiente")
    ...
    def _generate_general_recommendations(self, validations: List[ProfitabilityValidation]) -> List[str]:
        """Generar recomendaciones generales."""
        
        recommendations = []
        
        # Análisis de patrones comunes
        high_cost_strategies = [v for v in validations if v.metrics.cost_impact_ratio > Decimal('0.4')]
        high_drawdown_strategies = [v for v in validations if v.metrics.max_drawdown > Decimal('15')]
        ...
        if low_win_rate_strategies:
            recommendations.append("Mejorar criterios de entrada en estrategias con baja tasa de ganancia")
