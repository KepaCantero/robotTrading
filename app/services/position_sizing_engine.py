...
class PositionSizingEngine:
    ...
    def _calculate_base_position_size(
        ...
        win_probability = 0.6  # Probabilidad de ganancia estimada
    ...
    def _apply_risk_limits(
        ...
        min_position_value = available_capital * self.min_position_size
    ...
    def _apply_diversification_limits(
        ...
        max_sector_value = available_capital * self.max_sector_exposure
        ...
        max_total_value = available_capital * self.max_total_exposure
        ...
        if diversified_size != risk_limited_size:
            self.position_size_adjustments += 1
            logger.debug(f"Applied diversification limits: {risk_limited_size} -> {diversified_size}")
    ...
    def _apply_volatility_limits(
        ...
        if volatility_adjusted_size != diversified_size:
            self.position_size_adjustments += 1
            logger.debug(f"Applied volatility limits: {diversified_size} -> {volatility_adjusted_size}")
    ...
    def _calculate_sector_exposure(self, symbol: str, portfolio: Portfolio) -> Decimal:
        ...
        for position in portfolio.positions:
            # Asumir que todas las posiciones están en el mismo sector por simplicidad
