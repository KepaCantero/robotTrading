...
class CostAnalysisService:
    ...
    def analyze_trade_costs(self, trade: Trade, market_data: Dict[str, Any]) -> CostBreakdown:
        ...
        try:
            ...
            total_cost = commission + slippage + market_impact + infrastructure_cost + borrowing_cost
            ...
            cost_impact_ratio = (total_cost / gross_profit) * 100 if gross_profit > 0 else Decimal("0")
    ...
    def analyze_strategy_costs(self, trades: List[Trade], strategy_name: str, 
                             market_data: Dict[str, Any]) -> CostAnalysisResult:
        ...
        try:
            ...
            total_costs = total_commission + total_slippage + total_market_impact + total_infrastructure + total_borrowing
            ...
            recommendations = self._generate_recommendations(
                cost_impact_ratio, is_profitable, exceeds_cost_threshold, cost_breakdowns
            )
            
            return CostAnalysisResult(
                strategy_name=strategy_name,
                analysis_period=(min(t.entry_time for t in trades), max(t.exit_time for t in trades)),
    ...
    def _calculate_real_slippage(self, trade: Trade, market_data: Dict[str, Any], asset_class: str) -> Decimal:
    ...
    def _generate_recommendations(self, cost_impact_ratio: Decimal, is_profitable: bool, 
                                exceeds_threshold: bool, cost_breakdowns: List[CostBreakdown]) -> List[str]:
        """Generate recommendations based on cost analysis."""
        recommendations = []
        
        if not is_profitable:
            recommendations.append("Strategy is not profitable after costs. Consider reducing trade frequency or improving entry/exit timing.")
        
        if exceeds_threshold:
            recommendations.append(f"Cost Impact Ratio ({cost_impact_ratio:.1f}%) exceeds threshold ({self.max_cost_impact_ratio:.1f}%). Focus on reducing costs.")
        
        # Analyze cost components
        avg_slippage = sum(bd.slippage for bd in cost_breakdowns) / len(cost_breakdowns) if cost_breakdowns else Decimal("0")
        avg_commission = sum(bd.commission for bd in cost_breakdowns) / len(cost_breakdowns) if cost_breakdowns else Decimal("0")
        
        if avg_slippage > avg_commission * Decimal("2"):
            recommendations.append("Slippage costs are high relative to commissions. Consider using limit orders or trading during high-liquidity periods.")
        
        if avg_commission > Decimal("10"):
            recommendations.append("Commission costs are high. Consider negotiating better rates with broker or reducing trade frequency.")
        ...
        if len(high_cost_trades) > len(cost_breakdowns) * Decimal("0.2"):
            recommendations.append("More than 20% of trades have high costs (>5%). Review execution strategy.")
