...
from app.services.centralized_logging import centralized_logger, LogService, LogLevel
...
class TradingErrorHandler:
    ...
    def _initialize_error_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "signal_generation": {
                ...
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.RETRY, ErrorAction.CIRCUIT_BREAKER]
            },
            "order_placement": {
                "max_retries": 2,
                "retry_delay": 2.0,
                ...
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.RETRY, ErrorAction.CIRCUIT_BREAKER, ErrorAction.ALERT]
            },
            "order_execution": {
                "max_retries": 1,
                "retry_delay": 5.0,
                ...
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.RETRY, ErrorAction.ALERT, ErrorAction.KILL_SWITCH]
            },
            "portfolio_update": {
                "max_retries": 3,
                "retry_delay": 0.5,
                ...
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.RETRY, ErrorAction.ROLLBACK]
            },
            "risk_check": {
                "max_retries": 0,
                "retry_delay": 0,
                ...
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.KILL_SWITCH, ErrorAction.ALERT]
            },
            "market_data_fetch": {
                "max_retries": 5,
                "retry_delay": 1.0,
                "circuit_breaker_threshold": 10,
                "circuit_breaker_timeout": 60,
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.RETRY, ErrorAction.FALLBACK, ErrorAction.CIRCUIT_BREAKER]
            },
            "backtesting": {
                "max_retries": 2,
                "retry_delay": 5.0,
            ...
            "live_trading": {
                ...
                "actions": [ErrorAction.LOG_ONLY, ErrorAction.RETRY, ErrorAction.KILL_SWITCH, ErrorAction.ALERT]
            }
        }
    ...
    async def handle_error(
        ...
        for action in actions:
            try:
                result = await self._execute_action(action, error, context, operation_id, metadata, rules)
    ...
    def _track_error(self, operation_id: str, error: AlgoTradingError, context: ErrorContext) -> None:
    ...
    async def _log_error(
        ...
        if log_level == LogLevel.DEBUG:
            ...
            centralized_logger.error(service, error.message, log_metadata, str(error.original_error))
        elif log_level == LogLevel.CRITICAL:
            centralized_logger.critical(service, error.message, log_metadata, str(error.original_error))
    ...
    async def _check_kill_switch(
        ...
        if (context == ErrorContext.LIVE_TRADING and 
        ...
        return {
            "success": True,
            "kill_switch_should_activate": False
        }
