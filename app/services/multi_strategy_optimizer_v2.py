...
class MultiStrategyOptimizerV2:
    ...
    def __init__(
        ...
        self.best_config: dict[str, int | float] | None = None
    ...
    def _create_strategies(
        self, params: Mapping[str, int | float | str | bool]
    ) -> dict[str, object]:
    ...
    def _run_backtest(self, params: Mapping[str, int | float | str | bool]):
    ...
    def _calculate_objective(self, result: dict[str, object]) -> float:
        ...
        # Extract metrics
        sharpe = float(combined.get("weighted_sharpe", 0.0) or 0.0)
        max_dd = abs(float(combined.get("weighted_max_drawdown", 0.0) or 0.0))

        # Count trades per strategy
        strategy_results_raw = result.get("strategies", {})
        strategy_results: dict[str, dict[str, object]] = (
            strategy_results_raw if isinstance(strategy_results_raw, dict) else {}
        )
        ...
        for strategy_name, strategy_result in strategy_results.items():
            if isinstance(strategy_result, dict):
                ...
                trades_by_strategy[strategy_name] = (
                    int(trades) if isinstance(trades, (int, float)) else 0
                )

        total_trades = int(combined.get("total_trades", 0) or 0)
