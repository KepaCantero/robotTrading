...
class StrategyStockAllocator:
    ...
    def filter_stocks(self, historical_data: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
        ...
        for ticker, df in historical_data.items():
            ...
            min_required_days = max(
                40, min(min_days, 126)
            )  # At least 40 days (RELAXED from 60), max 126 (~6 months)
            if len(df) < min_required_days:
                rejection_reasons.append(
                    f"Insufficient history: {len(df)} < {min_required_days} days (relaxed min={min_required_days}, lookback={min_days})"
                )
                if self.config.LOG_FILTER_REJECTIONS:
                    rejection_log.append(
                        f"{ticker}: Only {len(df)} days, need at least {min_required_days} (relaxed from {min_days})"
                    )
                continue
            ...
            if isinstance(df.index, pd.DatetimeIndex):
                ...
                if len(large_gaps) > max_allowed_gaps:
                    rejection_reasons.append(
                        f"Too many time gaps: {len(large_gaps)} gaps (max allowed={max_allowed_gaps:.0f})"
                    )
                    if self.config.LOG_FILTER_REJECTIONS:
                        rejection_log.append(
            ...
            )  # 2% of requirement (was 5%) = ~$10k minimum

            if avg_liquidity_usd < min_liquidity_required:
                rejection_reasons.append(
                    f"Insufficient liquidity: ${avg_liquidity_usd:,.0f} < ${min_liquidity_required:,.0f} (2% of ${self.config.MIN_LIQUIDITY_USD:,.0f})"
                )
                if self.config.LOG_FILTER_REJECTIONS:
                    rejection_log.append(
    ...
    def calculate_garch_volatility(self, returns: pd.Series) -> float | None:
        ...
        if len(returns) < 50:
            logger.debug(
                "Insufficient data for volatility calculation (need at least 50 observations)"
            )
            return None
    ...
    def score_momentum(self, ticker: str, data: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            if self.config.DYNAMIC_WINDOW_ENABLED and len(prices) >= self.config.SLOPE_WINDOW_MIN:
                    ...
                    if len(recent_prices) > 1 and np.std(x_window) > 0:
                        try:
                            ...
                            if mse < best_slope_mse:
                                best_slope_mse = mse
                                slope_pct = (
                                    (slope_coef / recent_prices[0]) * 100
                ...
                logger.debug(
                    f"{ticker}: Dynamic window selected - slope={slope_pct:.4f}%, ROC={roc_optimal:.4f}"
                )
            else:
            ...
            momentum_score = (
                ...
                + weights.get("Liquidity", 0.0)  # Half-life not critical for momentum
                * liquidity_score
            )
    ...
    def score_pairs_trading(
        ...
        try:
            ...
            if not STATSMODELS_AVAILABLE:
                logger.debug(
                    f"Pair {ticker1}-{ticker2}: statsmodels not available - cointegration test skipped"
                )
                return {
                    "score": 0.0,
                    "rejected": True,
                ...
                try:
                    ...
                    if len(residuals) > 0:
                        ...
                        spread_returns = pd.Series(
                            spread_diff / (np.abs(residuals[:-1]) + 1e-10)
                        )  # Normalized returns
                        ...
                        if garch_vol_spread is not None and garch_vol_spread > 0:
                            ...
                            garch_normalization_factor = (
                                garch_vol_spread / (spread_std * np.sqrt(252))
                                if spread_std > 0
                                else 1.0
                            )
                            garch_normalized_z = spread_z_score / max(
                                garch_normalization_factor, 0.01
                            )  # Avoid division by zero
                            logger.debug(
    ...
    def calculate_wcm_scores(
                    ...
                    if isinstance(pair_symbols_raw, list):
                        if len(pair_symbols_raw) > 0 and isinstance(pair_symbols_raw[0], list):
                            ...
                            if len(pair_symbols_raw) >= 2:
                                configured_pairs_to_evaluate = [
                                    (pair_symbols_raw[0], pair_symbols_raw[1])
                                ]
        ...
        if configured_pairs_to_evaluate:
            ...
            if pairs_to_evaluate:
                logger.info(
                    f"Generated {len(pairs_to_evaluate)} pairs to evaluate (no configured pairs found)"
                )
        ...
        for ticker1, ticker2 in pairs_to_evaluate:
            ...
            if not pair_result.get("rejected", False):
                ...
                if configured_pairs_to_evaluate:
                    logger.debug(
                        f"Configured pair {ticker1}-{ticker2} rejected: {pair_result.get('reason', 'Unknown')}"
                    )
    ...
    def allocate_capital_erc(
        ...
        try:
            ...
            if len(returns_dict) == 0:
                ...
                return dict.fromkeys(tickers, total_capital * equal_weight)
            ...
            if result.success:
                ...
                allocations = {
                    ticker: float(total_capital * w) for ticker, w in zip(tickers, optimal_weights)
                }
                ...
                logger.info(
                    f"ERC optimization successful: {len(allocations)} allocations, total=${sum(allocations.values()):,.2f}"
                )
                return allocations
            else:
    ...
    def allocate(
        ...
        pairs_tickers_count = len(
            [t for t, s in strategy_assignments.items() if s == "pairs_trading"]
        )
        if pairs_assigned > 0:
            logger.info(
                f"✅ Assigned {pairs_assigned} pairs ({pairs_tickers_count} unique tickers) to pairs_trading strategy"
            )
        ...
        for strategy, strategy_capital in strategy_allocations.items():
            if strategy not in strategy_groups:
                logger.warning(
                    f"Strategy '{strategy}' not in strategy_groups, skipping ${strategy_capital:,.2f}"
                )
            ...
            if len(strategy_tickers) == 0:
                logger.warning(
                    f"Strategy '{strategy}' has no tickers assigned, skipping ${strategy_capital:,.2f}"
                )
                strategy_capital_used[strategy] = 0.0
                continue
            ...
            if allocated_total > 0.01:
                ...
                for ticker in final_allocations:
                    ...
                    proportional_capital = (
                        old_capital * (allocated_total + unused_capital) / allocated_total
                    )
                    ...
                    if total_weight > max_weight_per_ticker:
                        ...
                        for _ticker, alloc in final_allocations.items():
                            if alloc.strategy == strategy:
                                ...
                                alloc.weight = (
                                    alloc.capital / total_capital if total_capital > 0 else 0.0
                                )
                        logger.debug(
                            f"{strategy}: Strategy exposure capped at {max_weight_per_ticker:.1%}"
                        )
                ...
                if remaining_unused > 0.01:
                    ...
                    available = [
                        ...
                        if alloc.capital < max_capital_per_ticker
                        and alloc.weight < max_weight_per_ticker
                    ]
                    ...
                    for ticker, alloc in available:
                        strategy_total = sum(
                            a.weight
                            for a in final_allocations.values()
        ...
        is_valid, errors = self.validate_assignment(
            final_allocations, total_capital, strategy_allocations
        )
