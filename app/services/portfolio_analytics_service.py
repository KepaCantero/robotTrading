...
from app.models.portfolio_analytics import (
    PerformanceMetrics, RiskMetrics, PortfolioAnalytics, PortfolioAllocation,
    PortfolioRebalance, PortfolioComparison, PerformancePeriod, RiskLevel,
    ExtendedPortfolio
)
from app.services.market_data_service import get_market_data_service, MarketDataService
from app.models.market_data import Quote, HistoricalData, DataFrequency
...
class PortfolioAnalyticsService:
    ...
    async def generate_portfolio_analytics(self, portfolio: ExtendedPortfolio) -> PortfolioAnalytics:
        ...
        recommendations = self._generate_recommendations(portfolio, performance_metrics, risk_metrics)
    ...
    async def analyze_portfolio_allocation(self, portfolio: ExtendedPortfolio) -> PortfolioAllocation:
        ...
        for position in sorted(portfolio.positions, key=lambda p: p.market_value, reverse=True)[:10]:
            top_holdings.append({
                ...
                'weight': (position.market_value / total_value * 100) if total_value > 0 else Decimal('0'),
    ...
    async def generate_rebalance_recommendation(
        ...
        equity_deviation = abs(current_allocation.equity_allocation - target_allocation.equity_allocation)
        ...
        if equity_deviation >= rebalance_threshold:
            if current_allocation.equity_allocation > target_allocation.equity_allocation:
                ...
                excess_equity = (current_allocation.equity_allocation - target_allocation.equity_allocation) / 100 * portfolio.total_value
                ...
                deficit_equity = (target_allocation.equity_allocation - current_allocation.equity_allocation) / 100 * portfolio.total_value
                rebalance_actions.append({
                    'action': 'buy_equity',
        ...
        estimated_cost = Decimal('0')  # Simplified - would need transaction cost data
        risk_impact = self._estimate_rebalance_risk_impact(current_allocation, target_allocation)
        return_impact = self._estimate_rebalance_return_impact(current_allocation, target_allocation)
    ...
    async def _get_portfolio_values(self, portfolio: Portfolio, start_date: datetime, end_date: datetime) -> List[Decimal]:
        """Get historical portfolio values."""
    ...
    def _calculate_annualized_return(self, returns: List[Decimal], period: PerformancePeriod) -> Decimal:
    ...
    def _calculate_jensen_alpha(self, returns: List[Decimal]) -> Decimal:
        ...
        alpha = (avg_return - self._risk_free_rate / 252) - beta * (benchmark_return - self._risk_free_rate / 252)
    ...
    async def _get_benchmark_return(self, start_date: datetime, end_date: datetime) -> Optional[Decimal]:
    ...
    def _calculate_risk_score(self, risk_metrics: RiskMetrics) -> Decimal:
        ...
        volatility_score = min(risk_metrics.annualized_volatility, Decimal('50')) / Decimal('50') * 40
        concentration_score = min(risk_metrics.largest_position_weight, Decimal('50')) / Decimal('50') * 30
        correlation_score = risk_metrics.average_correlation * 30
        
        return volatility_score + concentration_score + correlation_score
    
    def _calculate_health_score(self, portfolio: Portfolio, performance: PerformanceMetrics, risk: RiskMetrics) -> Decimal:
        """Calculate portfolio health score."""
        # Performance component (40%)
        performance_score = min(max(performance.annualized_return, Decimal('0')), Decimal('20')) / Decimal('20') * 40
        
        # Risk component (30%)
        risk_score = max(Decimal('100') - self._calculate_risk_score(risk), Decimal('0')) * Decimal('0.3')
        
        # Diversification component (30%)
        diversification_score = min(risk.effective_number_of_positions / Decimal('10'), Decimal('1')) * 30
    ...
    def _calculate_diversification_score(self, portfolio: Portfolio) -> Decimal:
        ...
        effective_positions = self._calculate_effective_positions(portfolio)
        max_positions = Decimal('20')  # Assume 20 positions is well diversified
    ...
    def _calculate_liquidity_score(self, portfolio: Portfolio) -> Decimal:
        ...
        cash_ratio = portfolio.cash_balance / portfolio.total_value if portfolio.total_value > 0 else Decimal('0')
        
        # Higher cash ratio = higher liquidity score
        return min(cash_ratio * 100, Decimal('100'))
    
    def _generate_recommendations(self, portfolio: Portfolio, performance: PerformanceMetrics, risk: RiskMetrics) -> List[str]:
        ...
        if risk.largest_position_weight > Decimal('20'):
            recommendations.append("Concentration risk high - consider diversifying largest positions")
    ...
    def _generate_warnings(self, portfolio: Portfolio, performance: PerformanceMetrics, risk: RiskMetrics) -> List[str]:
        ...
        if risk.largest_position_weight > Decimal('30'):
            warnings.append(f"Extreme concentration: {risk.largest_position_weight:.2f}% in largest position")
    ...
    def _estimate_rebalance_risk_impact(self, current: PortfolioAllocation, target: PortfolioAllocation) -> Decimal:
        """Estimate risk impact of rebalancing."""
        # Simplified calculation
        equity_change = abs(current.equity_allocation - target.equity_allocation)
        return equity_change * Decimal('0.1')  # 0.1% risk impact per 1% allocation change
    
    def _estimate_rebalance_return_impact(self, current: PortfolioAllocation, target: PortfolioAllocation) -> Decimal:
        """Estimate return impact of rebalancing."""
        # Simplified calculation
        equity_change = target.equity_allocation - current.equity_allocation
        return equity_change * Decimal('0.05')  # 0.05% return impact per 1% allocation change
