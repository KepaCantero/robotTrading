    ...
    def calculate_new_allocations(
        ...
        for strategy_name in target_allocations.keys():
            ...
            if metrics is None:
                ...
                logger.warning(f"{strategy_name}: No performance data available, using neutral score")
        ...
        if total_normalized == 0:
            ...
            new_allocations = {
                name: total_capital * weight
                for name, weight in target_allocations.items()
            }
        else:
            ...
            for strategy_name in target_allocations.keys():
                ...
                blended_weight = max(float(self.min_weight), min(float(self.max_weight), blended_weight))
