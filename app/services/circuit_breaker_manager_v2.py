...
class CircuitBreakerConfig:
    ...
    # Market-wide thresholds (percentage drops)
    LEVEL_1_THRESHOLD = Decimal("-0.07")  # -7%
    LEVEL_2_THRESHOLD = Decimal("-0.13")  # -13%
    LEVEL_3_THRESHOLD = Decimal("-0.20")  # -20%

    # Single stock halt thresholds
    STOCK_VOLATILITY_THRESHOLD = Decimal("0.20")  # 20% move in 5 minutes
    STOCK_VOLUME_THRESHOLD = Decimal("0")  # No volume threshold

    # VIX thresholds
    VIX_HIGH = Decimal("40")  # Extreme volatility
    VIX_EXTREME = Decimal("60")  # Panic levels

    # Monitoring settings
    check_interval_seconds: float = 30.0
    market_index_symbol: str = "SPY"  # Use SPY as market proxy

    # Auto-resume settings
    auto_resume_on_halt_lift: bool = True
    halt_check_interval_seconds: float = 60.0
