...
class ClusteringRegimeDetector:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.regime_labels = ['bear', 'sideways', 'bull'] if self.n_clusters == 3 else [f'regime_{i}' for i in range(self.n_clusters)]
    ...
    def fit(self, prices: List[float]) -> bool:
        ...
        try:
            ...
            for i in range(20, len(prices)):
                window_prices = prices[i-window_size:i+1] if i >= window_size else prices[:i+1]
    ...
    def detect(self, prices: List[float]) -> Dict[str, Any]:
        ...
        try:
            ...
            regime = self.regime_labels[cluster] if cluster < len(self.regime_labels) else f'regime_{cluster}'
            ...
            if hasattr(self.model, 'cluster_centers_'):
                ...
                max_distance = np.max([np.linalg.norm(X[0] - c) for c in self.model.cluster_centers_])
