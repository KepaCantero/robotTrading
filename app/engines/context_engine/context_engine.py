...
class ContextEngine:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.correlation_regime_detector = CorrelationRegimeDetector(config.get('correlation_regime_config', {}))
        
        # Inicializar analizadores de volatilidad
        self.structural_change_detector = StructuralChangeDetector(config.get('structural_change_config', {}))
        self.volatility_regime_detector = VolatilityRegimeDetector(config.get('volatility_regime_config', {}))
        self.garch_analyzer = GARCHAnalyzer(config.get('garch_config', {}))
        
        # Inicializar analizadores de correlación
        self.rolling_correlation = RollingCorrelationAnalyzer(config.get('rolling_correlation_config', {}))
    ...
    def get_current_regime(
        ...
        if cache_key in self.cache:
            ...
            if datetime.now() - cached.get('timestamp', datetime.min) < timedelta(seconds=self.cache_ttl):
    ...
    def get_correlation_matrix(
        ...
        if method == 'rolling':
            ...
            if len(corr_matrix) > 0:
                network_result = self.network_analyzer.analyze_network(corr_matrix, list(returns_data.keys()))
                result = {
