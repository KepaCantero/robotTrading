    ...
    def fit(self, returns: List[float]) -> bool:
        ...
        try:
            ...
            if self.model_type == 'GARCH':
                self.model = arch_model(returns_array, vol='Garch', p=self.p, q=self.q, dist=self.dist)
            elif self.model_type == 'EGARCH':
                self.model = arch_model(returns_array, vol='EGARCH', p=self.p, q=self.q, dist=self.dist)
            elif self.model_type == 'GJR-GARCH':
                self.model = arch_model(returns_array, vol='GARCH', p=self.p, o=1, q=self.q, dist=self.dist)
            else:
