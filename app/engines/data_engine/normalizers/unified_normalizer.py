...
class UnifiedNormalizer:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.corporate_actions_handler = CorporateActionsHandler(config.get('corporate_actions_config', {}))
    ...
    def normalize_ohlcv_data(
        ...
        if self.corporate_actions_handler.actions_db.get(normalized_symbol):
            ...
            for price_field in ['open', 'high', 'low', 'close']:
                prices_dict[price_field] = self.corporate_actions_handler.apply_adjustments_to_prices(
                    normalized_symbol,
                    [Decimal(str(p)) if not isinstance(p, Decimal) else p for p in prices_dict[price_field]],
                    timestamps
                )
        ...
        for i, item in enumerate(data):
            normalized_item = {
                ...
                'open': prices_dict['open'][i] if i < len(prices_dict['open']) else item.get('open'),
                'high': prices_dict['high'][i] if i < len(prices_dict['high']) else item.get('high'),
                'low': prices_dict['low'][i] if i < len(prices_dict['low']) else item.get('low'),
                'close': prices_dict['close'][i] if i < len(prices_dict['close']) else item.get('close'),
    ...
    def normalize_quote(
        ...
        normalized_timestamp = self.timestamp_normalizer.normalize(
            ...
            source_timezone=quote_data.get('timezone')
        )
