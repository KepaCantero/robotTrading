...
# PostgreSQL (opcional)
try:
    from sqlalchemy import create_engine, Column, String, DateTime, LargeBinary, Text, Index
...
if POSTGRESQL_AVAILABLE:
    ...
    class CacheEntry(Base):
        ...
        __tablename__ = 'data_engine_cache'
...
class DistributedCache:
            ...
            try:
                ...
                if not postgres_url:
                    ...
                    if POSTGRESQL_AVAILABLE:
                        ...
                        CacheEntry.__tablename__ = postgres_schema.get('table_name', 'data_engine_cache')
                    
                    # Crear tablas si no existen
                    Base.metadata.create_all(self.postgres_engine)
                    logger.info("PostgreSQL cache inicializado")
            except Exception as e:
                logger.warning(f"No se pudo conectar a PostgreSQL: {e}. Usando cache en Redis/memoria.")
                self.use_postgres = False
        
        # Fallback: cache en memoria
        self.memory_cache: Dict[str, Dict[str, Any]] = {}
        
        logger.info(f"DistributedCache inicializado: Redis={self.use_redis}, PostgreSQL={self.use_postgres}")
    ...
    async def get(
        ...
        if self.use_postgres and self.postgres_session:
            try:
                entry = self.postgres_session.query(CacheEntry).filter(
                    CacheEntry.key == key,
    ...
    async def clear_expired(self) -> int:
        ...
        if self.use_postgres and self.postgres_session:
            try:
                expired = self.postgres_session.query(CacheEntry).filter(
