...
class FinancialModelingPrepSource(BaseDataSource):
    ...
    async def get_key_metrics(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with self.session.get(url, params={'apikey': self.api_key, 'limit': 5}) as response:
                if response.status != 200:
                    return []
...
class AlphaVantageFundamentalSource(BaseDataSource):
    ...
    async def connect(self) -> bool:
        ...
        if not AIOHTTP_AVAILABLE:
            logger.error("aiohttp no disponible. AlphaVantageFundamentalSource no puede conectarse.")
            self.last_error = "aiohttp no disponible"
            return False
        ...
        try:
            ...
            async with self.session.get(self.base_url, params=params) as response:
                ...
                if 'Error Message' in data or 'Note' in data:
                    logger.warning(f"Alpha Vantage error para {symbol}: {data.get('Error Message', data.get('Note'))}")
                    return None
