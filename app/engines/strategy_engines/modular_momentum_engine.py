...
class ModularMomentumStrategyEngine(BaseStrategyEngine):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"✅ ModularMomentumStrategyEngine inicializada (preset: {self.preset}, "
    ...
    def _generate_signals_impl(self, market_data: Quote) -> List[Signal]:
        ...
        try:
            ...
            confidence = self._calculate_signal_confidence(filter_results, None)  # Learning se aplicará después
            ...
            logger.error(f"Error generando señal en ModularMomentumStrategyEngine: {e}", exc_info=True)
            ...
            try:
                ...
                if context_result:
                    ...
                    market_context = {
                        ...
                        'volatility_regime': volatility_result.get('regime', 'normal') if volatility_result else 'normal',
                        'trend_strength': context_result.get('regime_probabilities', {}).get('bull', 0.0),
                        'volatility_percentile': volatility_result.get('percentile', 50) if volatility_result else 50,
                        'in_range': False  # ContextEngine no proporciona esto directamente
                    }
                    return market_context
            except Exception as e:
                logger.warning(f"Error usando ContextEngine, usando MarketAnalyzer como fallback: {e}")
    ...
    def risk_check(self, signal: Signal, portfolio: Portfolio) -> bool:
        ...
        if signal.confidence < (self.min_success_probability * 100):
            logger.debug(f"Risk check fallido: confidence {signal.confidence:.2f} < {self.min_success_probability * 100:.2f}")
            return False
