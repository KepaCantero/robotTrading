...
class PairsTradingStrategyEngine(BaseStrategyEngine):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if strategy_config:
            ...
            self.commission_per_trade_pct = Decimal(str(config.get("commission_per_trade_pct", 0.001)))
    ...
    def extract_features(self, market_data: Quote, historical_data: Optional[Sequence[Quote]] = None) -> Dict[str, Any]:
        ...
        if historical_data is None:
            ...
            # Separar por símbolo
            prices1 = [float(q.close or q.bid or q.last or 0) for q in historical_data if q.symbol == symbol1]
            prices2 = [float(q.close or q.bid or q.last or 0) for q in historical_data if q.symbol == symbol2]
        ...
        if len(prices1) >= self.lookback_period and len(prices2) >= self.lookback_period:
            ...
            if len(prices1) > 0 and len(prices2) > 0:
                ...
                features["spread_pct"] = float((prices1[-1] / prices2[-1] - 1) * 100) if prices2[-1] > 0 else 0.0
            ...
            if len(prices1) >= 30 and len(prices2) >= 30:
                try:
                    ...
                    if np.std(prices2_array) > 0:
                        hedge_ratio = np.cov(prices1_array, prices2_array)[0, 1] / np.var(prices2_array)
                        features["hedge_ratio"] = float(hedge_ratio) if not np.isnan(hedge_ratio) else 1.0
                    else:
            ...
            # Spread Z-score
            if len(prices1) >= self.lookback_period and len(prices2) >= self.lookback_period:
                spreads = [p1 - p2 for p1, p2 in zip(prices1[-self.lookback_period:], prices2[-self.lookback_period:])]
                if len(spreads) > 0 and np.std(spreads) > 0:
                    spread_mean = np.mean(spreads)
                    spread_std = np.std(spreads)
                    current_spread = spreads[-1]
                    spread_z_score = (current_spread - spread_mean) / spread_std
                    features["spread_z_score"] = float(spread_z_score) if not np.isnan(spread_z_score) else 0.0
    ...
    def _calculate_cointegration(self, prices1: List[float], prices2: List[float]) -> Optional[float]:
        """
    ...
    def _generate_signals_impl(self, market_data: Quote) -> List[Signal]:
        ...
        try:
            ...
            if len(self.price_history[symbol1]) < self.lookback_period or len(self.price_history[symbol2]) < self.lookback_period:
                return []
            ...
            is_cointegrated = cointegration_score is not None and cointegration_score > float(self.cointegration_threshold)
            ...
            if spread_z_score > float(self.min_spread_z_score):
                ...
                if market_data.symbol == symbol1:
                    signal = self._create_sell_signal(market_data, spread_z_score, cointegration_score, correlation)
                    signals.append(signal)
                elif market_data.symbol == symbol2:
                    signal = self._create_buy_signal(market_data, spread_z_score, cointegration_score, correlation)
                    signals.append(signal)
    ...
    def _create_buy_signal(self, market_data: Quote, spread_z_score: float, cointegration_score: Optional[float], correlation: float) -> Signal:
        """Crear señal de compra para pairs trading."""
        confidence = self._calculate_confidence(abs(spread_z_score), cointegration_score, correlation)
    ...
    def _create_sell_signal(self, market_data: Quote, spread_z_score: float, cointegration_score: Optional[float], correlation: float) -> Signal:
    ...
    def _calculate_confidence(self, abs_spread_z_score: float, cointegration_score: Optional[float], correlation: float) -> float:
        """
    ...
    def risk_check(self, signal: Signal, portfolio: Portfolio) -> bool:
        ...
        if total_exposure > self.max_total_exposure:
            logger.debug(f"Risk check fallido: exposición total {total_exposure:.2%} > {self.max_total_exposure:.2%}")
            return False
        
        # Verificar exposición por par
        pair_exposure = self._calculate_pair_exposure(portfolio)
        if pair_exposure > self.max_pair_exposure:
            logger.debug(f"Risk check fallido: exposición del par {pair_exposure:.2%} > {self.max_pair_exposure:.2%}")
            return False
        ...
        if cointegration_score is not None and cointegration_score < float(self.cointegration_threshold):
            logger.debug(f"Risk check fallido: cointegración {cointegration_score:.4f} < {float(self.cointegration_threshold):.4f}")
            return False
        
        if abs(correlation) < float(self.min_correlation):
            logger.debug(f"Risk check fallido: correlación {correlation:.4f} < {float(self.min_correlation):.4f}")
            return False
