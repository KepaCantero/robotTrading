...
class ThresholdRebalancer(BaseRebalancer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.min_rebalance_interval = timedelta(
            days=config.get('min_rebalance_interval_days', 1)
        )
...
class TimeBasedRebalancer(BaseRebalancer):
    ...
    def _set_frequency_interval(self) -> None:
        ...
        if self.rebalance_frequency == 'daily':
            ...
            self.interval = timedelta(days=1)
            self.logger.warning(f"Frecuencia desconocida: {self.rebalance_frequency}. Usando daily.")
...
class VolatilityTargetingRebalancer(BaseRebalancer):
    ...
    def should_rebalance(
        ...
        **kwargs
        ...
        weights_array = np.array([current_weights.get(s, 0.0) for s in sorted(current_weights.keys())])
...
class TransactionCostAwareRebalancer(BaseRebalancer):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.min_benefit_threshold = config.get('min_benefit_threshold', 0.001)  # 0.1% beneficio mínimo
    ...
    def should_rebalance(
        ...
        **kwargs
        ...
        total_cost = sum(
            abs(Decimal(str(trade['quantity']))) * prices.get(trade['symbol'], Decimal("1")) *
