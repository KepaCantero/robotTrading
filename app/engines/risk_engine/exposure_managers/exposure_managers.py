...
class ExposureManager(BaseExposureManager):
        ...
        **kwargs
        ...
        try:
            ...
            if strategy_allocations:
                strategy_exposure = self._analyze_strategy_exposure(
            ...
            return {
                ...
                'total_exposure': float(sum(pos.market_value for pos in portfolio.positions) / portfolio.total_equity) if portfolio.total_equity > 0 else 0.0,
        ...
        for position in portfolio.positions:
            ...
            if exposure > self.max_asset_exposure:
                violations.append({
                    ...
                    'exposure': exposure,
                    'limit': self.max_asset_exposure,
                    'severity': 'high' if exposure > self.max_asset_exposure * 1.5 else 'medium'
                })
        
        max_exposure = max(exposures.values(), key=lambda x: x['exposure'])['exposure'] if exposures else 0.0
    ...
    def _analyze_sector_exposure(self, portfolio: Portfolio) -> Dict[str, Any]:
        ...
        for position in portfolio.positions:
            sector = position.asset_class.value if hasattr(position.asset_class, 'value') else str(position.asset_class)
        ...
        for sector, value in sector_exposures.items():
            ...
            if exposure > self.max_sector_exposure:
                violations.append({
                    ...
                    'exposure': exposure,
                    'limit': self.max_sector_exposure,
                    'severity': 'high' if exposure > self.max_sector_exposure * 1.5 else 'medium'
                })
        ...
        for strategy, value in strategy_exposures.items():
            ...
            if exposure > self.max_strategy_exposure:
                violations.append({
                    ...
                    'exposure': exposure,
                    'limit': self.max_strategy_exposure,
                    'severity': 'high' if exposure > self.max_strategy_exposure * 1.5 else 'medium'
                })
    ...
    def _analyze_concentration(self, portfolio: Portfolio) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_5_weight': sum(sorted(weights, reverse=True)[:5]) if len(weights) >= 5 else sum(weights),
            'top_10_weight': sum(sorted(weights, reverse=True)[:10]) if len(weights) >= 10 else sum(weights)
        }
