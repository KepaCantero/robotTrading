...
class DrawdownController(BaseDrawdownController):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.strategy_max_drawdown_limit = config.get('strategy_max_drawdown_limit', 0.25)  # 25% por estrategia
    ...
    def _calculate_rolling_drawdown(self) -> Dict[str, Any]:
        ...
        if len(self.portfolio_value_history) >= self.rolling_window:
            drawdowns_window = []
            for i in range(len(self.portfolio_value_history) - self.rolling_window, len(self.portfolio_value_history)):
                window_values = [
                    float(entry['portfolio_value'])
                    for entry in self.portfolio_value_history[max(0, i - self.rolling_window):i+1]
                ]
                if window_values:
    ...
    def _check_circuit_breakers(
        ...
        strategy_drawdowns: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if current_drawdown > self.max_drawdown_limit:
            if not self.circuit_breaker_active:
                ...
                self.circuit_breaker_reason = f"Drawdown global {current_drawdown:.2%} > límite {self.max_drawdown_limit:.2%}"
        ...
        active_strategy_breakers = sum(
            1 for cb in self.strategy_circuit_breakers.values() if cb
        )
        
        return {
            'global_circuit_breaker_active': self.circuit_breaker_active,
            'global_circuit_breaker_reason': self.circuit_breaker_reason,
            'global_circuit_breaker_timestamp': self.circuit_breaker_timestamp.isoformat() if self.circuit_breaker_timestamp else None,
    ...
    def _assess_recovery(self, portfolio_drawdown: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.recovery_mode:
            if self.recovery_start_value and current_value > 0:
                recovery_from_peak = (current_value - float(self.recovery_start_value)) / float(self.recovery_start_value)
                ...
                if recovery_percentage >= self.recovery_threshold:
                    ...
                    self.recovery_mode = False
                    self.logger.info(f"✅ Recovery completo: {recovery_percentage:.2%} desde inicio recovery")
        
        return {
            'recovery_mode_active': self.recovery_mode,
            'recovery_complete': recovery_complete,
            'recovery_percentage': recovery_percentage,
            'recovery_start_value': float(self.recovery_start_value) if self.recovery_start_value else None,
            'current_drawdown': current_drawdown,
