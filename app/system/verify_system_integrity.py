...
class SystemIntegrityValidator:
    ...
    def _validate_data_engine(self) -> Dict[str, Any]:
        ...
        try:
            ...
            available_methods = [method for method in required_methods if hasattr(data_engine, method)]
            ...
            try:
                ...
                logger.info(f"  ✅ DataEngine data retrieval: OK (test data: {len(test_prices)} points)")
    ...
    def _validate_context_engine(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if volatility_result:
                    ...
                    if isinstance(volatility, (int, float)) and 0 <= volatility <= 10:
                        ...
                        logger.info(f"  ✅ Volatility regime: OK ({volatility_regime}, {volatility:.4f})")
                    else:
                        result["checks"]["volatility_regime"] = {
                            "status": "warning",
            ...
            try:
                ...
                correlation_result = context_engine.get_correlation_matrix(test_price_data, method='rolling')
    ...
    def _validate_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["status"] = "ok" if result["checks"]["data_flow"]["status"] == "ok" else "warning"
    ...
    def _validate_logs(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if log_files:
                ...
                logger.info(f"  ✅ Log files: OK ({len(log_files)} files, latest: {latest_log.name})")
            else:
                ...
                if log_file.exists():
                    ...
                    logger.warning(f"  ⚠️  Log files: Not found in {self.logs_dir} (may be written elsewhere)")
    ...
    def _write_dashboard_state(self) -> None:
        ...
        try:
            ...
            dashboard_state = {
                "data_engine_status": self.validation_results["checks"].get("data_engine", {}).get("status", "unknown"),
                ...
                "correlation_ok": self.validation_results["checks"].get("context_engine", {}).get("checks", {}).get("correlation_matrix", {}).get("status") == "ok",
            ...
            if isinstance(dashboard_state["volatility"], (int, float)):
                ...
                dashboard_state["volatility"] = max(0.0, min(1.0, float(dashboard_state["volatility"])))
