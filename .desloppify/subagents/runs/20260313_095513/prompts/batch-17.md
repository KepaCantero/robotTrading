...
Overview (640 signals):
  design_concern: 306 — alembic/env.py, app/__init__.py, ...
  mixed_responsibilities: 248 — app/api/cost_analysis.py, app/application/reporting/reporting_orchestrator.py, ...
...
For each concern, read the source code and report your verdict in issues[]:
  ...
  - [design_concern] app/backtesting/meta_analyzer/audit_trail.py
    ...
    fingerprint: 2c3fb879f7b6646a
  - [design_concern] app/backtesting/meta_analyzer/integration.py
    summary: Design signals from dict_keys, smells
    question: Review the flagged patterns — are they design problems that need addressing, or acceptable given the file's role?
    evidence: Flagged by: dict_keys, smells
    evidence: [smells] 1x Catch block that only logs (swallowed error)
    fingerprint: b83dd32b3d464bfe
  - [design_concern] app/backtesting/meta_analyzer/learning_storage.py
    summary: Design signals from smells, structural
    question: Review the flagged patterns — are they design problems that need addressing, or acceptable given the file's role?
    evidence: Flagged by: smells, structural
    evidence: File size: 518 lines
    fingerprint: 8f0552f825deb6df
  (+610 more — use `desloppify show <detector> --no-budget` to explore)
...
then read the actual source code.
  ...
  desloppify show dict_keys --no-budget      # 413 findings
  desloppify show dupes --no-budget      # 264 findings
  desloppify show facade --no-budget      # 3 findings
  desloppify show global_mutable_config --no-budget      # 186 findings
  desloppify show props --no-budget      # 6 findings
  desloppify show responsibility_cohesion --no-budget      # 37 findings
  desloppify show signature --no-budget      # 224 findings
  desloppify show single_use --no-budget      # 1 findings
  desloppify show smells --no-budget      # 1128 findings
  desloppify show structural --no-budget      # 469 findings
  desloppify show uncalled_functions --no-budget      # 19 findings
  desloppify show unused --no-budget      # 319 findings
