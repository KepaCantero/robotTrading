...
class TestPositionManagement(unittest.TestCase):
    ...
    def test_position_zeroed_after_sell(self):
        ...
        self.assertAlmostEqual(float(position_after), 0.0, places=2, msg="Position debe ser 0 después de vender toda la posición")
    ...
    def test_no_position_without_buy_trades(self):
        ...
        if len(open_buy_trades) == 0:
            self.assertAlmostEqual(
                float(position), 0.0, places=2, msg="Si no hay buy_trades abiertos, position debe ser 0"
            )
