...
from app.backtesting.models import BacktestConfig, BacktestResult, Trade, TradeStatus, PerformanceMetrics
...
class TestBacktestModels:
    ...
    def test_backtest_config_stop_loss_greater_than_take_profit(self):
        ...
        with pytest.raises(ValueError, match="Stop loss percentage must be less than take profit percentage"):
            BacktestConfig(
                initial_capital=Decimal("100000"),
                commission_per_trade=Decimal("1.0"),
                slippage_percentage=Decimal("0.1"),
