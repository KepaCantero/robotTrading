...
from app.services.cost_analysis_service import (
    CostAnalysisService,
    CostBreakdown,
    CostAnalysisResult,
    CostType
...
class TestCostAnalysisService:
    ...
    def test_analyze_trade_costs_limit_order(self):
        ...
        assert breakdown.slippage > Decimal("0")  # Still calculates slippage based on market conditions
        assert breakdown.commission > Decimal("0")
        assert breakdown.total_cost > Decimal("0")
    ...
    def test_analyze_strategy_costs_profitable(self):
        ...
        result = self.service.analyze_strategy_costs(trades, "Test Strategy", self.sample_market_data)
        
        assert result.strategy_name == "Test Strategy"
        assert result.total_trades == 1
        assert result.total_costs > Decimal("0")
        assert result.gross_profit == Decimal("500.00")
        assert result.net_profit < result.gross_profit  # Net profit should be less due to costs
    ...
    def test_analyze_strategy_costs_unprofitable(self):
        ...
        result = self.service.analyze_strategy_costs(trades, "Unprofitable Strategy", self.sample_market_data)
    ...
    def test_slippage_calculation_with_volatility(self):
        ...
        high_vol_breakdown = self.service.analyze_trade_costs(self.sample_trade, high_vol_market_data)
    ...
    def test_market_impact_calculation(self):
        ...
        large_breakdown = self.service.analyze_trade_costs(large_order_trade, self.sample_market_data)
        small_breakdown = self.service.analyze_trade_costs(small_order_trade, self.sample_market_data)
    ...
    def test_recommendations_generation(self):
        ...
        result = self.service.analyze_strategy_costs(high_cost_trades, "High Cost Strategy", self.sample_market_data)
    ...
    def test_error_handling_invalid_trade(self):
        ...
        # Even with invalid symbol, it should calculate costs (infrastructure cost is always applied)
        assert breakdown.total_cost > Decimal("0")  # Infrastructure cost is always applied
