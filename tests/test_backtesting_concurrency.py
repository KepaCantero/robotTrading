...
from app.core.centralized_config import StrategyConfig, get_config
...
class TestBacktestingConcurrency:
    ...
    def setup_method(self):
        ...
        # Crear mock para backtesting_service
        self.backtesting_service = Mock()

        async def mock_analyze_backtest_result(result):
            return {
        ...
        async def mock_optimize_parameters(config):
            return {
                "optimized_params": getattr(config, "parameters", {}),
                "performance_improvement": 0.1,
                "best_score": 0.9,
            }

        async def mock_compare_backtest_results(result, other_results):
            return {
                "rank": 1,
                "percentile": 0.8,
                "comparison_metrics": {"sharpe_vs_avg": 1.2, "return_vs_avg": 1.1},
            }

        def mock_execute_backtest_sync(config):
            return {
                "strategy_name": config.strategy_name,
                "final_capital": config.initial_capital * Decimal("1.05"),
                "total_return": Decimal("0.05"),
                "success": True,
            }

        async def mock_load_backtest_data(config):
            return [{"date": "2025-01-01", "price": 100.0} for _ in range(30)]

        async def mock_validate_backtest_config(config):
            # Solo validar configuraciones con nombres que empiecen con
            # "VALID_"
            return config.strategy_name.startswith("VALID_")

        async def mock_generate_backtest_report(result):
            return {
    ...
    async def test_concurrent_backtest_comparison(self):
        ...
        async def compare_backtest_results(
            result: BacktestResult, other_results: List[BacktestResult]
        ):
            try:
                comparison = await self.backtesting_service.compare_backtest_results(
        ...
        # Crear tareas concurrentes
        tasks = [
            asyncio.create_task(compare_backtest_results(result, backtest_results))
    ...
    async def test_concurrent_backtest_validation(self):
        ...
        invalid_configs = [
            ...
            BacktestConfig(
                ...
                # Fecha de inicio después del fin
