...
class TestPortfolioAnalyticsModels:
    ...
    def test_portfolio_allocation_creation(self):
        ...
        total = (allocation.equity_allocation + allocation.fixed_income_allocation + 
                allocation.cash_allocation + allocation.alternative_allocation)
...
class TestPortfolioAnalyticsService:
    ...
    async def test_generate_portfolio_analytics(self, analytics_service, mock_portfolio):
        ...
        assert analytics.risk_level in [RiskLevel.LOW, RiskLevel.MODERATE, RiskLevel.HIGH, RiskLevel.VERY_HIGH]
    ...
    def test_helper_methods(self, analytics_service):
        ...
        monthly_start = analytics_service._get_period_start_date(end_date, PerformancePeriod.MONTHLY)
        ...
        assert abs(returns[1] - Decimal('0.0476190476190476190476190476')) < Decimal('0.0000000000000000000000000001')  # ~4.76% return
        assert abs(returns[2] - Decimal('-0.0181818181818181818181818182')) < Decimal('0.0000000000000000000000000001')  # ~-1.82% return
    ...
    def test_risk_assessment_methods(self, analytics_service, mock_portfolio):
        ...
        # Effective positions can be greater than actual positions when there's concentration
    ...
    def test_score_calculation_methods(self, analytics_service, mock_portfolio):
        ...
        assert risk_level in [RiskLevel.LOW, RiskLevel.MODERATE, RiskLevel.HIGH, RiskLevel.VERY_HIGH]
        ...
        health_score = analytics_service._calculate_health_score(mock_portfolio, performance_metrics, risk_metrics)
...
class TestPortfolioAnalyticsAPI:
    ...
    def test_get_performance_metrics_with_period(self, client):
        ...
        for period in ["1d", "1w", "1m", "3m", "1y", "all"]:
            response = client.get(f"/portfolio-analytics/performance-metrics/{portfolio_id}?period={period}")
            assert response.status_code == 200
            data = response.json()
            assert data["success"] is True
    ...
    def test_get_rebalance_recommendation(self, client):
        ...
        response = client.post("/portfolio-analytics/rebalance", json={
            "portfolio_id": portfolio_id
        })
    ...
    def test_compare_portfolios(self, client):
        ...
        response = client.post("/portfolio-analytics/compare", json={
    ...
    def test_performance_metrics_post_endpoint(self, client):
        ...
        response = client.post("/portfolio-analytics/performance-metrics", json={
            "portfolio_id": portfolio_id,
            "period": "1m",
...
class TestPortfolioAnalyticsIntegration:
    ...
    async def test_full_analytics_workflow(self):
        ...
        total_allocation = (allocation.equity_allocation + allocation.fixed_income_allocation + 
                          allocation.cash_allocation + allocation.alternative_allocation)
