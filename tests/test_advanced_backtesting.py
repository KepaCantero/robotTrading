...
class TestAdvancedBacktestingMethods:
    ...
    def test_statistical_significance_test(self, config):
        ...
        if std_return > 0:
            ...
            assert abs(t_statistic) > 1.96, f"Returns not statistically significant: t={t_statistic}"
    ...
    def test_monte_carlo_simulation(self, config):
        ...
        assert std_return > 0, "Should have variation in results"
        ...
        # Verify that mean return is positive
        assert mean_return > 0, f"Mean return should be positive: {mean_return}"
    ...
    def test_trade_randomization(self, config):
        ...
        assert performance_difference > 0, \
    ...
    def test_bootstrap_analysis(self, config):
        ...
        bootstrap_mean = statistics.mean(float_bootstrap_results)
        ...
        assert ci_lower <= original_return_float <= ci_upper, \
        ...
        for param_name, param_values in parameter_tests:
            ...
            for param_value in param_values:
                ...
                test_config = BacktestConfig(
                    ...
                    commission_per_trade=param_value if param_name == "commission" else config.commission_per_trade,
                    slippage_percentage=param_value if param_name == "slippage" else config.slippage_percentage,
                    risk_free_rate=config.risk_free_rate,
                    max_position_size=param_value if param_name == "max_position_size" else config.max_position_size
                )
    ...
    def _generate_random_market_scenario(self, base_data: List[MarketData]) -> List[MarketData]:
        ...
        for md in base_data:
            ...
            random_data.append(MarketData(
                symbol=md.symbol,
                timestamp=md.timestamp,
                open_price=random_price * Decimal("0.999"),
    ...
    def _create_signals_for_scenario(self, market_data: List[MarketData]) -> List[Signal]:
        ...
        for i in range(0, len(market_data), 15):
            ...
            if i > 0:
                price_change = (md.close_price - market_data[i-1].close_price) / market_data[i-1].close_price
