...
class TestProfitabilityValidationService:
    ...
    def test_analyze_historical_performance(self):
        ...
        assert historical_analysis.consistency_rating in ["excellent", "good", "fair", "poor"]
...
class TestIntegrationTests:
    ...
    def test_end_to_end_validation(self):
        ...
        trades_data = [
            {'pnl': Decimal('150.0'), 'timestamp': '2025-01-01', 'symbol': 'AAPL', 'quantity': 10},
            {'pnl': Decimal('-75.0'), 'timestamp': '2025-01-02', 'symbol': 'GOOGL', 'quantity': 5},
            {'pnl': Decimal('200.0'), 'timestamp': '2025-01-03', 'symbol': 'MSFT', 'quantity': 8},
            {'pnl': Decimal('-50.0'), 'timestamp': '2025-01-04', 'symbol': 'TSLA', 'quantity': 3},
            {'pnl': Decimal('100.0'), 'timestamp': '2025-01-05', 'symbol': 'AMZN', 'quantity': 2}
        ]
        ...
        assert result.validation.metrics.net_profit < Decimal('325.0')  # Debe ser menor debido a costos
    ...
    def test_multiple_strategies_comparison(self):
        ...
        assert comparison.best_strategy in ["Momentum Strategy", "Mean Reversion Strategy", "Pairs Trading Strategy"]
    ...
    def test_historical_analysis(self):
        ...
        for month in range(1, 4):  # 3 meses de datos
            trades_data = [
                {'pnl': Decimal(f'{100 + month * 10}'), 'timestamp': f'2025-{month:02d}-01'},
                {'pnl': Decimal(f'{-50 - month * 5}'), 'timestamp': f'2025-{month:02d}-15'},
                {'pnl': Decimal(f'{200 + month * 20}'), 'timestamp': f'2025-{month:02d}-28'}
            ]
