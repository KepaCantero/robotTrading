...
from app.models.signal import Signal, SignalType, SignalStrength, SignalSource, MarketData
...
class TestCompleteTradingWorkflow:
    ...
    def test_multiple_signals_priority_workflow(self, client):
        ...
        if next_data["success"]:
            ...
            # Should have processed 3 signals, executed some (may include signals from previous tests)
            assert stats_data["signals_processed"] >= 3
            assert stats_data["signals_executed"] >= 0  # May be 0 if execution failed
    ...
    def test_portfolio_diversification_workflow(self, client):
        ...
        assert portfolio_data["positions_count"] >= 3  # Should have multiple positions
        assert portfolio_data["total_equity"] > 0
...
class TestErrorHandlingWorkflows:
    ...
    def test_signal_threshold_workflow(self, client):
        ...
        if eval_data["success"]:
            assert eval_data["signal"]["confidence"] >= 60.0  # Signal should be accepted with lowered thresholds
...
class TestPerformanceWorkflows:
    ...
    def test_high_volume_signal_processing(self, client):
        ...
        # Should have processed all signals (may include signals from previous tests)
        assert stats_data["signals_processed"] >= 20
        assert stats_data["queue_size"] >= 20
    ...
    def test_concurrent_signal_execution(self, client):
        ...
        # Should have executed some signals (may not execute all due to portfolio constraints)
        assert stats_data["signals_executed"] >= 1
    ...
    def test_memory_usage_under_load(self, client):
        ...
        assert stats_data["signals_processed"] >= 100
