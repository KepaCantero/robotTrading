...
class TestTechnicalIndicatorsEdgeCases:
    ...
    def test_rsi_with_nan_values(self):
        ...
        prices = [100.0, float('nan'), 105.0, 110.0, 108.0, 112.0, 115.0, 118.0, 120.0, 122.0, 125.0, 128.0, 130.0, 132.0, 135.0]
        
        # RSI calculation doesn't explicitly check for NaN, so it might return a value
        rsi = TechnicalIndicatorCalculator.calculate_rsi(prices)
    ...
    def test_rsi_with_infinite_values(self):
        ...
        prices = [100.0, float('inf'), 105.0, 110.0, 108.0, 112.0, 115.0, 118.0, 120.0, 122.0, 125.0, 128.0, 130.0, 132.0, 135.0]
        
        # RSI calculation doesn't explicitly check for infinity, so it might return a value
        rsi = TechnicalIndicatorCalculator.calculate_rsi(prices)
        # The actual behavior depends on how Python handles infinity in arithmetic
    ...
    def test_rsi_boundary_values(self):
        ...
        prices = [100.0, 100.0001, 100.0002, 100.0003, 100.0004, 100.0005, 100.0006, 100.0007, 100.0008, 100.0009, 100.0010, 100.0011, 100.0012, 100.0013, 100.0014]
        rsi = TechnicalIndicatorCalculator.calculate_rsi(prices)
        assert rsi is not None
        assert 0 <= rsi <= 100
        
        # Test with alternating gains and losses
        prices = [100.0, 101.0, 99.0, 101.0, 99.0, 101.0, 99.0, 101.0, 99.0, 101.0, 99.0, 101.0, 99.0, 101.0, 99.0]
        rsi = TechnicalIndicatorCalculator.calculate_rsi(prices)
        assert rsi is not None
        assert 0 <= rsi <= 100
    ...
    def test_ema_with_zero_values(self):
        ...
        prices = [100.0, 0.0, 105.0, 0.0, 110.0, 0.0, 108.0, 0.0, 112.0, 0.0, 115.0, 0.0, 118.0, 0.0, 120.0, 0.0, 122.0, 0.0, 125.0, 0.0]
        ema = TechnicalIndicatorCalculator.calculate_ema(prices, period=10)
    ...
    def test_ema_with_negative_values(self):
        ...
        prices = [-100.0, -105.0, -110.0, -108.0, -112.0, -115.0, -118.0, -120.0, -122.0, -125.0, -128.0, -130.0, -132.0, -135.0, -138.0]
        ema = TechnicalIndicatorCalculator.calculate_ema(prices, period=10)
        assert ema is not None
    ...
    def test_ema_with_extreme_values(self):
        ...
        prices = [1e10, 1e10 + 1000, 1e10 + 2000, 1e10 + 1500, 1e10 + 3000, 1e10 + 2500, 1e10 + 4000, 1e10 + 3500, 1e10 + 5000, 1e10 + 4500, 1e10 + 6000, 1e10 + 5500, 1e10 + 7000, 1e10 + 6500, 1e10 + 8000]
        ema = TechnicalIndicatorCalculator.calculate_ema(prices, period=10)
        assert ema is not None
        assert ema > 1e10  # Should handle large values
        
        # Test with very small values - EMA might return 0 due to rounding
        prices = [1e-10, 1e-10 + 1e-12, 1e-10 + 2e-12, 1e-10 + 1.5e-12, 1e-10 + 3e-12, 1e-10 + 2.5e-12, 1e-10 + 4e-12, 1e-10 + 3.5e-12, 1e-10 + 5e-12, 1e-10 + 4.5e-12, 1e-10 + 6e-12, 1e-10 + 5.5e-12, 1e-10 + 7e-12, 1e-10 + 6.5e-12, 1e-10 + 8e-12]
        ema = TechnicalIndicatorCalculator.calculate_ema(prices, period=10)
        assert ema is not None
        assert ema >= 0  # Should handle small values (might be 0 due to rounding)
    ...
    def test_macd_with_division_by_zero(self):
        ...
        assert macd == (0.0, 0.0, 0.0)  # MACD should be (0, 0, 0) for constant prices
    ...
    def test_atr_with_invalid_data(self):
        ...
        # ATR calculation doesn't validate high >= low, so it might return None due to insufficient data
    ...
    def test_volume_sma_with_zero_volume(self):
        ...
        volumes = [Decimal("1000.0"), Decimal("0.0"), Decimal("2000.0"), Decimal("0.0"), Decimal("1500.0"), Decimal("0.0"), Decimal("3000.0"), Decimal("0.0"), Decimal("2500.0"), Decimal("0.0"), Decimal("4000.0"), Decimal("0.0"), Decimal("3500.0"), Decimal("0.0"), Decimal("5000.0"), Decimal("0.0"), Decimal("4500.0"), Decimal("0.0"), Decimal("6000.0"), Decimal("0.0")]
        sma = TechnicalIndicatorCalculator.calculate_volume_sma(volumes, period=10)
    ...
    def test_indicators_with_empty_lists(self):
        ...
        assert TechnicalIndicatorCalculator.calculate_macd(empty_prices) == (None, None, None)
    ...
    def test_indicators_with_single_value(self):
        ...
        assert TechnicalIndicatorCalculator.calculate_macd(single_price) == (None, None, None)
        assert TechnicalIndicatorCalculator.calculate_atr([100.0], [100.0], [100.0]) is None
        assert TechnicalIndicatorCalculator.calculate_volume_sma(single_volume, period=1) == Decimal("1000.0")
    ...
    def test_indicators_with_very_large_periods(self):
        ...
        volumes = [Decimal("1000.0"), Decimal("2000.0"), Decimal("1500.0"), Decimal("3000.0"), Decimal("2500.0")]
        ...
        assert TechnicalIndicatorCalculator.calculate_macd(prices) == (None, None, None)  # MACD needs more data
        assert TechnicalIndicatorCalculator.calculate_atr([100.0, 105.0], [98.0, 103.0], [99.0, 104.0], period=100) is None
    ...
    def test_indicators_with_negative_periods(self):
        ...
        atr = TechnicalIndicatorCalculator.calculate_atr([100.0, 105.0], [98.0, 103.0], [99.0, 104.0], period=-1)
    ...
    def test_indicators_with_zero_periods(self):
        ...
        with pytest.raises(ZeroDivisionError):
            TechnicalIndicatorCalculator.calculate_atr([100.0, 105.0], [98.0, 103.0], [99.0, 104.0], period=0)
...
class TestRiskCalculationEdgeCases:
    ...
    def test_position_sizing_with_zero_price(self):
        ...
        # Should handle zero price gracefully - Decimal division doesn't raise ZeroDivisionError
        risk_amount = account_balance * risk_per_trade
        position_size = risk_amount / (price - stop_loss)
    ...
    def test_position_sizing_with_zero_stop_loss(self):
        ...
        # Should handle zero stop loss gracefully - Decimal division doesn't raise ZeroDivisionError
        risk_amount = account_balance * risk_per_trade
        position_size = risk_amount / (price - stop_loss)
...
class TestSignalEdgeCases:
    ...
    def test_signal_with_incomplete_inputs(self):
        ...
        # Test with missing required fields - empty symbol is allowed by Pydantic
        signal = Signal(
            symbol="",  # Empty symbol - Pydantic allows this
            signal_type=SignalType.BUY,
            strength=SignalStrength.STRONG,
            confidence=80.0,
            liquidity_score=75.0,
        ...
        # Test with None values - Pydantic will raise ValidationError
        with pytest.raises(Exception):  # Pydantic validation error
            Signal(
                symbol="AAPL",
                signal_type=None,  # None signal type
                strength=SignalStrength.STRONG,
                confidence=80.0,
                liquidity_score=75.0,
                priority_score=85.0,
