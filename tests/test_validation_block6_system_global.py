    ...
    def test_backtest_result_has_expected_structure(self):
        ...
        for json_file in json_files[:1]:  # Validar el primero
            ...
            if isinstance(data, dict):
                ...
                has_strategy = "strategy" in data or any("strategy" in str(k).lower() for k in data.keys())
                has_capital = "capital" in data or "initial_capital" in data or "final_capital" in data
                ...
                if not (has_strategy or has_capital or has_trades):
                    ...
                    def has_backtest_fields(obj, depth=0):
                        ...
                        if isinstance(obj, dict):
                            for key in obj.keys():
                                ...
                                if any(field in key_lower for field in ["strategy", "capital", "trade", "pnl"]):
                                    return True
                    ...
                    self.assertTrue(
                        has_fields,
                        f"JSON {json_file} debe tener campos relacionados con backtest"
                    )
                else:
                    self.assertTrue(
                        True,  # Ya tiene campos válidos
...
class TestReproducibility(unittest.TestCase):
    ...
    def test_same_backtest_same_results(self):
        ...
        self.assertEqual(
            len(result1.trades),
            len(result2.trades),
            "Número de trades debe ser idéntico"
        )
        ...
        self.assertEqual(
            pnl1,
            pnl2,
            f"PnL total debe ser idéntico: {pnl1} vs {pnl2}"
        )
...
class TestSharpeAndDrawdown(unittest.TestCase):
    ...
    def test_sharpe_calculation_correct(self):
        ...
        equity = [
            Decimal("100000"), Decimal("101000"), Decimal("102000"), Decimal("101500"),
            Decimal("103000"), Decimal("104000"), Decimal("103500"), Decimal("105000")
        ]
        ...
        if sharpe is not None:
            self.assertGreater(
                sharpe,
                Decimal("0"),
                f"Sharpe con retornos positivos debe ser > 0, got {sharpe}"
            )
    ...
    def test_drawdown_calculation_correct(self):
        ...
        equity = [
            Decimal("100000"), Decimal("105000"), Decimal("102000"),
            Decimal("95000"), Decimal("100000"), Decimal("110000")
        ]
