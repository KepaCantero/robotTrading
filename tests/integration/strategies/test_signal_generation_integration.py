...
def allocated_portfolio(allocator, real_historical_data):
    ...
    result = allocator.allocate(
        historical_data=real_historical_data, total_capital=total_capital, strategy_allocations=None
    )
    return result
...
class TestMultiStrategySignalGeneration:
    ...
    def test_multistrategy_portfolio_signals(
        self, allocator, real_historical_data, allocated_portfolio
    ):
        ...
        if allocated_portfolio.pairs and latest_quotes_by_strategy.get('pairs_trading'):
            ...
            for pair in allocated_portfolio.pairs[:5]:  # Test first 5 pairs
                ...
                if not ticker1_quotes or not ticker2_quotes:
                    if (
                        pair.ticker1 in real_historical_data
                        and pair.ticker2 in real_historical_data
                    ):
...
class TestMomentumOnlyPortfolio:
    ...
    def test_momentum_only_best_assets(self, allocator, real_historical_data):
        ...
        logger.info(
            f"✅ Generated {len(signals)} momentum signals from {len(top_symbols)} best assets"
        )
...
class TestMeanReversionOnlyPortfolio:
    ...
    def test_mean_reversion_only_best_assets(self, allocator, real_historical_data):
        ...
        logger.info(
            f"✅ Generated {len(signals)} mean reversion signals from {len(top_symbols)} best assets"
        )
...
class TestPairsTradingOnlyPortfolio:
    ...
    def test_pairs_trading_only_best_pairs(self, allocator, real_historical_data):
        ...
        for i, pair in enumerate(top_pairs, 1):
            logger.info(
                f"  {i}. {pair['symbol1']}-{pair['symbol2']}: score={pair['score']:.4f}, coint={pair['cointegration']:.4f}"
            )

        # Skip test if no pairs found (may happen with strict cointegration filters)
        if len(top_pairs) == 0:
            pytest.skip(
                "No cointegrated pairs found (may be expected with strict filters: p-value < 0.01, lookback 250 days)"
            )
        ...
        logger.info(
            f"✅ Generated {len(signals)} pairs trading signals from {len(top_pairs)} best pairs"
        )
