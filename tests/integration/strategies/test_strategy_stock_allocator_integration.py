...
class TestStrategyStockAllocatorIntegration:
    ...
    def test_real_data_loading(self, available_symbols, real_historical_data):
        ...
        logger.info(f"✅ Loaded {len(real_historical_data)}/{len(available_symbols)} symbols successfully")
        ...
        for symbol, df in list(real_historical_data.items())[:5]:  # Check first 5
            ...
            logger.info(f"  {symbol}: {len(df)} days, price range ${df['close'].min():.2f}-${df['close'].max():.2f}")
    ...
    def test_allocator_filters_stocks(self, allocator, real_historical_data):
        ...
        logger.info(f"✅ Filtered: {len(filtered)}/{len(real_historical_data)} stocks passed validation")
    ...
    def test_allocator_allocates_capital(self, allocator, real_historical_data, total_capital):
        ...
        assert len(result.allocations) >= 10, f"Should assign at least 10 tickers, got {len(result.allocations)}"
        ...
        for strategy, count in strategy_counts.items():
            strategy_capital = sum(
                m.capital for m in result.allocations.values() 
        ...
        assert total_allocated <= total_capital * 1.01, f"Allocated capital should not exceed total (got ${total_allocated:,.2f} > ${total_capital:,.2f})"
        ...
        for ticker, metrics in list(result.allocations.items())[:10]:  # Check first 10
            ...
            assert metrics.strategy in ['momentum', 'mean_reversion', 'pairs_trading'], \
    ...
    def test_allocator_minimum_tickers(self, allocator, real_historical_data, total_capital):
        ...
        if len(real_historical_data) >= min_expected:
            ...
            assert actual_count >= len(real_historical_data), \
    ...
    def test_allocator_capital_utilization(self, allocator, real_historical_data, total_capital):
        ...
        assert residual < total_capital * 0.35, \
