...
def test_allocation():
    ...
    print(
        f"   Symbols: {', '.join(sorted(historical_data.keys())[:10])}{'...' if len(historical_data) > 10 else ''}"
    )
    ...
    print(
        f"   {'Residual':20s}: {'':3s} {'':10s} ${result.residual_capital:>10,.2f} ({result.residual_capital/total_capital*100:5.1f}%)"
    )
    ...
    if result.pairs:
        ...
        for i, pair in enumerate(result.pairs[:5], 1):  # Show first 5
            print(
                f"   {i}. {pair.ticker1}-{pair.ticker2}: cointegration={pair.cointegration_score:.4f}"
            )
    ...
    strategies_with_allocs = len(
        [
            s
            for s in ['momentum', 'mean_reversion', 'pairs_trading']
