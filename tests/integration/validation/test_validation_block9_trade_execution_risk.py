    ...
    def test_partial_fill_when_insufficient_capital(self):
        ...
        if result.trades:
            for trade in result.trades:
                ...
                self.assertGreater(trade.quantity, 0, "Trade parcial debe tener cantidad > 0")
...
class TestRiskExposureAfterTrade(unittest.TestCase):
    ...
    def test_exposure_within_limit_after_trade(self):
        ...
        max_exposure = (
            self.strategy.max_exposure if hasattr(self.strategy, 'max_exposure') else Decimal("0.8")
        )
