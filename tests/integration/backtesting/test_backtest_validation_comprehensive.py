...
class TestDatasetIntegrity(unittest.TestCase):
    ...
    def test_ohlcv_normalized(self):
        ...
        for quote in quotes:
            ...
            self.assertGreaterEqual(quote.close, quote.low,
                f"Close < Low en {quote.timestamp}")
            self.assertLessEqual(quote.close, quote.high,
                f"Close > High en {quote.timestamp}")
            
            # Open debe estar entre Low y High
            self.assertGreaterEqual(quote.open, quote.low,
                f"Open < Low en {quote.timestamp}")
            self.assertLessEqual(quote.open, quote.high,
                f"Open > High en {quote.timestamp}")
            
            # Volume debe ser positivo
            self.assertGreater(quote.volume, 0,
                f"Volume <= 0 en {quote.timestamp}")
    ...
    def test_symbols_have_complete_series(self):
        ...
        if counts:
            ...
            for symbol, count in counts.items():
                self.assertGreaterEqual(count, expected_count * 0.9,
...
class TestTechnicalIndicatorsValidation(unittest.TestCase):
    ...
    def test_macd_converges_correctly(self):
        ...
        if macd is not None and signal is not None and hist is not None:
            self.assertAlmostEqual(hist, macd - signal, places=2, msg="Histogram debe ser MACD - Signal")
    ...
    def test_atr_calculates_volatility_correctly(self):
        ...
        if atr_high is not None and atr_low is not None:
            self.assertGreater(atr_high, atr_low,
                "ATR debe ser mayor en alta volatilidad")
...
class TestStrategySignalLogic(unittest.TestCase):
    ...
    def _create_quote_with_history(self, symbol: str, base_price: Decimal, 
                                   trend: str, days: int) -> Quote:
...
class TestStrategyFunctionalTests(unittest.TestCase):
    ...
    def test_no_exceptions_in_strategy_execution(self):
        ...
        for i in range(30):
            historical_quote = self._create_quote(
                "AAPL",
                Decimal("200") + Decimal(str(i * 0.5))
            )
...
class TestBacktestingEngineValidation(unittest.TestCase):
    ...
    def test_orders_execute_with_valid_prices(self):
        ...
        self.assertGreaterEqual(signal.price, quote.low,
            "Precio de señal debe ser >= Low")
        self.assertLessEqual(signal.price, quote.high,
            "Precio de señal debe ser <= High")
    ...
    def test_balance_updates_correctly(self):
        ...
        self.assertEqual(new_cash, initial_cash - (trade_price * shares),
    ...
    def test_pnl_calculation(self):
        ...
        self.assertEqual(pnl, expected_pnl,
            f"PnL calculado incorrectamente: {pnl} != {expected_pnl}")
...
class TestExpectedResults(unittest.TestCase):
    ...
    def test_drawdown_calculation(self):
        ...
        self.assertLess(max_drawdown, 0.5,  # < 50%
            f"Drawdown máximo {max_drawdown:.2%} es demasiado alto")
