    ...
    try:
        ...
        for engine_name in engines_to_test:
            ...
            if engine_name not in runner.config['learning_engines']:
                ...
                runner.config['learning_engines'][engine_name] = {'enabled': True, 'parameters': {}}
            ...
            for other_engine in engines_to_test:
                if other_engine in runner.config.get('learning_engines', {}):
                    runner.config['learning_engines'][other_engine]['enabled'] = (
                        other_engine == engine_name
                    )
            ...
            try:
                ...
                if engine_results:
                    ...
                    if 'after_training_metrics' in result:
                        ...
                        if after.get('total_pnl') == before.get('total_pnl') and after.get(
                            'sharpe_ratio'
                        ) == before.get('sharpe_ratio'):
                            logger.warning(
                                f"⚠️ {engine_name}: Métricas idénticas a baseline - posible fallo en entrenamiento"
                            )
                    ...
                    logger.warning(
                        f"⚠️ No se encontraron resultados para {engine_name} - puede no haberse ejecutado"
                    )
        ...
        if all_results:
                ...
                comparison_data.append(
                    {
                        ...
                        'Mejora Sharpe': f"{improvement.get('sharpe_ratio', 0):+.2f}%"
                        if improvement
                        else "N/A",
                    }
                )
            ...
            for data in comparison_data:
                print(
                    f"{data['Estrategia']:<20} {data['PnL ($)']:>12} {data['Return (%)']:>10} "
            ...
            best_sharpe = max(
                all_results,
                key=lambda x: x.get('after_training_metrics', x).get('sharpe_ratio', -999)
                if 'after_training_metrics' in x
                else x.get('sharpe_ratio', -999),
            )
            print(
            ...
            best_return = max(
                all_results,
                key=lambda x: x.get('after_training_metrics', x).get('return_pct', -999)
                if 'after_training_metrics' in x
                else x.get('return_pct', -999),
            )
            print(
                f"💰 Mejor Return: {best_return.get('test_name', 'Unknown')} "
            ...
            best_pf = max(
                all_results,
                key=lambda x: x.get('after_training_metrics', x).get('profit_factor', 0)
                if 'after_training_metrics' in x
                else x.get('profit_factor', 0),
            )
            print(
                f"📈 Mejor Profit Factor: {best_pf.get('test_name', 'Unknown')} "
            ...
            best_dd = min(
                all_results,
                key=lambda x: x.get('after_training_metrics', x).get('max_drawdown', 999)
                if 'after_training_metrics' in x
                else x.get('max_drawdown', 999),
            )
            print(
                f"🛡️  Menor Drawdown: {best_dd.get('test_name', 'Unknown')} "
            ...
            best_wr = max(
                all_results,
                key=lambda x: x.get('after_training_metrics', x).get('win_rate', 0)
                if 'after_training_metrics' in x
                else x.get('win_rate', 0),
            )
            print(
                f"🎯 Mejor Win Rate: {best_wr.get('test_name', 'Unknown')} "
                f"({best_wr.get('after_training_metrics', best_wr).get('win_rate', 0):.2f}%)"
            )

            # Comparativa vs Baseline (solo si Baseline fue ejecutado)
            baseline_result = next(
                (r for r in all_results if r.get('test_name') == 'Baseline'), None
            )
            if baseline_result:
                ...
                for result in all_results:
                    ...
                    if 'after_training_metrics' in result:
                        ...
                        sharpe_diff = metrics.get('sharpe_ratio', 0) - baseline_metrics.get(
                            'sharpe_ratio', 0
                        )
                        return_diff = metrics.get('return_pct', 0) - baseline_metrics.get(
                            'return_pct', 0
                        )
                        pnl_diff = metrics.get('total_pnl', 0) - baseline_metrics.get(
                            'total_pnl', 0
                        )

                        sharpe_arrow = (
                            "⬆️" if sharpe_diff > 0 else "⬇️" if sharpe_diff < 0 else "➡️"
                        )
                        return_arrow = (
                            "⬆️" if return_diff > 0 else "⬇️" if return_diff < 0 else "➡️"
                        )
                        pnl_arrow = "⬆️" if pnl_diff > 0 else "⬇️" if pnl_diff < 0 else "➡️"

                        print(f"\n{test_name}:")
                        print(
                            f"  Sharpe: {baseline_metrics.get('sharpe_ratio', 0):.2f} → {metrics.get('sharpe_ratio', 0):.2f} "
                            f"({sharpe_diff:+.2f}) {sharpe_arrow}"
                        )
                        print(
                            f"  Return: {baseline_metrics.get('return_pct', 0):.2f}% → {metrics.get('return_pct', 0):.2f}% "
                            f"({return_diff:+.2f}%) {return_arrow}"
                        )
                        print(
                            f"  PnL: ${baseline_metrics.get('total_pnl', 0):,.2f} → ${metrics.get('total_pnl', 0):,.2f} "
                            f"({pnl_diff:+,.2f}) {pnl_arrow}"
                        )
            else:
                print("\n" + "=" * 100)
                print(
                    "ℹ️  Baseline no ejecutado (SKIP_BASELINE=True) - Solo resultados de Deep Learning"
                )
                print("=" * 100)
