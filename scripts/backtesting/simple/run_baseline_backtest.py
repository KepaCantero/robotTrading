...
# ============================================================================
# CRITICAL: Thread safety configuration (must be BEFORE any imports)
# Prevents mutex.cc deadlocks on macOS with numpy, pandas, torch, etc.
# ============================================================================
...
def run_baseline_backtest(
    ...
    backtester = SimpleBacktester(
        strategy=strategy, initial_capital=initial_capital, enable_risk_envelope=True
    )
    ...
    results_data = {
        ...
        "performance": {
            ...
            "average_win": float(result.performance.average_win)
            if result.performance.average_win
            else 0,
            "average_loss": float(result.performance.average_loss)
            if result.performance.average_loss
            else 0,
            "profit_factor": float(result.performance.profit_factor)
            if result.performance.profit_factor
            else 0,
        ...
        "equity_curve": [{"date": str(d), "value": float(v)} for d, v in result.equity_curve],
        "trades": [
            {
    ...
    # Guardar JSON (with run_id for idempotency)
    json_path = output_dir / f"baseline_results_{symbol}_{run_id}_{timestamp}.json"
    with open(json_path, 'w') as f:
        json.dump(results_data, f, indent=2)
    logger.info(f"✅ Resultados guardados en: {json_path}")

    # Guardar equity curve CSV
    csv_path = output_dir / f"baseline_equity_curve_{symbol}_{run_id}_{timestamp}.csv"
    import pandas as pd
    ...
    # Guardar trades CSV
    trades_path = output_dir / f"baseline_trades_{symbol}_{run_id}_{timestamp}.csv"
    ...
    logger.info(
        f"Retorno total:        ${result.total_return:,.2f} ({result.total_return_percentage:.2f}%)"
    )
    ...
    logger.info(
        f"Sharpe ratio:         {result.performance.sharpe_ratio:.2f}"
        if result.performance.sharpe_ratio
...
def main():
    ...
    parser.add_argument(
        "--symbol", type=str, required=True, help="Símbolo a backtestear (ej: AAPL)"
    )
    parser.add_argument(
        "--start-date", type=str, required=True, help="Fecha de inicio (YYYY-MM-DD)"
    )
    parser.add_argument("--end-date", type=str, required=True, help="Fecha de fin (YYYY-MM-DD)")
    parser.add_argument(
        "--initial-capital", type=float, default=100000, help="Capital inicial (default: 100000)"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="docs/BACKTEST_RESULTS/baseline",
