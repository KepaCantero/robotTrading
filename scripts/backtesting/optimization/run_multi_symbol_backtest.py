...
def create_temp_config(symbol: str, config_dir: Path) -> Path:
    ...
    logger.info(
        f"✅ Created temp config for {symbol} ({PORTFOLIO[symbol]['sector']}): {config_path}"
    )
    return config_path
    ...
    try:
        ...
        if not results_df.empty:
            ...
            csv_path = (
                project_root
                / "reports"
                / "comprehensive_backtest_25years"
...
def create_aggregated_report(all_results: List[Dict[str, Any]]) -> None:
    ...
    all_tests = []
    strategy_performance = {'momentum': [], 'mean_reversion': [], 'pairs_trading': []}
    sector_performance = {}
    ...
    if all_tests:
        ...
        for row in top_15.to_dict('records'):
            print(
                f"  {row['symbol']:6} ({row['sector']:20}): Sharpe {row['sharpe_ratio']:6.2f} | Return {row['return_pct']:6.1f}% | PnL ${row['total_pnl']:9,.2f}"
            )
        ...
        for strategy, perf in strategy_performance.items():
            if perf:
                ...
                print(
                    f"  {strategy:20} | Avg Sharpe: {avg_sharpe:6.2f} | Avg Return: {avg_return:6.1f}% | Total PnL: ${total_pnl:10,.2f}"
                )
        ...
        for sector, metrics in sorted(
            sector_avg.items(), key=lambda x: x[1]['avg_sharpe'], reverse=True
        ):
            print(
                f"  {sector:25} | Avg Sharpe: {metrics['avg_sharpe']:6.2f} | Avg Return: {metrics['avg_return']:6.1f}% | Tests: {metrics['num_tests']}"
            )
        ...
        summary = {
            ...
            'strategy_comparison': {
                strategy: {
                    'avg_sharpe_ratio': sum([p['sharpe_ratio'] for p in perf]) / len(perf)
                    if perf
                    else 0,
                    'avg_return_pct': sum([p['return_pct'] for p in perf]) / len(perf)
                    if perf
                    else 0,
