...
def evaluate_pair(symbol1: str, symbol2: str, historical_data: dict) -> dict:
    ...
    if symbol1 not in historical_data or symbol2 not in historical_data:
        return {"error": f"Missing data: {symbol1 if symbol1 not in historical_data else symbol2}"}
...
def main():
    ...
    print("=" * 80)
    print(
        f"{'Pair':<15} {'Status':<12} {'Corr':<7} {'Coint-P':<8} {'Half-Life':<10} {'Score':<8} {'Notes'}"
    )
    print("-" * 80)
    ...
    for pair in CONFIGURED_PAIRS:
        ...
        if "error" in result:
            print(
                f"{pair_name:<15} {'ERROR':<12} {'N/A':<7} {'N/A':<8} {'N/A':<10} {'N/A':<8} {result['error']}"
            )
            continue
        ...
        print(
            f"{pair_name:<15} {status:<12} {correlation:<7.3f} {pvalue_str:<8} {half_life_str:<10} {score:<8.4f} {notes_str}"
        )
    ...
    if valid_count == 0:
        ...
        valid_results = sorted(
            [r for r in results if not r.get('rejected', True)],
            key=lambda x: x['score'],
            reverse=True,
        )
        for r in valid_results[:5]:
            print(
                f"  - {r['pair']}: Score={r['score']:.4f}, Corr={r['correlation']:.3f}, p-value={r['pvalue']:.4f}"
            )
