...
def download_symbol(
    symbol: str, start_date: datetime, end_date: datetime, output_file: Path, delay: float = 7.0
) -> bool:
...
def main():
    ...
    print("=" * 80)
    print(
        f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')} (10 years)"
    )
